...
static bool probe_bpf_atomics(void) {
	/* r0 = 0 (cmpxchg reads it as the expected value); r1 = 0; *(u64 *)(r10 - 8) = r1;
	 * r2 = 1; r0 = cmpxchg(r10 - 8, r0, r2); r0 = 0; exit */
...
/* Tell the probe whether it has atomics, and if it does not, take the cmpxchg out of the
 * programs altogether.
 *
...
int pman_prepare_maps_before_loading() {
	 ...
	 */
	const uint32_t auxmap_entries = (uint32_t)g_state.n_possible_cpus * AUXMAP_POOL_DEPTH;
	err = size_auxiliary_maps(g_state.skel, auxmap_entries);
