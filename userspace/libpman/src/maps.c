...
int pman_fill_syscalls_tail_table() {
	...
	for(int syscall_id = 0; syscall_id < SYSCALL_TABLE_SIZE; syscall_id++) {
		...
		int enter_event_type = g_syscall_table[syscall_id].enter_event_type;
		 ...
		 * event (except for enter events generated by TOCTOU mitigation programs, which we
		 * explicitly check in the below code). Until we miss some syscalls, this is not true so we
