...
int32_t scap_read_section_header(scap_t *handle, gzFile f)
{
	section_header_block sh;
	uint32_t bt;
	...
	// Read the section header block
	//
	if(gzread(f, &sh, sizeof(sh)) != sizeof(sh) ||
...
int32_t scap_read_init(scap_t *handle, gzFile f)
{
	...
	if((rc = scap_read_section_header(handle, f)) != SCAP_SUCCESS)
	{
		return rc;
	}
...
int32_t scap_next_offline(scap_t *handle, OUT scap_evt **pevent, OUT uint16_t *pcpuid)
{
	...
	while(true)
	{
		...
		// For the simple case of concatenating two capture
		// files, where the two capture files were both
		// generated by plugins, you might see section header
		// blocks. Just skip them.
		//
		// If the concatenated capture file is from a
		// non-plugin capture, this loop will run into one of
		// the other block types (user list, process list,
		// etc), and the read will still eventually fail.
		if(bh.block_type == SHB_BLOCK_TYPE)
		{
			int32_t rc;
