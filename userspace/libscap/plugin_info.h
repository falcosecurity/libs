...
typedef struct
{
	...
	// - s: the plugin context, returned by init(). Can be NULL.
	// - h: the capture context, returned by open(). Can be NULL.
	// - evt: pointer to a ss_plugin_event pointer. The plugin must
	//   allocate a ss_plugin_event struct and the data buffer within
	//   the ss_plugin_event struct.
	//   Once returned, both the struct and data buffer are owned by
	//   the framework until the next call to next(), next_batch(),
	//   or close().
	//
	...
	// Required: no
	// Arguments:
	// - evt: an event struct produced by a call to next()/batch_next().
