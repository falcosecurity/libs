...
bool scap_is_api_compatible(unsigned long driver_api_version, unsigned long required_api_version) {
	...
	return scap_apply_semver_check(driver_major,
	                               driver_minor,
	                               driver_patch,
	                               required_major,
	                               required_minor,
...
static int32_t check_api_compatibility_impl(uint64_t current_version,
...
int32_t check_api_compatibility(const struct scap_vtable* vtable,
	...
	if(vtable && vtable->get_schema_version) {
		...
		rc = check_api_compatibility_impl(version,
