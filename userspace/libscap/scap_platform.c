...
int32_t scap_platform_early_init(struct scap_platform* platform, char* lasterr, struct scap_open_args* oargs)
{
	...
	if(platform->m_vtable && platform->m_vtable->early_init_platform)
	{
		rc = platform->m_vtable->early_init_platform(platform, lasterr, oargs);
		if(rc != SCAP_SUCCESS)
		{
			scap_platform_close(platform);
		}
...
int32_t scap_platform_init(struct scap_platform* platform, struct scap_engine_handle engine, struct scap_open_args* oargs)
{
	...
	{
		rc = platform->m_vtable->init_platform(platform, engine, oargs);
		if(rc != SCAP_SUCCESS)
		{
			scap_platform_close(platform);
		}
