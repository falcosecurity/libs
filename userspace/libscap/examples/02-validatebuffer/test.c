	...
	*nevents = 0;
	...
	while(buf_len) {
		...
		if(buf_len < sizeof(struct ppm_evt_hdr)) {
			fprintf(stderr,
			        "Error: event not on buffer boundary, offset %x, data to read %d\n",
			        offset,
			        buf_len);
			return SCAP_FAILURE;
		}
		...
		event_size =
		        g_get_event_size(hdr->type,
		                         (uint16_t*)(copy_buffer + offset + sizeof(struct ppm_evt_hdr)));

		if(event_size == -1) {
			fprintf(stderr,
			        "Error: unrecognized event %u, cnt %u, offset %x\n",
			        (uint32_t)(hdr->type),
		...
		if(event_size <
		   sizeof(struct ppm_evt_hdr) + g_event_info[hdr->type].nparams * sizeof(uint16_t)) {
			fprintf(stderr,
			        "Error: event size too short %u, cnt %u, offset %x\n",
		...
		if(sentinel_begin != sentinel_end) {
			fprintf(stderr,
			        "Error: sentinel begin %d, sentinel end %d, evt_type %u, evt_size %zu, cnt %u, "
			        "offset %x, remaining %u\n",
			        sentinel_begin,
			        sentinel_end,
			        (uint32_t)hdr->type,
			        event_size,
		...
		if(sentinel_begin != *cur_event) {
			fprintf(stderr,
			        "Error1: sentinel begin %d, sentinel end %d, cnt %u, offset %x, remaining %u\n",
			        sentinel_begin,
			        sentinel_end,
			        *cur_event,
			        offset,
	...
	while(1) {
		for(j = 0; j < ndevs; j++) {
			...
			if(g_check_integrity(&(cur_evts[j]), buf, buflen, &nevents) != SCAP_SUCCESS) {
				fprintf(stderr,
				        "Integrity check failure at event %u.\nDumping buffer to dump.bin\n",
			...
			if(nloops == 1000) {
				printf(" %u)bps:%" PRIu64 " totbytes:%" PRIu64 " - evts/s:%" PRIu64
				       " totevs:%" PRIu64 " \n",
				       j,
