...
uint8_t* sinsp_filter_check_fd::extract(sinsp_evt *evt, OUT uint32_t* len, bool sanitize_strings)
{
	*len = 0;
	...
	case TYPE_FILE_HASH_CATEGORY:
		...
		{
			m_tstr.clear();
			int64_t hres = m_inspector->m_exe_hash_calculator->checksum_process_file(m_tinfo,
										  m_fdinfo->m_name,
										  (m_field_id == TYPE_FILE_MD5)?file_hash_calculator::HT_MD5 : file_hash_calculator::HT_SHA256,
										  true,
										  &m_tstr);

			if(hres != 0)
			{
				m_tstr = "<ERROR " + to_string(hres) + ">";
			}

			if(m_field_id == TYPE_FILE_SHA256 || m_field_id == TYPE_FILE_MD5)
			{
				RETURN_EXTRACT_STRING(m_tstr);
			}
