...
void sinsp_dns_resolver::refresh(uint64_t erase_timeout,
	...
	while(true) {
		if(!manager.m_cache.empty()) {
			...
			for(auto &it : manager.m_cache) {
				...
				if((ts > info.m_last_used_ts) && (ts - info.m_last_used_ts) > erase_timeout) {
		...
		if(f_exit.wait_for(std::chrono::nanoseconds(base_refresh_timeout)) ==
...
#if !defined(_WIN32) && !defined(__EMSCRIPTEN__)
inline sinsp_dns_manager::dns_info sinsp_dns_manager::resolve(const std::string &name,
                                                              uint64_t ts) {
	dns_info dinfo;
...
bool sinsp_dns_manager::match(const char *name, int af, void *addr, uint64_t ts) {
	...
	if(!m_resolver) {
		m_resolver = std::make_unique<std::thread>(sinsp_dns_resolver::refresh,
		                                           m_erase_timeout,
		                                           m_base_refresh_timeout,
		                                           m_max_refresh_timeout,
			...
			if(af == AF_INET6) {
				...
				if(info.m_v6_addrs.find(v6) != info.m_v6_addrs.end()) {
					...
					break;
			          ...
			          info.m_v4_addrs.find(*(uint32_t *)addr) != info.m_v4_addrs.end()) {
				info.m_last_used_ts = ts;
				ret = name;
				break;
			}
