...
TEST(table_registry, defs_and_access) {
	...
	public:
		...
		using dynamic_table_fields::add_field;
		using built_in_table<uint64_t>::add_field;
		using dynamic_table_fields::get_field;
		using built_in_table<uint64_t>::get_field;
...
TEST(thread_manager, table_access) {
	...
	auto tid_acc = table->get_field<int64_t>("tid");
	auto comm_acc = table->get_field<std::string>("comm");
	auto fdtable_acc = table->get_field<libsinsp::state::base_table*>("file_descriptors");
	...
	std::string tmpstr;
	auto dynf_acc = table->add_field<std::string>("some_new_field");
...
TEST(thread_manager, fdtable_access) {
	...
	const auto& dfield = subtable->add_field<std::string>("str_val");
	...
	ASSERT_NE(subtable->get_field<std::string>("str_val"), nullptr);

	// checking if the new field has been added to the other subtable
	ASSERT_NE(subtable2->get_field<std::string>("str_val"), nullptr);
...
TEST(thread_manager, env_vars_access) {
	...
	// getting an existing field
	auto sfield = subtable->get_field<std::string>("value");
	ASSERT_NE(sfield, nullptr);
