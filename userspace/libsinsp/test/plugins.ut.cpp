...
TEST_F(sinsp_with_test_input, plugin_subtables_array) {
	...
	auto dfield = subtable->get_field<std::string>("value");
	ASSERT_NE(dfield, nullptr);
...
TEST_F(sinsp_with_test_input, plugin_subtables_array_pair) {
	...
	auto dfield_first = subtable->get_field<std::string>("first");
	...
	auto dfield_second = subtable->get_field<std::string>("second");
