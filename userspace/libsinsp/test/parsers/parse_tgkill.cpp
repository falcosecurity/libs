...
TEST_F(sinsp_with_test_input, TGKILL_parse) {
	add_default_init_thread();
	open_inspector();

	constexpr int64_t return_value = 0;
	constexpr int64_t pid = 20;
	constexpr int64_t tid = 21;
	...
	// Check that the tid value is as expected.
	ASSERT_EQ(evt->get_param_by_name("tid")->as<int64_t>(), tid);
