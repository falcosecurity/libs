...
TEST_F(sinsp_with_test_input, SEMGET_parse) {
	...
	ASSERT_EQ(evt->get_param_by_name("key")->as<int32_t>(), key);
	// Check that the semnum value is as expected.
	ASSERT_EQ(evt->get_param_by_name("nsems")->as<int32_t>(), nsems);
	// Check that the cmd value is as expected.
	ASSERT_EQ(evt->get_param_by_name("semflg")->as<uint32_t>(), semflg);
}
