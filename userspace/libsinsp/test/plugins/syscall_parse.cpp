...
ss_plugin_rc plugin_parse_event(ss_plugin_t* s,
	...
	tmp.u64 = ev->evt->type;
	auto evtcounter =
	        ps->event_count_table->reader.get_table_entry(ps->event_count_table->table, &tmp);
	if(!evtcounter) {
		auto newentry =
		        ps->event_count_table->writer.create_table_entry(ps->event_count_table->table);
		tmp.u64 = ev->evt->type;
		evtcounter = ps->event_count_table->writer.add_table_entry(ps->event_count_table->table,
		                                                           &tmp,
		                                                           newentry);
		if(!evtcounter) {
	...
	if(SS_PLUGIN_SUCCESS !=
	   ps->event_count_table->reader.read_entry_field(ps->event_count_table->table,
	                                                  evtcounter,
	                                                  ps->event_count_table_count_field,
	                                                  &tmp)) {
		ps->lasterr = "can't read event counter in table";
		ps->event_count_table->reader_ext->release_table_entry(ps->event_count_table->table,
		                                                       evtcounter);
		return SS_PLUGIN_FAILURE;
	}
	tmp.u64++;
	if(SS_PLUGIN_SUCCESS !=
	   ps->event_count_table->writer.write_entry_field(ps->event_count_table->table,
	                                                   evtcounter,
	                                                   ps->event_count_table_count_field,
	                                                   &tmp)) {
	...
	if(evt_type_is_open(ev->evt->type)) {
		...
		if(SS_PLUGIN_SUCCESS != in->table_reader.read_entry_field(ps->thread_table,
		                                                          thread,
		                                                          ps->thread_opencount_field,
		                                                          &tmp)) {
			auto err = in->get_owner_last_error(in->owner);
			ps->lasterr = err ? err
			                  : ("can't read open counter from thread with tid=" +
		...
		tmp.u64++;
		if(SS_PLUGIN_SUCCESS != in->table_writer.write_entry_field(ps->thread_table,
		                                                           thread,
		                                                           ps->thread_opencount_field,
		                                                           &tmp)) {
			auto err = in->get_owner_last_error(in->owner);
			ps->lasterr = err ? err
			                  : ("can't write open counter to thread with tid=" +
			                     std::to_string(ev->evt->tid));
			in->table_reader_ext->release_table_entry(ps->thread_table, thread);
			return SS_PLUGIN_FAILURE;
		}
