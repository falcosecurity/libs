...
ss_plugin_t* plugin_init(const ss_plugin_init_input* in, ss_plugin_rc* rc) {
	...
	*rc = in->tables->reader_ext->read_entry_field(ret->thread_table,
	...
	ret->table_field_envtable_value =
	        in->tables->fields_ext->get_table_field(envtable,
	                                                "value",
...
ss_plugin_rc plugin_parse_event(ss_plugin_t* s,
	...
	if(ps->step == 0) {
		...
		for(int i = 0; i < max_iterations; i++) {
			...
			data.str = "hello";
			res = in->table_reader_ext->read_entry_field(envtable,
			                                             nentry,
			                                             ps->table_field_envtable_value,
			                                             &data);
			if(res != SS_PLUGIN_SUCCESS) {
			...
			data.str = "hello";
			res = in->table_writer_ext->write_entry_field(envtable,
			                                              nentry,
			                                              ps->table_field_envtable_value,
			                                              &data);
			if(res != SS_PLUGIN_SUCCESS) {
