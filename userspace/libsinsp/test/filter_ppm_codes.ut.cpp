...
struct testdata_sc_set {
	...
	virtual set_t filter_set(const std::string filter) const {
		return libsinsp::filter::ast::ppm_sc_codes(libsinsp::filter::parser(filter).parse().get());
	}
};
...
struct testdata_event_set {
	...
	const set_t openat_set = {PPME_SYSCALL_OPENAT_E,
...
#define ASSERT_FILTER_EQ(t, a, b) \
	{ ASSERT_EQ(t.filter_set(a), t.filter_set(b)); }
#define ASSERT_FILTER_SET_EQ(t, a, b) \
	{ ASSERT_EQ(t.filter_set(a), b); }
...
#define TEST_CODES(test_suite_name, test_name)            \
	...
	TEST(test_suite_name, sc_##test_name) {               \
		test_##test_name<testdata_sc_set>();              \
	};                                                    \
	TEST(test_suite_name, event_##test_name) {            \
		test_##test_name<testdata_event_set>();           \
	};                                                    \
	TEST(test_suite_name, sc_converted_##test_name) {     \
		test_##test_name<testdata_sc_set_converted>();    \
	};                                                    \
	TEST(test_suite_name, event_converted_##test_name) {  \
		test_##test_name<testdata_event_set_converted>(); \
	};                                                    \
	template<typename T>                                  \
	void test_##test_name()
...
TEST_CODES(filter_ppm_codes, check_openat) {
	...
	ASSERT_FILTER_SET_EQ(t, "evt.type=openat and (proc.name=nginx)", openat_only);
	ASSERT_FILTER_SET_EQ(t,
	                     "evt.type=openat and not (evt.type=close and proc.name=nginx)",
	                     openat_only);
...
TEST_CODES(filter_ppm_codes, check_openat_or_close) {
	...
	ASSERT_FILTER_SET_EQ(t,
	                     "evt.type=openat or (evt.type=close and proc.name=nginx)",
	                     openat_close_only);
	ASSERT_FILTER_SET_EQ(t,
	                     "evt.type=close or (evt.type=openat and proc.name=nginx)",
	                     openat_close_only);
...
TEST_CODES(filter_ppm_codes, check_all_events) {
	...
	ASSERT_FILTER_SET_EQ(t, "(evt.type=openat) or proc.name=nginx", all_events);
	ASSERT_FILTER_SET_EQ(t,
	                     "evt.type=close or not (evt.type=openat and proc.name=nginx)",
	                     all_events);
	ASSERT_FILTER_SET_EQ(t,
	                     "evt.type=openat or not (evt.type=close and proc.name=nginx)",
	                     all_events);
}
...
TEST_CODES(filter_ppm_codes, check_properties) {
	...
	ASSERT_FILTER_EQ(t,
	                 "evt.type in (connect, execve, accept, mmap, container) and not "
	                 "(proc.name=cat and evt.type=mmap)",
	...
	std::string neutral1 =
	        "(proc.name=cat)";  ///< "neutral"-sets: evt types are not checked in the filter
