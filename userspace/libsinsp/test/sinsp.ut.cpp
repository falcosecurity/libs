...
TEST_F(sinsp_with_test_input, bind_listen_accept_ipv4)
{
	...
	ASSERT_EQ(get_field_as_string(evt, "fd.name"), "192.168.0.3:40556->192.168.0.2:8080");
	...
	ASSERT_EQ(get_field_as_string(evt, "fd.sport"), "8080");
	ASSERT_EQ(get_field_as_string(evt, "fd.lport"), "8080");
...
TEST_F(sinsp_with_test_input, bind_listen_accept_ipv6)
{
	...
	evt = add_event_advance_ts(increasing_ts(), 1, PPME_SOCKET_BIND_X, 2, 0, scap_const_sized_buffer{sa.data(), sa.size()});
	ASSERT_EQ(get_field_as_string(evt, "fd.name"), ":::8080");
	ASSERT_EQ(get_field_as_string(evt, "fd.is_server"), "true");
	...
	std::string fdname = std::string(remote_client) + ":40556->" + std::string(local_server) + ":8080";
