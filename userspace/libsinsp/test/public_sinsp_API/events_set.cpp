...
TEST(events_set, names_to_event_set) {
	auto event_set = libsinsp::events::names_to_event_set(
	        std::unordered_set<std::string>{"openat", "execveat"});
	libsinsp::events::set<ppm_event_code> event_set_truth = {PPME_SYSCALL_OPENAT_E,
	...
	ASSERT_EQ(event_set.size(), 6);  // enter/exit events for each event name, special case "openat"
	                                 // has 4 PPME instead of 2

	// generic event case
	event_set = libsinsp::events::names_to_event_set(
	        std::unordered_set<std::string>{"openat", "execveat", "syncfs"});
	event_set_truth = {PPME_SYSCALL_OPENAT_E,
	                   PPME_SYSCALL_OPENAT_X,
	                   PPME_SYSCALL_OPENAT_2_E,
	                   PPME_SYSCALL_OPENAT_2_X,
	                   PPME_SYSCALL_EXECVEAT_E,
	                   ...
	                   PPME_GENERIC_X};
	ASSERT_PPM_EVENT_CODES_EQ(event_set_truth, event_set);
	ASSERT_EQ(event_set.size(), 8);  // enter/exit events for each event name, special case "openat"
	                                 // has 4 PPME instead of 2
}
...
TEST(events_set, generic_no_events) {
	auto generic_ev_set_truth =
	        libsinsp::events::set<ppm_event_code>({PPME_GENERIC_E, PPME_GENERIC_X});
...
TEST(events_set, event_set_to_names_generic_events) {
	static libsinsp::events::set<ppm_event_code> generic_event_set = {PPME_GENERIC_E,
	                                                                  PPME_GENERIC_X};
	auto names = libsinsp::events::event_set_to_names(generic_event_set);
	/* Negative assertions. */
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"execve"}).empty());
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"accept"}).empty());
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"mprotect"}).empty());
	ASSERT_TRUE(unordered_set_intersection(names, std::unordered_set<std::string>{"mmap"}).empty());
	ASSERT_TRUE(unordered_set_intersection(names, std::unordered_set<std::string>{"container"})
	                    .empty());
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"procexit"}).empty());
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"umount2"}).empty());
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"eventfd2"}).empty());
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"syscall"}).empty());
	ASSERT_TRUE(unordered_set_intersection(names, std::unordered_set<std::string>{"init_module"})
	                    .empty());
	/* Random checks for some generic sc events. */
	ASSERT_FALSE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"syncfs"}).empty());
	ASSERT_FALSE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"perf_event_open"})
	                .empty());
	ASSERT_FALSE(unordered_set_intersection(names, std::unordered_set<std::string>{"timer_create"})
	                     .empty());
	ASSERT_FALSE(unordered_set_intersection(names, std::unordered_set<std::string>{"lsetxattr"})
	                     .empty());
	ASSERT_FALSE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"getsid"}).empty());
	ASSERT_FALSE(unordered_set_intersection(names, std::unordered_set<std::string>{"sethostname"})
	                     .empty());
	ASSERT_FALSE(unordered_set_intersection(names, std::unordered_set<std::string>{"readlinkat"})
	                     .empty());
...
TEST(events_set, event_set_to_names_no_generic_events1) {
	static std::set<std::string> names_truth =
	        {"kill", "dup", "umount", "eventfd", "procexit", "container"};
	auto names_unordered = libsinsp::events::event_set_to_names(
	        libsinsp::events::set<ppm_event_code>{PPME_SYSCALL_KILL_E,
	                                              PPME_SYSCALL_KILL_X,
	...
	ASSERT_TRUE(
	        unordered_set_intersection(names_unordered, std::unordered_set<std::string>{"syncfs"})
	                .empty());
	ASSERT_TRUE(
	        unordered_set_intersection(names_unordered, std::unordered_set<std::string>{"eventfd2"})
	                .empty());
	ASSERT_FALSE(unordered_set_intersection(names_unordered,
	                                        std::unordered_set<std::string>{"container"})
	                     .empty());
	ASSERT_FALSE(
	        unordered_set_intersection(names_unordered, std::unordered_set<std::string>{"eventfd"})
	                .empty());
}
...
TEST(events_set, event_set_to_names_no_generic_events2) {
	...
	ASSERT_FALSE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"mprotect"}).empty());
	ASSERT_FALSE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"mmap"}).empty());
	ASSERT_FALSE(unordered_set_intersection(names, std::unordered_set<std::string>{"container"})
	                     .empty());
	ASSERT_FALSE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"procexit"}).empty());
	ASSERT_FALSE(unordered_set_intersection(names, std::unordered_set<std::string>{"init_module"})
	                     .empty());
	...
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"perf_event_open"})
	                .empty());
	ASSERT_TRUE(unordered_set_intersection(names, std::unordered_set<std::string>{"timer_create"})
	                    .empty());
	ASSERT_TRUE(unordered_set_intersection(names, std::unordered_set<std::string>{"lsetxattr"})
	                    .empty());
	ASSERT_TRUE(
	        unordered_set_intersection(names, std::unordered_set<std::string>{"getsid"}).empty());
	ASSERT_TRUE(unordered_set_intersection(names, std::unordered_set<std::string>{"sethostname"})
	                    .empty());
	ASSERT_TRUE(unordered_set_intersection(names, std::unordered_set<std::string>{"readlinkat"})
	                    .empty());
}
...
TEST(events_set, all_non_generic_sc_event_set) {
	auto event_set =
	        libsinsp::events::all_event_set()
	                .filter([&](ppm_event_code e) { return libsinsp::events::is_syscall_event(e); })
...
TEST(events_set, all_non_sc_event_set) {
	auto event_set = libsinsp::events::all_event_set().filter(
	        [&](ppm_event_code e) { return !libsinsp::events::is_syscall_event(e); });
