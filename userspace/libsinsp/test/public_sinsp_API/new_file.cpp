...
#define NA_EVENTS_INTO_EVENT_TABLE 12
...
const libsinsp::events::set<ppm_event_code> expected_sinsp_state_event_set = {
	...
	PPME_DROP_E,
	PPME_DROP_X,
	PPME_SCAPEVENT_E, // PPME_SCAPEVENT_X is not a metaevent
...
TEST(new_tmp, check_generic_events)
{
...
TEST(new_tmp, check_skip_parse_reset_events)
{
...
TEST(new_tmp, all_event_names)
{
	...
	/* `NA` events were now removed so we don't want them again, all other syscall names have no events associated so they should be in this set */
	 ...
	 * Events are almost all paired when they are not there dividing by 2 we remove the `NA` entries.
	 * Since we consider the `NA` a valid name we need to add it to the set, so `+1`
	 * `ACCEPT4` uses the same events as ACCEPT so we have 2 times the "accept" name, so `-1`. `-1` and not `-2` because we have already divided by 2.
	 ...
	 */
	ASSERT_EQ(events_names.size(), (PPM_EVENT_MAX / 2) + 1 - 1 - 1 - old_versions_events / 2 + GENERIC_SYSCALLS_NUM);
}

TEST(new_tmp, sinsp_state_event_set)
{
	ASSERT_PPM_EVENT_CODES_EQ(libsinsp::events::sinsp_state_event_set(), expected_sinsp_state_event_set);
}

TEST(new_tmp, all_generic_events_names)
{
	libsinsp::events::set<ppm_event_code> generic_events{PPME_GENERIC_E, PPME_GENERIC_X};
...
TEST(new_tmp, all_generic_ppm_sc)
{
	libsinsp::events::set<ppm_event_code> generic_events{PPME_GENERIC_E, PPME_GENERIC_X};
...
TEST(new_tmp, generic_e_generic_x_comparison)
{
...
/* Information Loss */
TEST(new_tmp, AES_sc_set_AES)
{
	const auto all_events = libsinsp::events::all_event_set();

	/* Here we try to recover all ppm_sc from all the events.
	 * The only information that we lose is `PPM_SC_UNKNOWN` since we have no events associated with this code.
	 */
	auto all_ppm_sc = libsinsp::events::event_set_to_sc_set(all_events);
	all_ppm_sc.insert((ppm_sc_code)PPM_SC_UNKNOWN);
...
TEST(new_tmp, AEN_event_set_AEN)
{
...
/* Information Enrichment */
TEST(new_tmp, SEN_event_set_SEN)
{
...
TEST(new_tmp, NGEN_event_set_NGEN)
{
...
TEST(new_tmp, from_event_names_to_event_names_with_information_loss)
{
...
/// todo(@Andreagit97) remove duplicated
TEST(new_tmp, event_set_to_names_misc)
{
...
TEST(new_tmp, all_sc_set)
{
	const auto all_sc = libsinsp::events::all_sc_set();
	ASSERT_EQ(all_sc.size(), PPM_SC_SYSCALL_END + PPM_SC_TP_LEN);
	for(int i = 0; i < PPM_SC_SYSCALL_END; i++)
	{
		ASSERT_TRUE(all_sc.contains((ppm_sc_code)i)) << "\n- The syscall '" << scap_get_syscall_info_table()[i].name << "' is not present inside the sc set" << std::endl;
	}
	/* Not sure we want to expose also this one, probably yes since with events we expose also unused ones */
	ASSERT_TRUE(all_sc.contains(PPM_SC_UNKNOWN));

	for(int i = PPM_SC_TP_START; i < PPM_SC_MAX; i++)
	{
		/// todo(@Andreagit97) we need to change the name to the syscall info table, we don't have only syscalls
		ASSERT_TRUE(all_sc.contains((ppm_sc_code)i)) << "\n- The tracepoint '" << scap_get_syscall_info_table()[i].name << "' is not present inside the sc set" << std::endl;
	}
}

TEST(new_tmp, all_sc_names)
{
	auto ppm_sc_names = test_utils::unordered_set_to_ordered(libsinsp::events::sc_set_to_names(libsinsp::events::all_sc_set()));
	ASSERT_EQ(ppm_sc_names.size(), PPM_SC_SYSCALL_END + PPM_SC_TP_LEN);
	std::set<std::string> expected_ppm_sc_names;
	for(int i = 0; i < PPM_SC_SYSCALL_END; i++)
	{
		expected_ppm_sc_names.insert(scap_get_syscall_info_table()[i].name);
	}
	...
	ASSERT_NAMES_EQ(ppm_sc_names, expected_ppm_sc_names);
}

TEST(new_tmp, sinsp_state_sc_set)
{
	ASSERT_PPM_SC_CODES_EQ(expected_sinsp_state_sc_set, libsinsp::events::sinsp_state_sc_set());
}

TEST(new_tmp, enforce_sinsp_state_sc_set)
{
	...
	// todo(@Andreagit97) enable after rebase when insert() is fixed
	// ASSERT_CONTAINS(expected_final_state_set, libsinsp::events::sinsp_state_sc_set());
}

TEST(new_tmp, sc_empty_sets)
{
	std::unordered_set<std::string> empty_string_set;
...
TEST(new_tmp, ASS_event_set_ASS)
{
	...
	/* Convert again from events to sc.
	 * In the conversion we lose `PPM_SC_UNKNOWN` since it is not associated with any event...
	 */
	auto all_sc_again = libsinsp::events::event_set_to_sc_set(all_events);
	all_sc_again.insert(PPM_SC_UNKNOWN);
	ASSERT_PPM_SC_CODES_EQ(all_sc_again, all_sc);
}

/* Information Loss */
TEST(new_tmp, SSS_event_set_SSS)
{
...
/* Information Loss */
TEST(new_tmp, NGSS_event_set_NGSS)
{
