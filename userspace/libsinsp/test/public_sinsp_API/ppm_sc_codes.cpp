...
const libsinsp::events::set<ppm_sc_code> expected_sinsp_state_sc_set = {
        ...
        PPM_SC_BIND,           PPM_SC_CAPSET,
        PPM_SC_CHDIR,          PPM_SC_CHROOT,
        PPM_SC_CLONE,          PPM_SC_CLONE3,
        PPM_SC_CLOSE,          PPM_SC_CONNECT,
        PPM_SC_CREAT,          PPM_SC_DUP,
        PPM_SC_DUP2,           PPM_SC_DUP3,
        PPM_SC_EVENTFD,        PPM_SC_EVENTFD2,
        PPM_SC_EXECVE,         PPM_SC_EXECVEAT,
        PPM_SC_FCHDIR,         PPM_SC_FCNTL,
        PPM_SC_FCNTL64,        PPM_SC_FORK,
        PPM_SC_INOTIFY_INIT,   PPM_SC_INOTIFY_INIT1,
        PPM_SC_IO_URING_SETUP, PPM_SC_MOUNT,
        PPM_SC_OPEN,           PPM_SC_OPEN_BY_HANDLE_AT,
        PPM_SC_OPENAT,         PPM_SC_OPENAT2,
        PPM_SC_PIPE,           PPM_SC_PIPE2,
        PPM_SC_PRLIMIT64,      PPM_SC_RECVFROM,
        PPM_SC_RECVMSG,        PPM_SC_GETSOCKOPT,
        PPM_SC_SENDMSG,        PPM_SC_SENDTO,
        PPM_SC_SETGID,         PPM_SC_SETGID32,
        PPM_SC_SETPGID,        PPM_SC_SETRESGID,
        PPM_SC_SETRESGID32,    PPM_SC_SETRESUID,
        PPM_SC_SETRESUID32,    PPM_SC_SETRLIMIT,
        PPM_SC_SETSID,         PPM_SC_SETUID,
        PPM_SC_SETUID32,       PPM_SC_SHUTDOWN,
        PPM_SC_SIGNALFD,       PPM_SC_SIGNALFD4,
        PPM_SC_SOCKET,         PPM_SC_SOCKETPAIR,
        PPM_SC_TIMERFD_CREATE, PPM_SC_UMOUNT,
        PPM_SC_UMOUNT2,        PPM_SC_USERFAULTFD,
        PPM_SC_VFORK,          PPM_SC_EPOLL_CREATE,
        PPM_SC_EPOLL_CREATE1,  PPM_SC_SCHED_PROCESS_EXIT,
        PPM_SC_PRCTL,          PPM_SC_MEMFD_CREATE,
        PPM_SC_PIDFD_OPEN,     PPM_SC_PIDFD_GETFD,
        PPM_SC_SETREUID,       PPM_SC_SETREGID,
};

const libsinsp::events::set<ppm_event_code> expected_unknown_event_set = {
        PPME_PROCEXIT_X,      PPME_SCHEDSWITCH_1_X,
        PPME_SCHEDSWITCH_6_X, PPME_PROCEXIT_1_X,
        PPME_PLUGINEVENT_X,   PPME_USER_ADDED_X,
        PPME_USER_DELETED_X,  PPME_GROUP_ADDED_X,
        PPME_GROUP_DELETED_X, PPME_CONTAINER_JSON_2_X,
...
TEST(ppm_sc_API, generic_syscalls_set) {
	...
	ASSERT_EQ(
	        scap_get_ppm_sc_from_events(generic_enter_event.data(), generic_syscalls_enter.data()),
	        SCAP_SUCCESS);
	ASSERT_EQ(scap_get_ppm_sc_from_events(generic_exit_event.data(), generic_syscalls_exit.data()),
	          SCAP_SUCCESS);
...
TEST(ppm_sc_API, all_event_names) {
	 ...
	 * names, but we don't want the "syscall" event name associated with `GENERIC_E`/`GENERIC_X`
	 * events extracted from the event table.
	 */
	...
	std::set<std::string> some_not_desired_names{
	        ...
	        "getgid32",    "getegid32",  "getresuid32", "getresgid32", "NA1",
	        "NA2",         "NA3",        "NA4",         "NA5",         "NA6"};
	...
	ASSERT_EQ(scap_get_ppm_sc_from_events(generic_events.data(), generic_syscalls.data()),
	          SCAP_SUCCESS);
...
TEST(ppm_sc_API, sinsp_state_event_set) {
	ASSERT_PPM_EVENT_CODES_EQ(libsinsp::events::sinsp_state_event_set(),
	                          expected_sinsp_state_event_set);
}

TEST(ppm_sc_API, all_generic_events_names) {
	libsinsp::events::set<ppm_event_code> generic_events{PPME_GENERIC_E, PPME_GENERIC_X};
	std::set<std::string> generic_events_names = test_utils::unordered_set_to_ordered(
	        libsinsp::events::event_set_to_names(generic_events));
...
TEST(ppm_sc_API, SES_names_SES) {
	...
	std::set<std::string> some_desired_event_names{"alarm",
	                                               "clone",
	                                               "container",
	                                               "pluginevent",
	                                               "close",
	                                               "scapevent",
	                                               "NA"};  // PPME_PROCEXIT_1_X is UNKNOWN
	ASSERT_CONTAINS(shared_events_names, some_desired_event_names);
	/* size = all generic names + 6 names written above (alarm is generic one so already included in
	 * the generic names) */
	...
	ASSERT_PPM_EVENT_CODES_EQ(expected_shared_events,
	                          libsinsp::events::names_to_event_set(shared_events_names));
}
...
TEST(ppm_sc_API, SES_sc_set_SES) {
	...
	/* Convert again to codes, here we recover enter/exit and old versions but we cannot recover not
	 * syscall/tracepoints events */
	libsinsp::events::set<ppm_event_code> expected_shared_events{
	        PPME_GENERIC_E,
	        PPME_GENERIC_X,
...
TEST(ppm_sc_API, NGES_names_NGES) {
	...
	ASSERT_NAMES_EQ(test_utils::unordered_set_to_ordered(not_generic_events_names),
	                some_desired_event_names);
	/* Convert again to codes, here we recover also enter/exit and old version */
	libsinsp::events::set<ppm_event_code> expected_not_generic_events{PPME_SYSCALL_CLONE_11_E,
	                                                                  PPME_SYSCALL_CLONE_11_X,
	                                                                  PPME_SYSCALL_CLONE_16_E,
	                                                                  PPME_SYSCALL_CLONE_16_X,
	                                                                  PPME_SYSCALL_CLONE_17_E,
	...
	ASSERT_PPM_EVENT_CODES_EQ(expected_not_generic_events,
	                          libsinsp::events::names_to_event_set(not_generic_events_names));
}
...
TEST(ppm_sc_API, AEN_event_set_AEN) {
	const auto all_events_names =
	        libsinsp::events::event_set_to_names(libsinsp::events::all_event_set());
	const auto all_events = libsinsp::events::names_to_event_set(all_events_names);
	ASSERT_EQ(all_events.size(), PPM_EVENT_MAX);
	const auto all_events_names_again =
	        test_utils::unordered_set_to_ordered(libsinsp::events::event_set_to_names(all_events));
...
TEST(ppm_sc_API, SEN_event_set_SEN) {
	...
	std::unordered_set<std::string> shared_events_names{
	        "syncfs",
	        "clone",
	        "switch",
	        "not-exists",
	        "Read"};  // Note the capital letter in 'Read'
	const auto shared_events = libsinsp::events::names_to_event_set(shared_events_names);
	libsinsp::events::set<ppm_event_code> expected_shared_events{PPME_GENERIC_E,
	                                                             PPME_GENERIC_X,
	                                                             PPME_SYSCALL_CLONE_11_E,
	                                                             PPME_SYSCALL_CLONE_11_X,
	                                                             PPME_SYSCALL_CLONE_16_E,
	...
	auto shared_events_names_again = test_utils::unordered_set_to_ordered(
	        libsinsp::events::event_set_to_names(shared_events));
...
TEST(ppm_sc_API, NGEN_event_set_NGEN) {
	...
	libsinsp::events::set<ppm_event_code> expected_not_generic_events{PPME_SYSCALL_FCNTL_E,
	                                                                  PPME_SYSCALL_FCNTL_X,
	...
	ASSERT_NAMES_EQ(test_utils::unordered_set_to_ordered(not_generic_events_names),
	                not_generic_events_names_again);
}
...
TEST(ppm_sc_API, from_event_names_to_event_names_with_information_loss) {
	...
	auto event_names_with_all_generics =
	        test_utils::unordered_set_to_ordered(libsinsp::events::event_set_to_names(event_codes));

	/* Expected set */
	auto expected_events_names = test_utils::unordered_set_to_ordered(
	        libsinsp::events::event_set_to_names({PPME_GENERIC_E, PPME_GENERIC_X}));
 ...
 * - (ASS) -> names -> (ASS)         ASS = all sc set (this test is not so meaningful the mapping is
 * 1:1)
 * - (ASS) -> event set -> (ASS)
 * - (SSS) -> event set -> (SSS)     SSS = shared sc set (some syscall associated with generic
 * events + not generic syscalls)
...
TEST(ppm_sc_API, all_sc_set) {
	...
	/* In all_sc we don't have `PPM_SC_UNKNOWN` and the code `382` that corresponds to old/wrong
	 * code */
	ASSERT_EQ(all_sc.size(), PPM_SC_MAX - 2);
}

TEST(ppm_sc_API, all_sc_names) {
	auto sc_names = test_utils::unordered_set_to_ordered(
	        libsinsp::events::sc_set_to_sc_names(libsinsp::events::all_sc_set()));
	std::set<std::string> expected_sc_names;
	/* In all_sc we don't have `PPM_SC_UNKNOWN` so we don't have to retrieve the "unknown" name, we
...
TEST(ppm_sc_API, enforce_sinsp_state_sc_set) {
	...
	ASSERT_PPM_SC_CODES_EQ(expected_final_state_set,
	                       libsinsp::events::enforce_simple_sc_set({PPM_SC_UNKNOWN,
	                                                                PPM_SC__NEWSELECT,
	                                                                PPM_SC_PAGE_FAULT_KERNEL,
	                                                                PPM_SC_SCHED_SWITCH}));
...
TEST(ppm_sc_API, sc_unknown) {
	...
	ASSERT_NAMES_EQ(libsinsp::events::sc_set_to_sc_names(unknown_sc),
...
TEST(ppm_sc_API, SSS_event_set_SSS) {
	const libsinsp::events::set<ppm_sc_code> shared_sc_set{PPM_SC_UNKNOWN,
	                                                       PPM_SC_SYSLOG,
	                                                       PPM_SC_ACCEPT4,
	                                                       PPM_SC_PAGE_FAULT_KERNEL};
	const auto shared_event_set = libsinsp::events::sc_set_to_event_set(shared_sc_set);
	const libsinsp::events::set<ppm_event_code> expected_shared_event_set{PPME_GENERIC_E,
	                                                                      PPME_GENERIC_X,
	                                                                      PPME_SOCKET_ACCEPT4_6_E,
	                                                                      PPME_SOCKET_ACCEPT4_6_X,
	                                                                      PPME_SOCKET_ACCEPT4_5_E,
	...
	/* +3 because we have to add `PPM_SC_ACCEPT4` `PPM_SC_PAGE_FAULT_KERNEL`
	 * `PPM_SC_PAGE_FAULT_USER` */
	ASSERT_EQ(shared_sc_set_again.size(), GENERIC_SYSCALLS_NUM + 3);
}

/* Information Loss */
TEST(ppm_sc_API, NGSS_event_set_NGSS) {
	const libsinsp::events::set<ppm_sc_code> not_generic_sc_set{PPM_SC_UNKNOWN,
	                                                            PPM_SC_PIPE2,
	                                                            PPM_SC_EVENTFD2,
	                                                            PPM_SC_BRK};
	...
	/* We lose also the `PPM_SC_UNKNOWN` */
	const auto not_generic_sc_set_again =
	        libsinsp::events::event_set_to_sc_set(not_generic_event_set);
...
TEST(ppm_sc_API, SSN_sc_set_SSN) {
	auto sc_set = libsinsp::events::sc_names_to_sc_set(
...
/// todo(@Andreagit97) Here we miss all tests on `io_sc_set` and others... Not sure we want all
/// those helpers, if yes we need to create sets here in tests so we can assert against them
