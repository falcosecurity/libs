	...
	*len = 0;
	...
	case TYPE_SNAME: {
		...
		if(!tinfo->is_in_pid_namespace()) {
			...
			// `threadinfo` lookup only applies when the process is running on the host and not in a
			// pid namespace. However, if the process is running in a pid namespace, we instead
	...
	case TYPE_VPGID_NAME: {
		...
		if(!tinfo->is_in_pid_namespace()) {
			// Relying on the convention that a process group id is the process id of the process
			// group leader. `threadinfo` lookup only applies when the process is running on the
			// host and not in a pid namespace. However, if the process is running in a pid
			// namespace, we instead traverse the process lineage until we find a match.
