	   ...
	   proc.aexepath */
		...
		if(alloc_state) {
			...
			m_thread_dyn_field_accessor =
			        std::make_unique<libsinsp::state::dynamic_field_accessor<uint64_t>>(
...
uint8_t* sinsp_filter_check_thread::extract_single(sinsp_evt* evt,
	...
	*len = 0;
	...
	case TYPE_PVPID: {
		auto ptinfo_ptr = m_inspector->m_thread_manager->find_thread(tinfo->m_ptid, true);

		if(ptinfo_ptr != NULL) {
			RETURN_EXTRACT_VAR(ptinfo_ptr->m_vpid);
		} else {
			return NULL;
		}
	}
	...
	case TYPE_PPID_CLONE_TS: {
		...
		if(ptinfo_ptr != NULL) {
			RETURN_EXTRACT_VAR(ptinfo_ptr->m_clone_ts);
		} else {
			return NULL;
		}
	}
