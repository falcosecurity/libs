...
static inline void check_storage_size(const char* str,
                                      std::string::size_type storage_len,
                                      std::string::size_type used_len) {
	if(used_len > storage_len) {
		throw sinsp_exception(+"filter parameter too long (used=" + std::to_string(used_len) +
		                      ", available=" + std::to_string(storage_len) +
		                      "):" + std::string(str));
	}
}

size_t sinsp_filter_value_parser::string_to_rawval(const char* str,
                                                   uint32_t len,
                                                   uint8_t* storage,
