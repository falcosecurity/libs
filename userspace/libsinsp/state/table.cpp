...
void libsinsp::state::table_accessor::set(sinsp_table_owner* p, libsinsp::state::base_table* t) {
	...
	auto key_type = t->key_type();
	if(key_type == SS_PLUGIN_ST_TABLE) {
...
static ss_plugin_table_field_t* cast(const libsinsp::state::accessor* ptr) {
...
ss_plugin_table_field_t* libsinsp::state::built_in_table<KeyType>::get_field(
	...
	__CATCH_ERR_MSG(owner->m_last_owner_err, {
		...
		const auto& acc = this->get_field(name, data_type);
		this->m_field_accessors[name] = &acc;
		return cast(&acc);
	});
...
template<typename KeyType>
const libsinsp::state::accessor& libsinsp::state::extensible_table<KeyType>::get_field(
        const char* name,
...
template<typename KeyType>
const libsinsp::state::accessor& libsinsp::state::extensible_table<KeyType>::add_field(
        const char* name,
        ss_plugin_state_type type_id) {
