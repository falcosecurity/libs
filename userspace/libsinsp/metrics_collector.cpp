...
libs_resource_utilization::libs_resource_utilization(sinsp* inspector, double start_time)
{
	auto metrics_vtable = inspector->get_scap_platform()->m_metrics_vtable;

	if(metrics_vtable)
	{
		metrics_vtable->get_rss_vsz_pss_total_memory_and_open_fds(&m_rss, &m_vsz, &m_pss, &m_host_memory_used, &m_host_open_fds);
		metrics_vtable->get_cpu_usage_and_total_procs(start_time, &m_cpu_usage_perc, &m_host_cpu_usage_perc, &m_host_procs_running);
		metrics_vtable->get_container_memory_used(&m_container_memory_used);
	}
}
...
void libs_metrics_collector::snapshot()
{
	...
	if((m_metrics_flags & METRICS_V2_RESOURCE_UTILIZATION))
	{
		...
		libs_resource_utilization resource_utilization(m_inspector, agent_info->start_time);
