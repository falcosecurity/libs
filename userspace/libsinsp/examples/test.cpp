...
void parse_CLI_options(sinsp& inspector, int argc, char** argv) {
	...
	options.add_options()("h,help", "Print this page.")(
	        "f,filter",
	        "Filter string for events (see https://falco.org/docs/rules/supported-fields/ for "
	        "supported fields).",
	        cxxopts::value<std::string>())("j,json", "Use JSON as the output format.")(
	        "a,all-threads",
	        "Output information about all threads, not just the main one.")(
	        "b,bpf",
	        "Classic eBPF probe.",
	        cxxopts::value<std::string>())("m,modern_bpf", "Modern eBPF probe.")("k,kmod",
	                                                                             "Kernel module.")(
	        "G,gvisor",
	        "Gvisor engine.",
	        ...
	        "p,plugin",
	        "Plugin. Path can follow the pattern \"filepath.so|init_cfg|open_params\". Must come "
	        "after the \"-s\" option when reading events from a file.",
	        cxxopts::value<std::string>())("d,buffer_dim",
	                                       "Dimension in bytes that every buffer will have.",
	                                       cxxopts::value<unsigned long>())(
	        "c,cpus-for-each-buffer",
	        "(modern eBPF probe only) Allocate a ring buffer every <num> CPU(s) (default: 1).",
	        cxxopts::value<uint16_t>())(
	        "A,all-cpus",
	        "(modern eBPF probe only) Allocate ring buffers for all available CPUs (default: "
	        "allocate ring buffers for online CPU(s) only).")(
	        "o,output-fields",
	        "Output fields string (see <filter> for supported display fields) that overwrites "
	        "default output fields for all events. * at the beginning prints JSON keys with null "
	        "values, else no null fields are printed.",
	        cxxopts::value<std::string>())("E,exclude-users",
	                                       "Don't create the user/group tables.")(
	        "n,num-events",
	        "Number of events to be retrieved (no limit by default).",
	        cxxopts::value<uint64_t>())(
	        "z,ppm-sc-modifies-state",
	        "Select ppm sc codes from filter AST plus enforce sinsp state ppm sc codes via "
	        "`sinsp_state_sc_set`, requires valid filter expression.")(
	        "x,ppm-sc-repair-state",
	        "Select ppm sc codes from filter AST plus enforce sinsp state ppm sc codes via "
	        "`sinsp_repair_state_sc_set`, requires valid filter expression.")(
	        "q,remove-io-sc-state",
	        "Remove ppm sc codes belonging to `io_sc_set` from `sinsp_state_sc_set` sinsp state "
	        "enforcement, defaults to false and only applies when choosing `-z` option, used for "
	        "e2e testing of sinsp state.")(
	        "g,enable-glogger",
	        "Enable libs g_logger, set to SEV_DEBUG. For a different severity adjust the test "
	        "binary source and re-compile.")("r,raw", "Raw event output.")(
	        "t,perftest",
	        "Run in performance test mode.")(
	        "T,tables",
	        "-T or -Tbrief print tables descriptions. -Tlist print table entries, if -n is "
	        "specified, print only the first n entries.",
