...
int32_t sinsp_filter_check_reference::parse_field_name(std::string_view,
                                                       bool alloc_state,
...
uint8_t* sinsp_filter_check_reference::extract_single(sinsp_evt* evt, uint32_t* len) {
	*len = m_len;
	return m_val;
}
...
char* sinsp_filter_check_reference::format_time(uint64_t val, uint32_t str_len) {
	if(val >= 3600 * ONE_SECOND_IN_NS) {
		...
		snprintf(m_getpropertystr_storage.data(),
		         STRPROPERTY_STORAGE_SIZE,
		         "%.2u:%.2u:%.2u",
		         (unsigned int)(val / (3600 * ONE_SECOND_IN_NS)),
		...
		snprintf(m_getpropertystr_storage.data(),
		         STRPROPERTY_STORAGE_SIZE,
		         "%u:%u",
		         (unsigned int)(val / (60 * ONE_SECOND_IN_NS)),
		         (unsigned int)((val / ONE_SECOND_IN_NS) % 60));
		...
		snprintf(m_getpropertystr_storage.data(),
		         STRPROPERTY_STORAGE_SIZE,
		         "%u.%02us",
		         (unsigned int)(val / ONE_SECOND_IN_NS),
		         (unsigned int)((val % ONE_SECOND_IN_NS) / 10000000));
	} else if(val >= ONE_SECOND_IN_NS / 100) {
		m_getpropertystr_storage.resize(STRPROPERTY_STORAGE_SIZE);
		snprintf(m_getpropertystr_storage.data(),
		         STRPROPERTY_STORAGE_SIZE,
		         "%ums",
		         (unsigned int)(val / (ONE_SECOND_IN_NS / 1000)));
		...
		snprintf(m_getpropertystr_storage.data(),
		         ...
		         "%u.%02ums",
		         (unsigned int)(val / (ONE_SECOND_IN_NS / 1000)),
		         (unsigned int)((val % ONE_MILLISECOND_IN_NS) / 10000));
	} else if(val >= ONE_SECOND_IN_NS / 100000) {
		m_getpropertystr_storage.resize(STRPROPERTY_STORAGE_SIZE);
		snprintf(m_getpropertystr_storage.data(),
		         STRPROPERTY_STORAGE_SIZE,
		         "%uus",
		         (unsigned int)(val / (ONE_SECOND_IN_NS / 1000000)));
		...
		snprintf(m_getpropertystr_storage.data(),
		         ...
		         "%u.%02uus",
		         (unsigned int)(val / (ONE_SECOND_IN_NS / 1000000)),
		         (unsigned int)((val % ONE_MICROSECOND_IN_NS) / 10));
	} else {
		m_getpropertystr_storage.resize(STRPROPERTY_STORAGE_SIZE);
		snprintf(m_getpropertystr_storage.data(),
		         STRPROPERTY_STORAGE_SIZE,
...
char* sinsp_filter_check_reference::print_double(uint8_t* rawval, uint32_t str_len) {
	...
	if(m_print_format == PF_ID) {
		m_getpropertystr_storage.resize(STRPROPERTY_STORAGE_SIZE);
		snprintf(m_getpropertystr_storage.data(), STRPROPERTY_STORAGE_SIZE, "%*lf", str_len, val);
		return m_getpropertystr_storage.data();
	} else {
