...
std::unordered_set<std::string> libsinsp::events::event_set_to_names(
        const libsinsp::events::set<ppm_event_code>& events_set,
	...
	for(const auto& ev : events_set) {
		if(libsinsp::events::is_generic(ev)) {
			if(resolve_generic && !resolved_generic) {
				/* note: using existing ppm sc APIs and generic set operations to minimize new logic
				 * that requires maintenance beyond what we already have. */
				auto sc_set = libsinsp::events::event_set_to_sc_set(
				        libsinsp::events::set<ppm_event_code>{PPME_GENERIC_E, PPME_GENERIC_X});
				events_names_set = unordered_set_union(libsinsp::events::sc_set_to_sc_names(sc_set),
				                                       events_names_set);
...
libsinsp::events::set<ppm_sc_code> libsinsp::events::event_set_to_sc_set(
        const set<ppm_event_code>& events_of_interest) {
