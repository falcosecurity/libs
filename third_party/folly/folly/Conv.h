...
public:
	ConversionError(const std::string& str, ConversionCode code):
	        ConversionErrorBase(str),
	        code_(code) {}

	ConversionError(const char* str, ConversionCode code): ConversionErrorBase(str), code_(code) {}
...
typename std::enable_if<std::is_same<Tgt, typename std::decay<Src>::type>::value,
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<std::is_same<Tgt, typename std::decay<Src>::type>::value, Tgt>::type to(
        Src&& value) {
	return static_cast<Src&&>(value);
}
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<is_arithmetic_v<Src> && !std::is_same<Tgt, Src>::value &&
...
inline size_t unsafeTelescope128(char* outb, char* oute, unsigned __int128 x) {
	...
	const auto leading = [&](Usrc v) {
		assert(v >> 64 == 0);
		p = detail::to_ascii_with_route<10, to_ascii_alphabet_lower>(outb,
		                                                             oute,
	...
	const auto append = [&](uint64_t v) {
		...
		detail::to_ascii_with_route<10, to_ascii_alphabet_lower>(outb + p, kBaseDigits, v64);
		p += kBaseDigits;
	};
 ...
 */
template<class T>
constexpr typename std::enable_if<std::is_same<T, char>::value, size_t>::type estimateSpaceNeeded(
        T) {
	return 1;
}
...
template<class Src>
typename std::enable_if<std::is_convertible<Src, const char*>::value, size_t>::type
...
template<class Src>
typename std::enable_if<IsSomeString<Src>::value, size_t>::type estimateSpaceNeeded(
        Src const& value) {
	return value.size();
}
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<IsSomeString<Src>::value && IsSomeString<Tgt>::value>::type toAppend(
        const Src& value,
        Tgt* result) {
	result->append(value);
}
 ...
 */
template<class Tgt>
typename std::enable_if<IsSomeString<Tgt>::value>::type toAppend(StringPiece value, Tgt* result) {
 ...
 */
template<class Tgt>
typename std::enable_if<IsSomeString<Tgt>::value>::type toAppend(const fbstring& value,
                                                                 Tgt* result) {
	result->append(value.data(), value.size());
}
...
template<class T>
constexpr typename std::enable_if<std::is_same<T, __int128>::value, size_t>::type
estimateSpaceNeeded(T) {
	return detail::digitsEnough<__int128>();
}

template<class T>
constexpr typename std::enable_if<std::is_same<T, unsigned __int128>::value, size_t>::type
estimateSpaceNeeded(T) {
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<is_integral_v<Src> && is_signed_v<Src> && IsSomeString<Tgt>::value &&
                        sizeof(Src) >= 4>::type
toAppend(Src value, Tgt* result) {
...
template<class Src>
typename std::enable_if<is_integral_v<Src> && is_signed_v<Src> && sizeof(Src) >= 4 &&
                                sizeof(Src) < 16,
                        size_t>::type
estimateSpaceNeeded(Src value) {
	auto uvalue = value < 0 ? ~static_cast<uint64_t>(value) + 1 : static_cast<uint64_t>(value);
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<is_integral_v<Src> && IsSomeString<Tgt>::value && sizeof(Src) < 4>::type
toAppend(Src value, Tgt* result) {
	using Intermediate = typename std::conditional<is_signed_v<Src>, int64_t, uint64_t>::type;
	toAppend<Tgt>(static_cast<Intermediate>(value), result);
}

template<class Src>
typename std::enable_if<is_integral_v<Src> && sizeof(Src) < 4 && !std::is_same<Src, char>::value,
                        size_t>::type
estimateSpaceNeeded(Src value) {
	using Intermediate = typename std::conditional<is_signed_v<Src>, int64_t, uint64_t>::type;
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<std::is_enum<Src>::value && IsSomeString<Tgt>::value>::type toAppend(
        Src value,
        Tgt* result) {
...
template<class Src>
typename std::enable_if<std::is_enum<Src>::value, size_t>::type estimateSpaceNeeded(Src value) {
...
constexpr double kConvMaxDecimalInShortestHighValue = 1'000'000'000'000'000'000'000.0;
...
constexpr double_conversion::DoubleToStringConverter::DtoaMode convert(DtoaMode mode) {
	switch(mode) {
...
constexpr double_conversion::DoubleToStringConverter::Flags convert(DtoaFlags flags) {
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<std::is_floating_point<Src>::value && IsSomeString<Tgt>::value>::type
toAppend(Src value,
         Tgt* result,
         DtoaMode mode,
         unsigned int numDigits,
         DtoaFlags flags = DtoaFlags::NO_FLAGS) {
	double_conversion::DoubleToStringConverter::Flags dcFlags = detail::convert(flags);
	double_conversion::DoubleToStringConverter conv(dcFlags,
	                                                "Infinity",
	                                                "NaN",
	                                                'E',
	...
	double_conversion::DoubleToStringConverter::DtoaMode dcMode = detail::convert(mode);
	FOLLY_PUSH_WARNING
 ...
 */
template<class Src>
typename std::enable_if<std::is_floating_point<Src>::value, size_t>::type estimateSpaceNeeded(
        Src value) {
	...
	return size_t(kMaxPositiveSpace + (value < 0 ? 1 : 0));  // +1 for minus sign, if negative
}
...
#endif
                !IsSomeString<Src>::value && !std::is_convertible<Src, const char*>::value &&
                !std::is_convertible<Src, StringPiece>::value && !std::is_enum<Src>::value,
        size_t>::type
...
void reserveInTarget(const T0& v0, const T1& v1, const Ts&... vs) {
	...
	getLastElement(vs...)->reserve(EstimateSpaceToReserveAll<seq>::call(v0, v1, vs...));
}
...
void reserveInTargetDelim(const Delimiter& d, const Ts&... vs) {
	...
	getLastElement(vs...)->reserve(fordelim + EstimateSpaceToReserveAll<seq>::call(vs...));
}
...
template<class Delimiter,
         class T,
         class... Ts,
         std::enable_if_t<sizeof...(Ts) >= 2 && IsSomeString<typename std::remove_pointer<
 ...
 */
template<class... Ts,
         std::enable_if_t<sizeof...(Ts) >= 3 && IsSomeString<typename std::remove_pointer<
                                                        detail::LastElement<Ts...>>::type>::value,
                          int> = 0>
...
template<
        class... Ts,
        std::enable_if_t<
                IsSomeString<typename std::remove_pointer<detail::LastElement<Ts...>>::type>::value,
                int> = 0>
 ...
 */
template<class Tgt>
typename std::enable_if<IsSomeString<Tgt>::value>::type toAppend(Tgt* /* result */) {}
...
typename std::enable_if<IsSomeString<Tgt>::value>::type toAppendDelim(const Delimiter& /* delim */,
                                                                      Tgt* /* result */) {}
...
template<class Tgt,
         class... Ts,
         std::enable_if_t<IsSomeString<Tgt>::value &&
                                  (sizeof...(Ts) != 1 ||
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<IsSomeString<Tgt>::value && std::is_floating_point<Src>::value, Tgt>::type
to(Src value) {
	Tgt result;
...
extern template Expected<float, ConversionCode> str_to_floating<float>(StringPiece* src) noexcept;
extern template Expected<double, ConversionCode> str_to_floating<double>(StringPiece* src) noexcept;

template<typename T>
Expected<T, ConversionCode> str_to_floating_fast_float_from_chars(StringPiece* src) noexcept;

extern template Expected<float, ConversionCode> str_to_floating_fast_float_from_chars<float>(
        StringPiece* src) noexcept;
extern template Expected<double, ConversionCode> str_to_floating_fast_float_from_chars<double>(
        StringPiece* src) noexcept;

template<class Tgt>
Expected<Tgt, ConversionCode> digits_to(const char* b, const char* e) noexcept;

extern template Expected<char, ConversionCode> digits_to<char>(const char*, const char*) noexcept;
extern template Expected<signed char, ConversionCode> digits_to<signed char>(const char*,
                                                                             const char*) noexcept;
extern template Expected<unsigned char, ConversionCode> digits_to<unsigned char>(
        const char*,
        const char*) noexcept;

extern template Expected<short, ConversionCode> digits_to<short>(const char*, const char*) noexcept;
extern template Expected<unsigned short, ConversionCode> digits_to<unsigned short>(
        const char*,
        const char*) noexcept;

extern template Expected<int, ConversionCode> digits_to<int>(const char*, const char*) noexcept;
extern template Expected<unsigned int, ConversionCode> digits_to<unsigned int>(
        const char*,
        const char*) noexcept;

extern template Expected<long, ConversionCode> digits_to<long>(const char*, const char*) noexcept;
extern template Expected<unsigned long, ConversionCode> digits_to<unsigned long>(
        const char*,
        const char*) noexcept;

extern template Expected<long long, ConversionCode> digits_to<long long>(const char*,
                                                                         const char*) noexcept;
extern template Expected<unsigned long long, ConversionCode> digits_to<unsigned long long>(
        const char*,
        const char*) noexcept;

#if FOLLY_HAVE_INT128_T
extern template Expected<__int128, ConversionCode> digits_to<__int128>(const char*,
                                                                       const char*) noexcept;
extern template Expected<unsigned __int128, ConversionCode> digits_to<unsigned __int128>(
        const char*,
        const char*) noexcept;
#endif
...
extern template Expected<char, ConversionCode> str_to_integral<char>(StringPiece* src) noexcept;
extern template Expected<signed char, ConversionCode> str_to_integral<signed char>(
        StringPiece* src) noexcept;
extern template Expected<unsigned char, ConversionCode> str_to_integral<unsigned char>(
        StringPiece* src) noexcept;

extern template Expected<short, ConversionCode> str_to_integral<short>(StringPiece* src) noexcept;
extern template Expected<unsigned short, ConversionCode> str_to_integral<unsigned short>(
        StringPiece* src) noexcept;

extern template Expected<int, ConversionCode> str_to_integral<int>(StringPiece* src) noexcept;
extern template Expected<unsigned int, ConversionCode> str_to_integral<unsigned int>(
        StringPiece* src) noexcept;

extern template Expected<long, ConversionCode> str_to_integral<long>(StringPiece* src) noexcept;
extern template Expected<unsigned long, ConversionCode> str_to_integral<unsigned long>(
        StringPiece* src) noexcept;

extern template Expected<long long, ConversionCode> str_to_integral<long long>(
        StringPiece* src) noexcept;
extern template Expected<unsigned long long, ConversionCode> str_to_integral<unsigned long long>(
        StringPiece* src) noexcept;

#if FOLLY_HAVE_INT128_T
extern template Expected<__int128, ConversionCode> str_to_integral<__int128>(
        StringPiece* src) noexcept;
extern template Expected<unsigned __int128, ConversionCode> str_to_integral<unsigned __int128>(
        StringPiece* src) noexcept;
#endif

template<typename T>
typename std::enable_if<std::is_same<T, bool>::value, Expected<T, ConversionCode>>::type convertTo(
        StringPiece* src) noexcept {
	return str_to_bool(src);
}

template<typename T>
typename std::enable_if<std::is_floating_point<T>::value, Expected<T, ConversionCode>>::type
convertTo(StringPiece* src) noexcept {
	return str_to_floating<T>(src);
}

template<typename T>
typename std::enable_if<is_integral_v<T> && !std::is_same<T, bool>::value,
...
typename std::enable_if<is_integral_v<Tgt> && !std::is_same<Tgt, bool>::value,
                        Expected<Tgt, ConversionCode>>::type
...
parseTo(StringPiece src, Tgt& out) {
	return detail::convertTo<Tgt>(&src).then([&](Tgt res) { return void(out = res), src; });
}
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<is_integral_v<Src> && !std::is_same<Tgt, Src>::value &&
                                !std::is_same<Tgt, bool>::value && is_integral_v<Tgt>,
                        Expected<Tgt, ConversionCode>>::type
convertTo(const Src& value) noexcept {
	if constexpr(make_unsigned_t<Tgt>(std::numeric_limits<Tgt>::max()) <
	...
	if constexpr(is_signed_v<Src> && (!is_signed_v<Tgt> || sizeof(Src) > sizeof(Tgt))) {
 ...
 */
template<class Tgt, class Src>
typename std::enable_if<std::is_floating_point<Tgt>::value && std::is_floating_point<Src>::value &&
...
convertTo(const Src& value) noexcept {
	...
	if constexpr(std::numeric_limits<Tgt>::max() < std::numeric_limits<Src>::max()) {
...
inline typename std::enable_if<std::is_floating_point<Src>::value && is_integral_v<Tgt> &&
                                       !std::is_same<Tgt, bool>::value,
                               bool>::type
...
template<typename Tgt, typename Src>
constexpr typename std::enable_if<is_integral_v<Src> && std::is_floating_point<Tgt>::value,
                                  bool>::type
...
constexpr typename std::
        enable_if<std::is_floating_point<Src>::value && std::is_same<Tgt, bool>::value, bool>::type
        checkConversion(const Src&) {
	return true;
}
 ...
 */
template<typename Tgt, typename Src>
typename std::enable_if<(is_integral_v<Src> && std::is_floating_point<Tgt>::value) ||
...
template<typename Tgt, typename Src>
using IsArithToArith =
        std::bool_constant<!std::is_same<Tgt, Src>::value && !std::is_same<Tgt, bool>::value &&
...
template<typename Tgt, typename Src>
typename std::enable_if<detail::IsArithToArith<Tgt, Src>::value, Tgt>::type to(const Src& value) {
...
template<class T>
[[nodiscard]]
typename std::enable_if<std::is_enum<T>::value, Expected<StringPiece, ConversionCode>>::type
...
[[nodiscard]]
inline Expected<StringPiece, ConversionCode> parseTo(StringPiece in, StringPiece& out) noexcept {
	out = in;
...
template<class Str>
FOLLY_ERASE Expected<StringPiece, ConversionCode> parseToStr(StringPiece in, Str& out) {
	out.clear();
...
[[nodiscard]]
inline Expected<StringPiece, ConversionCode> parseTo(StringPiece in, std::string& out) {
	return detail::parseToStr(in, out);
}

[[nodiscard]]
inline Expected<StringPiece, ConversionCode> parseTo(StringPiece in, std::string_view& out) {
	out = std::string_view(in.data(), in.size());
	return StringPiece{in.end(), in.end()};
}

[[nodiscard]]
inline Expected<StringPiece, ConversionCode> parseTo(StringPiece in, fbstring& out) {
	return detail::parseToStr(in, out);
}

template<class Str>
[[nodiscard]] inline typename std::enable_if<IsSomeString<Str>::value,
                                             Expected<StringPiece, ConversionCode>>::type
...
template<class Tgt>
inline typename std::enable_if<!std::is_void<ParseToResult<Tgt>>::value, ParseToResult<Tgt>>::type
parseToWrap(StringPiece sp, Tgt& out) {
...
template<typename Tgt>
using ParseToError =
        ExpectedErrorType<decltype(detail::parseToWrap(StringPiece{}, std::declval<Tgt&>()))>;
...
tryTo(StringPiece src) noexcept {
	...
	using Check = typename std::conditional<is_arithmetic_v<Tgt>,
	                                        detail::CheckTrailingSpace,
	                                        detail::ReturnUnit<Error>>::type;
	return parseTo(src, result).then(Check(), [&](Unit) { return std::move(result); });
}

template<class Tgt, class Src>
inline typename std::enable_if<IsSomeString<Src>::value && !std::is_same<StringPiece, Tgt>::value,
                               Tgt>::type
to(Src const& src) {
...
template<class Tgt>
inline typename std::enable_if<!std::is_same<StringPiece, Tgt>::value, Tgt>::type to(
        StringPiece src) {
	Tgt result{};
	using Error = detail::ParseToError<Tgt>;
	using Check = typename std::conditional<is_arithmetic_v<Tgt>,
	...
	return tmp.thenOrThrow(Check(), [&](Error e) { throw_exception(makeConversionError(e, src)); })
	        .thenOrThrow([&](Unit) { return std::move(result); },
	                     [&](Error e) { throw_exception(makeConversionError(e, tmp.value())); });
}
...
template<class Tgt, class Src>
typename std::enable_if<std::is_enum<Src>::value && !std::is_same<Src, Tgt>::value &&
...
template<class Tgt, class Src>
typename std::enable_if<!std::is_convertible<Src, StringPiece>::value && std::is_enum<Tgt>::value &&
