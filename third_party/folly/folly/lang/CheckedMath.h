...
template<typename T, typename = std::enable_if_t<std::is_unsigned<T>::value>>
std::enable_if_t<sizeof(T) < sizeof(uint64_t), bool> generic_checked_mul(T* result, T a, T b) {
...
std::enable_if_t<sizeof(T) == sizeof(uint64_t), bool> generic_checked_mul(T* result, T a, T b) {
	...
	if(FOLLY_UNLIKELY(!generic_checked_add(result, bot_bits, mid_bits << halfBits))) {
		*result = {};
		return false;
	}
	return true;
