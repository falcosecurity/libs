...
static constexpr size_t __folly_unwind_exception_align = alignof(__folly_unwind_exception_align_t);
#endif

static constexpr uint64_t __gxx_primary_exception_class = 0x474E5543432B2B00;    // GNCUC++\0
static constexpr uint64_t __gxx_dependent_exception_class = 0x474E5543432B2B01;  // GNCUC++\1
...
public:
	...
	virtual bool can_catch(const std::type_info* thrown_type, void*& adjustedPtr) const = 0;
};
...
public:
	...
	virtual bool __do_catch(std::type_info const* thrown_type,
	                        void** thrown_object,
	                        unsigned outer) const = 0;
	virtual bool __do_upcast(std::type_info const* target, void** thrown_object) const = 0;
};
...
std::type_info const* exception_ptr_exception_typeid(std::exception const& ex) noexcept {
...
void* exception_ptr_get_object_(std::exception_ptr const& ptr,
...
static bool cxxabi_cxa_exception_sans_reserve() noexcept {
	...
	auto exception = static_cast<abi::__folly_cxa_exception_sans_reserve*>(object) - 1;
	auto result = exception->referenceCount == 1;
	assert(result ||
	       (static_cast<abi::__folly_cxa_exception_with_reserve*>(object) - 1)->referenceCount ==
	               1);
...
static T* win32_decode_pointer(T* ptr) {
	return static_cast<T*>(DecodePointer(const_cast<void*>(static_cast<void const*>(ptr))));
}
...
static EHExceptionRecord* win32_get_record(std::exception_ptr const& ptr) noexcept {
...
std::exception_ptr current_exception() noexcept {
	...
	auto const exception = static_cast<abi::__cxa_exception*>(globals.caughtExceptions);
	if(!exception) {
	...
	case abi::__gxx_primary_exception_class: {
		...
		assume(!!object);
		return std::exception_ptr(reinterpret_cast<std::exception_ptr const&>(object));
	}
...
std::exception_ptr make_exception_ptr_with_(make_exception_ptr_with_arg_ const& arg,
                                            void* func) noexcept {
...
[[maybe_unused]] static void exception_cleanup_(_Unwind_Reason_Code reason,
                                                _Unwind_Exception* uwexception) {
	if(reason == _URC_FOREIGN_EXCEPTION_CAUGHT) {
		auto handler = cxxabi_with_cxa_exception(uwexception + 1,
		                                         [](auto exn) { return exn->terminateHandler; });
...
#if _LIBCPP_VERSION >= 180000 && _LIBCPP_AVAILABILITY_HAS_INIT_PRIMARY_EXCEPTION
	(void)abi::__cxa_init_primary_exception(object, type, arg.dtor);
	cxxabi_with_cxa_exception(object, [&](auto exception) { exception->referenceCount = 1; });
#else
...
auto exception_shared_string::to_state(const tagged_what_t& w) noexcept -> state* {
	if(w.is_literal()) {
		return nullptr;
	}
...
exception_shared_string::exception_shared_string(std::size_t const len,
                                                 format_sig_& ffun,
                                                 void* const fobj):
        tagged_what_{vtag<false>, from_state(state::make(len, ffun, fobj))} {}

exception_shared_string::exception_shared_string(char const* const str, std::size_t const len):
        tagged_what_{vtag<false>, from_state(state::make(str, len))} {}

exception_shared_string::exception_shared_string(exception_shared_string const& that) noexcept:
        tagged_what_{(state::copy(to_state(that.tagged_what_)), that.tagged_what_)} {}
