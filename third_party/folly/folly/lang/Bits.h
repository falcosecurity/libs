...
template<typename To,
         typename From,
         std::enable_if_t<sizeof(From) == sizeof(To) && std::is_trivially_copyable<To>::value &&
...
template<typename T>
inline constexpr bool supported_in_bits_operations_v = std::is_unsigned_v<T> && sizeof(T) <= 8;
...
inline constexpr clear_n_least_significant_bits_fn clear_n_least_significant_bits;
...
struct get_bit_at_fn {
	...
	[[nodiscard]] constexpr bool operator()(const Uint* ptr, std::size_t idx) const noexcept {
...
inline T partialLoadUnaligned(const void* p, size_t l) {
	static_assert(std::is_integral<T>::value && std::is_unsigned<T>::value && sizeof(T) <= 8,
	              "Invalid type");
	assume(l < sizeof(T));
...
constexpr T constexprLoadUnaligned(const S* s) {
	...
	return is_constant_evaluated_or(false) ? detail::constexprLoadUnalignedImpl<T>(s)
	                                       : loadUnaligned<T>(s);
}
...
constexpr T constexprPartialLoadUnaligned(const S* s, std::size_t l) {
	...
	return is_constant_evaluated_or(false) ? detail::constexprPartialLoadUnalignedImpl<T>(s, l)
