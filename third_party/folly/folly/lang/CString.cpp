...
void* memrchr_fallback(void* s, int c, std::size_t len) noexcept {
	return const_cast<void*>(memrchr_fallback(const_cast<void const*>(s), c, len));
}
...
using invoke_memrchr_fn =
        invoke_first_match<invoke_primary_memrchr_fn, invoke_fallback_memrchr_fn, tag_t<>>;
...
std::size_t strlcpy(char* const dest, char const* const src, std::size_t const size) {
