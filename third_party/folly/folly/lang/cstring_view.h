...
class basic_cstring_view {
	...
	static constexpr Char const* check_string(Char const* str,
...
public:
	...
	constexpr const_reverse_iterator rbegin() const noexcept { return view_.rbegin(); }
	constexpr const_reverse_iterator rend() const noexcept { return view_.rend(); }
	constexpr const_reverse_iterator crbegin() const noexcept { return view_.crbegin(); }
	constexpr const_reverse_iterator crend() const noexcept { return view_.crend(); }
	...
	constexpr size_type find(Char const* s, size_type pos = 0) const { return view_.find(s, pos); }
	...
	constexpr int compare(view_type sv) const noexcept { return view_.compare(sv); }
	...
	constexpr void swap(basic_cstring_view& other) noexcept { view_.swap(other.view_); }

#if __cpp_lib_starts_ends_with >= 201711L
	constexpr bool starts_with(view_type sv) const noexcept { return view_.starts_with(sv); }
	constexpr bool starts_with(Char ch) const noexcept { return view_.starts_with(ch); }
	constexpr bool starts_with(Char const* s) const { return view_.starts_with(s); }

	constexpr bool ends_with(view_type sv) const noexcept { return view_.ends_with(sv); }
	constexpr bool ends_with(Char ch) const noexcept { return view_.ends_with(ch); }
	constexpr bool ends_with(Char const* s) const { return view_.ends_with(s); }
#endif

#if defined(__cpp_lib_string_contains) && __cpp_lib_string_contains >= 202011L
	constexpr bool contains(view_type sv) const noexcept { return view_.contains(sv); }
	...
	constexpr basic_cstring_view substr(size_type pos, size_type len) const = delete;
	constexpr void remove_suffix(size_type n) = delete;

	friend bool operator==(self lhs, self rhs) noexcept { return view_type(lhs) == view_type(rhs); }
	friend bool operator!=(self lhs, self rhs) noexcept { return view_type(lhs) != view_type(rhs); }
	friend bool operator<(self lhs, self rhs) noexcept { return view_type(lhs) < view_type(rhs); }
	friend bool operator<=(self lhs, self rhs) noexcept { return view_type(lhs) <= view_type(rhs); }
	friend bool operator>(self lhs, self rhs) noexcept { return view_type(lhs) > view_type(rhs); }
...
namespace std {
template<typename Char, typename Traits>
struct hash<folly::basic_cstring_view<Char, Traits>> : hash<std::basic_string_view<Char, Traits>> {
};
}  // namespace std
