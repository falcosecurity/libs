...
constexpr std::memory_order memory_order_load(std::memory_order const order) noexcept {
...
constexpr std::memory_order memory_order_store(std::memory_order const order) noexcept {
	constexpr auto relaxed = std::memory_order_relaxed;
	constexpr auto release = std::memory_order_release;
...
constexpr std::memory_order atomic_compare_exchange_succ(bool cond,
...
constexpr std::memory_order atomic_compare_exchange_succ(std::memory_order succ,
                                                         std::memory_order fail) {
...
struct atomic_fetch_set_fallback_fn {
	...
	bool operator()(Atomic& atomic, std::size_t bit, std::memory_order order) const {
...
inline bool atomic_fetch_set_native(std::atomic<Integer>& atomic,
                                    std::size_t bit,
	...
	if constexpr(sizeof(Integer) == 4) {
		return _interlockedbittestandset(reinterpret_cast<volatile long*>(&atomic),
		                                 static_cast<long>(bit));
	} else if constexpr(sizeof(Integer) == 8) {
		return _interlockedbittestandset64(reinterpret_cast<volatile long long*>(&atomic),
...
template<typename Atomic>
inline bool atomic_fetch_set_native(Atomic& atomic, std::size_t bit, std::memory_order order) {
...
template<typename Integer>
inline bool atomic_fetch_reset_native(std::atomic<Integer>& atomic,
                                      std::size_t bit,
                                      std::memory_order order) {
	static_assert(alignof(std::atomic<Integer>) == alignof(Integer));
	static_assert(sizeof(std::atomic<Integer>) == sizeof(Integer));
	...
	if constexpr(sizeof(Integer) == 4) {
		return _interlockedbittestandreset(reinterpret_cast<volatile long*>(&atomic),
		                                   static_cast<long>(bit));
	} else if constexpr(sizeof(Integer) == 8) {
		return _interlockedbittestandreset64(reinterpret_cast<volatile long long*>(&atomic),
		                                     static_cast<long long>(bit));
	} else {
		assert(sizeof(Integer) != 4 && sizeof(Integer) != 8);
...
template<typename Atomic>
inline bool atomic_fetch_reset_native(Atomic& atomic, std::size_t bit, std::memory_order mo) {
	static_assert(!std::is_same<Atomic, std::atomic<std::uint32_t>>{});
	static_assert(!std::is_same<Atomic, std::atomic<std::uint64_t>>{});
...
template<typename Atomic>
inline bool atomic_fetch_flip_native(Atomic& atomic, std::size_t bit, std::memory_order mo) {
...
#define FOLLY_DETAIL_ATOMIC_BIT_OP_ONE(mnem, suff)                                       \
	if constexpr(Instr == mnem_t::mnem && sizeof(Int) == 1 << (int(suff_t::suff) + 1)) { \
...
struct atomic_fetch_bit_op_native_do_instr_fn {
	...
	FOLLY_ERASE bool operator()(Int* ptr, Int bit, std::memory_order order) const {
...
static constexpr auto& atomic_fetch_bit_op_native_bts =
        atomic_fetch_bit_op_native_do_instr<atomic_fetch_bit_op_native_instr_mnem::bts>;
static constexpr auto& atomic_fetch_bit_op_native_btr =
        atomic_fetch_bit_op_native_do_instr<atomic_fetch_bit_op_native_instr_mnem::btr>;
static constexpr auto& atomic_fetch_bit_op_native_btc =
        atomic_fetch_bit_op_native_do_instr<atomic_fetch_bit_op_native_instr_mnem::btc>;

#undef FOLLY_DETAIL_ATOMIC_BIT_OP_ONE

template<typename Integer, typename Op, typename Fb>
FOLLY_ERASE bool atomic_fetch_bit_op_native_(std::atomic<Integer>& atomic,
                                             std::size_t bit,
                                             std::memory_order order,
                                             Op op,
                                             Fb fb) {
...
template<typename Integer, typename Op, typename Fb>
FOLLY_ERASE bool atomic_fetch_bit_op_native_(std::atomic_ref<Integer>& atomic,
                                             std::size_t bit,
                                             std::memory_order order,
                                             Op op,
                                             Fb fb) {
...
template<typename Integer>
inline bool atomic_fetch_set_native(atomic_ref<Integer>& atomic,
                                    std::size_t bit,
                                    std::memory_order order) {
...
#if defined(__cpp_lib_atomic_ref) && __cpp_lib_atomic_ref >= 201806L
template<typename Integer>
inline bool atomic_fetch_set_native(std::atomic_ref<Integer>& atomic,
                                    std::size_t bit,
                                    std::memory_order order) {
	auto op = atomic_fetch_bit_op_native_bts;
	auto fb = atomic_fetch_set_fallback;
...
template<typename Integer>
inline bool atomic_fetch_reset_native(atomic_ref<Integer>& atomic,
                                      std::size_t bit,
                                      std::memory_order order) {
...
#if defined(__cpp_lib_atomic_ref) && __cpp_lib_atomic_ref >= 201806L
template<typename Integer>
inline bool atomic_fetch_reset_native(std::atomic_ref<Integer>& atomic,
                                      std::size_t bit,
                                      std::memory_order order) {
	auto op = atomic_fetch_bit_op_native_btr;
	auto fb = atomic_fetch_reset_fallback;
...
template<typename Atomic>
bool atomic_fetch_reset_native(Atomic& atomic, std::size_t bit, std::memory_order order) {
	static_assert(!is_instantiation_of_v<std::atomic, Atomic>);
	return atomic_fetch_reset_fallback(atomic, bit, order);
}

template<typename Integer>
inline bool atomic_fetch_flip_native(std::atomic<Integer>& atomic,
                                     std::size_t bit,
                                     std::memory_order order) {
...
template<typename Integer>
inline bool atomic_fetch_flip_native(atomic_ref<Integer>& atomic,
                                     std::size_t bit,
                                     std::memory_order order) {
...
#if defined(__cpp_lib_atomic_ref) && __cpp_lib_atomic_ref >= 201806L
template<typename Integer>
inline bool atomic_fetch_flip_native(std::atomic_ref<Integer>& atomic,
                                     std::size_t bit,
                                     std::memory_order order) {
	auto op = atomic_fetch_bit_op_native_btc;
	auto fb = atomic_fetch_flip_fallback;
...
template<typename Atomic>
bool atomic_fetch_flip_native(Atomic& atomic, std::size_t bit, std::memory_order order) {
	static_assert(!is_instantiation_of_v<std::atomic, Atomic>);
...
template<typename Atomic>
bool atomic_fetch_set_fn::operator()(Atomic& atomic, std::size_t bit, std::memory_order mo) const {
	detail::atomic_fetch_bit_op_check_(atomic, bit);
	return detail::atomic_fetch_set_native(atomic, bit, mo);
}

template<typename Atomic>
bool atomic_fetch_reset_fn::operator()(Atomic& atomic,
                                       std::size_t bit,
...
template<typename Atomic>
bool atomic_fetch_flip_fn::operator()(Atomic& atomic, std::size_t bit, std::memory_order mo) const {
	detail::atomic_fetch_bit_op_check_(atomic, bit);
	return detail::atomic_fetch_flip_native(atomic, bit, mo);
}

template<typename Atomic, typename Op>
atomic_value_type_t<Atomic> atomic_fetch_modify_fn::operator()(Atomic& atomic,
                                                               Op op,
