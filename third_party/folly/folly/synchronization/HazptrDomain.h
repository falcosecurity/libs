...
class hazptr_domain {
	...
	struct HazptrRecArrayPolicy : atomic_grow_array_policy_default<Rec, Atom> {
		...
		/* implicit */ HazptrRecArrayPolicy(hazptr_domain* domain) noexcept: domain_{domain} {}
	...
	static_assert((kNumShards & kShardMask) == 0, "kNumShards must be a power of 2");
...
public:
	...
	~hazptr_domain() {
		...
		if(kIsDebug && !tagged_empty()) {
			LOG(WARNING) << "Tagged objects remain. This may indicate a higher-level leak "
	...
	void set_executor(ExecutorFn exfn) { exec_fn_.store(exfn, std::memory_order_release); }

	void clear_executor() { exec_fn_.store(&no_executor, std::memory_order_release); }
	...
	void retire(T* obj, D reclaim = {}) {
		struct hazptr_retire_node : hazptr_obj<Atom> {
			...
			hazptr_retire_node(T* retireObj, D toReclaim): obj_{retireObj, std::move(toReclaim)} {}
		};
	...
	void list_match_tag(uintptr_t ftag, Obj* obj, ObjList& match, ObjList& nomatch) {
...
private:
	...
	explicit hazptr_domain(default_domain_tag) noexcept: is_default_domain_{true} {}
	...
	int exchange_count(int val) { return count_.exchange(val, std::memory_order_acq_rel); }
	...
	bool cas_due_time(uint64_t& expected, uint64_t newval) {
		return due_time_.compare_exchange_strong(expected,
		                                         newval,
		                                         std::memory_order_acq_rel,
		                                         std::memory_order_relaxed);
	}

	uint16_t load_num_bulk_reclaims() { return num_bulk_reclaims_.load(std::memory_order_acquire); }

	void inc_num_bulk_reclaims() { num_bulk_reclaims_.fetch_add(1, std::memory_order_release); }
	...
	void store_avail(uintptr_t val) { avail_.store(val, std::memory_order_release); }

	bool cas_avail(uintptr_t& expval, uintptr_t newval) {
		return avail_.compare_exchange_weak(expval,
		                                    newval,
		                                    std::memory_order_acq_rel,
	...
	void push_list(ObjList& l) {
		...
		/*** Full fence ***/ asymmetric_thread_fence_light(std::memory_order_seq_cst);
	...
	size_t calc_shard(uintptr_t ftag) {
		size_t shard = (std::hash<uintptr_t>{}(ftag) >> kIgnoredLowBits) & kShardMask;
		DCHECK(shard < kNumShards);
		return shard;
	}

	size_t calc_shard(Obj* obj) { return calc_shard(reinterpret_cast<uintptr_t>(obj)); }
	...
	void do_reclamation(int rcount) {
		...
		while(true) {
			...
			if(extract_retired_objects(untagged, tagged)) {
				/*** Full fence ***/ asymmetric_thread_fence_heavy(std::memory_order_seq_cst);
	...
	void list_match_condition(Obj* obj, ObjList& match, ObjList& nomatch, const Cond& cond) {
		while(obj) {
			auto next = obj->next();
			DCHECK_NE(obj, next);
	...
	void dcheck_connected(Rec* head, Rec* tail) {
		Rec* rec = head;
		[[FOLLY_ATTR_MAYBE_UNUSED_IF_NDEBUG]] bool connected = false;
		while(rec) {
	...
	FOLLY_NOINLINE HazptrRecArray& get_or_create_hprecs_slow() {
		...
		if(!hprecs_.compare_exchange_strong(expected,
		                                    instance,
	...
	void schedule_reclamation(int rcount) {
		...
		bool canUseExecutor = std::is_same<Atom<int>, std::atomic<int>>{} && is_default_domain() &&
		                      hazptr_use_executor();
		if(canUseExecutor) {
	...
	FOLLY_EXPORT FOLLY_NOINLINE void hazptr_warning_list_too_large(uintptr_t ftag,
	                                                               size_t shard,
	                                                               int count) {
	...
	FOLLY_EXPORT FOLLY_NOINLINE void hazptr_warning_executor_backlog(int backlog) {
		static std::atomic<uint64_t> warning_count{0};
		if((warning_count++ % 10000) == 0) {
			LOG(WARNING) << backlog
...
template<template<typename> class Atom>
FOLLY_ALWAYS_INLINE hazard_pointer_domain<Atom>& hazard_pointer_default_domain() {
	return default_hazptr_domain<Atom>();
}

/** hazptr_domain_push_retired: push a list of retired objects into a domain */
template<template<typename> class Atom>
void hazptr_domain_push_retired(hazptr_obj_list<Atom>& l, hazptr_domain<Atom>& domain) noexcept {
	domain.push_list(l);
}
