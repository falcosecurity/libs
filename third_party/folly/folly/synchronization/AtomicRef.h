...
public:
	static_assert(sizeof(value_type) == sizeof(std::atomic<value_type>), "size mismatch");
	static_assert(std::is_trivially_copyable_v<value_type>, "value not trivially-copyable");

	static inline constexpr std::size_t required_alignment = alignof(std::atomic<value_type>);
...
private:
	void check_alignment_() const noexcept {
		auto ptr = reinterpret_cast<uintptr_t>(&reinterpret_cast<unsigned char const&>(ref_));
...
using atomic_ref_select =
        conditional_t<std::is_integral<TD>::value && !std::is_same<TD, bool>::value,
...
struct make_atomic_ref_t {
	template<typename T,
	         ...
	         std::enable_if_t<std::is_trivially_copyable_v<TD> &&  //
