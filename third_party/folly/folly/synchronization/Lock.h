...
public:
	...
	static_assert(std::is_void<state_type>::value || std::is_constructible<bool, state_type>::value,
...
public:
	...
	[[nodiscard]] lock_base(type_t<M>& mutex, std::adopt_lock_t):
	        ...
	        state_{owner_true_(tag<owner_type>)} {}
	template<typename M = mutex_type, if_<has_state_, M>* = nullptr>
	[[nodiscard]] lock_base(type_t<M>& mutex, std::adopt_lock_t, owner_type const& state):
	        mutex_{std::addressof(mutex)},
	        state_{state} {
		state_ || (check_fail_<true>(), 0);
	}
	template<typename M = mutex_type, if_<has_state_, M>* = nullptr>
	[[nodiscard]] lock_base(type_t<M>& mutex, adopt_lock_state_t, owner_type const& state):
	        lock_base{mutex, std::adopt_lock, state} {}
	[[nodiscard]] explicit lock_base(mutex_type& mutex): mutex_{std::addressof(mutex)} { lock(); }
	lock_base(mutex_type& mutex, std::defer_lock_t) noexcept: mutex_{std::addressof(mutex)} {}
	[[nodiscard]] lock_base(mutex_type& mutex, std::try_to_lock_t): mutex_{std::addressof(mutex)} {
		try_lock();
	}
	template<typename Rep, typename Period>
	[[nodiscard]] lock_base(mutex_type& mutex, std::chrono::duration<Rep, Period> const& timeout):
	        mutex_{std::addressof(mutex)} {
		try_lock_for(timeout);
	}
	template<typename Clock, typename Duration>
	[[nodiscard]] lock_base(mutex_type& mutex,
...
template<typename Mutex, typename Policy>
class lock_guard_base : unsafe_for_async_usage_if<!is_coro_aware_mutex_v<Mutex>> {
private:
...
public:
	...
	template<bool C = has_state_, if_<!C> = 0>
	lock_guard_base(mutex_type& mutex, std::adopt_lock_t): lock_{mutex, std::adopt_lock} {}
	template<bool C = has_state_, if_<C> = 0>
	lock_guard_base(mutex_type& mutex, std::adopt_lock_t, state_type_ const& state):
	        lock_{mutex, std::adopt_lock, state} {}
	template<bool C = has_state_, if_<C> = 0>
	lock_guard_base(mutex_type& mutex, adopt_lock_state_t, state_type_ const& state):
	        lock_{mutex, std::adopt_lock, state} {}
...
using lock_policy_hybrid = conditional_t<is_invocable_v<access::lock_shared_fn, Mutex&>,
                                         lock_policy_shared,
                                         lock_policy_unique>;
...
//  to derive this.
template<typename Mutex>
class unique_lock_guard_base : public detail::lock_guard_base<Mutex, detail::lock_policy_unique> {
private:
...
//  Works with both lockable mutex types and lockable-with-state mutex types.
template<typename Mutex>
class shared_lock_guard : public detail::lock_guard_base<Mutex, detail::lock_policy_shared> {
private:
...
template<typename Mutex>
class hybrid_lock_guard : public detail::lock_guard_base<Mutex, detail::lock_policy_hybrid<Mutex>> {
private:
	using base = detail::lock_guard_base<Mutex, detail::lock_policy_hybrid<Mutex>>;
...
using transition_lock_result_t_ = typename transition_lock_result_<
        lock_state_type_of_t<From>>::template apply<Transition, typename From::mutex_type&, A...>;
...
struct transition_lock_policy<shared_lock<Mutex>, upgrade_lock<Mutex>> {
	...
	using try_transition_for_fn = access::try_unlock_shared_and_lock_upgrade_for_fn;
	using try_transition_until_fn = access::try_unlock_shared_and_lock_upgrade_until_fn;
};
template<typename Mutex>
...
template<template<typename> class ToLock, typename Mutex, template<typename> class FromLock>
...
//  available.
template<template<typename> class ToLock,
         typename Mutex,
         template<typename>
         class FromLock,
         typename Rep,
         typename Period>
ToLock<Mutex> try_transition_lock_for(FromLock<Mutex>& lock,
...
template<template<typename> class ToLock,
         ...
         typename Clock,
         typename Duration>
ToLock<Mutex> try_transition_lock_until(FromLock<Mutex>& lock,
                                        std::chrono::time_point<Clock, Duration> const& deadline) {
	using policy = detail::transition_lock_policy<FromLock<Mutex>, ToLock<Mutex>>;
