...
private:
	...
	void set_head(Node* node) noexcept { head_.store(node, std::memory_order_release); }

	bool cas_head(Node* expected, Node* node) noexcept {
		return head_.compare_exchange_weak(expected,
		                                   node,
	...
	bool cas_tail(Node* expected, Node* node) noexcept {
		return tail_.compare_exchange_weak(expected,
		                                   node,
		                                   std::memory_order_acq_rel,
		                                   std::memory_order_relaxed);
	}

	Node* exchange_head() noexcept { return head_.exchange(nullptr, std::memory_order_acq_rel); }

	Node* exchange_tail() noexcept { return tail_.exchange(nullptr, std::memory_order_acq_rel); }
...
public:
	...
	void push_unlock(linked_list<Node>& l) noexcept {
		...
		while(true) {
			...
			auto newval = (t == nullptr) ? ptrval : reinterpret_cast<uintptr_t>(l.head());
			newval += lockbit;
			if(cas_head(oldval, newval)) {
				break;
			}
...
private:
	uintptr_t head() const noexcept { return head_.load(std::memory_order_acquire); }
	...
	bool cas_head(uintptr_t& oldval, uintptr_t newval) noexcept {
		return head_.compare_exchange_weak(oldval,
		                                   newval,
		                                   std::memory_order_acq_rel,
