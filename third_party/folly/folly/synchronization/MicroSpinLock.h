...
struct MicroSpinLock {
	...
	bool try_lock() noexcept {
		...
		annotate_rwlock_try_acquired(this, annotate_rwlock_level::wrlock, ret, __FILE__, __LINE__);
		return ret;
	}
	...
	void lock() noexcept {
		...
		annotate_rwlock_acquired(this, annotate_rwlock_level::wrlock, __FILE__, __LINE__);
	}

	void unlock() noexcept {
		assert(payload()->load() == LOCKED);
		annotate_rwlock_released(this, annotate_rwlock_level::wrlock, __FILE__, __LINE__);
...
private:
	...
	uint8_t xchg(uint8_t newVal) noexcept {
		return std::atomic_exchange_explicit(payload(), newVal, std::memory_order_acq_rel);
	}
};
static_assert(std::is_standard_layout<MicroSpinLock>::value &&
...
private:
	...
	static_assert(sizeof(PaddedSpinLock) == hardware_destructive_interference_size,
	              "Invalid size of PaddedSpinLock");

	// Check if T can theoretically cross a cache line.
	static_assert(max_align_v > 0 && hardware_destructive_interference_size % max_align_v == 0 &&
