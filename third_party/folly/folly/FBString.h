...
public:
	...
	size_t size() const {
		...
		if constexpr(kIsLittleEndian) {
			...
			auto maybeSmallSize =
			        size_t(maxSmallSize) - size_t(static_cast<UChar>(small_[maxSmallSize]));
			// With this syntax, GCC and Clang generate a CMOV instead of a branch.
			ret = (static_cast<ptrdiff_t>(maybeSmallSize) >= 0) ? maybeSmallSize : ret;
		} else {
...
private:
	...
	struct RefCounted {
		...
		static RefCounted* create(size_t* size) {
			...
			if(!folly::checked_muladd(&capacityBytes,
			                          capacityBytes,
			                          sizeof(Char),
			                          getDataOffset())) {
				throw_exception(std::length_error(""));
	...
	struct MediumLarge {
		...
		void setCapacity(size_t cap, Category cat) {
			capacity_ = kIsLittleEndian ? cap | (static_cast<size_t>(cat) << kCategoryShift)
...
public:
	dummy_fbstring_core() {}
	dummy_fbstring_core(const dummy_fbstring_core& another): backend_(another.backend_) {}
...
class basic_fbstring {
	...
	struct Invariant {
		...
		explicit Invariant(const basic_fbstring& s) noexcept: s_(s) { assert(s_.isSane()); }
...
private:
	...
	using if_is_string_view_like_t = std::enable_if_t<is_string_view_like_v<StringViewLike>, Dummy>;
	...
	FOLLY_NOINLINE basic_fbstring(string_view_type view, const A&, string_view_ctor):
...
public:
	...
	FOLLY_NOINLINE
	basic_fbstring(const value_type* s, size_type n, const A& /*a*/ = A()): store_(s, n) {}
	...
	FOLLY_NOINLINE
	basic_fbstring(std::initializer_list<value_type> il) { assign(il.begin(), il.end()); }

	template<typename StringViewLike, if_is_string_view_like_t<StringViewLike, int> = 0>
	explicit basic_fbstring(const StringViewLike& view, const A& a = A()):
	        basic_fbstring(string_view_type(view), a, string_view_ctor{}) {}

	template<typename StringViewLike, if_is_string_view_like_t<StringViewLike, int> = 0>
	basic_fbstring(const StringViewLike& view, size_type pos, size_type n, const A& a = A()):
	        basic_fbstring(string_view_type(view).substr(pos, n), a, string_view_ctor{}) {}
	...
	basic_fbstring& append(const basic_fbstring& str, const size_type pos, size_type n);

	basic_fbstring& append(const value_type* s, size_type n);

	basic_fbstring& append(const value_type* s) { return append(s, traitsLength(s)); }
	...
	basic_fbstring& assign(basic_fbstring&& str) { return *this = std::move(str); }
	...
	basic_fbstring& assign(const value_type* s) { return assign(s, traitsLength(s)); }
...
public:
	friend inline istream_type& getline(istream_type& is, basic_fbstring& str, value_type delim) {
...
public:
...
#if FOLLY_CPLUSPLUS >= 202002L
	friend auto operator<=>(const basic_fbstring& lhs, const basic_fbstring& rhs) {
	...
	template<typename A2>
	friend auto operator<=>(const basic_fbstring& lhs, const std::basic_string<E, T, A2>& rhs) {
		return lhs.spaceship(rhs.data(), rhs.size());
	}
...
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A, S>::operator=(value_type c) {
	Invariant checker(*this);
...
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A, S>::append(const basic_fbstring& str) {
#ifndef NDEBUG
...
template<typename E, class T, class A, class S>
FOLLY_NOINLINE basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A, S>::append(const value_type* s,
                                                                              size_type n) {
	Invariant checker(*this);
...
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A, S>::append(size_type n, value_type c) {
	Invariant checker(*this);
	auto pData = store_.expandNoinit(n, /* expGrowth = */ true);
	fbstring_detail::podFill(pData, pData + n, c);
	return *this;
...
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A, S>::assign(const basic_fbstring& str,
                                                                      const size_type pos,
                                                                      size_type n) {
	const size_type sz = str.size();
	enforce<std::out_of_range>(pos <= sz, "");
...
template<typename E, class T, class A, class S>
FOLLY_NOINLINE basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A, S>::assign(const value_type* s,
                                                                              const size_type n) {
	Invariant checker(*this);
...
template<typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::istream_type& basic_fbstring<E, T, A, S>::getlineImpl(
        istream_type& is,
        value_type delim) {
	Invariant checker(*this);
...
template<typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type basic_fbstring<E, T, A, S>::find(
...
template<typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::iterator basic_fbstring<E, T, A, S>::insertImplDiscr(
        const_iterator i,
        size_type n,
        value_type c,
        std::true_type) {
...
template<typename E, class T, class A, class S>
template<class FwdIterator>
inline typename basic_fbstring<E, T, A, S>::iterator basic_fbstring<E, T, A, S>::insertImpl(
        const_iterator i,
        FwdIterator s1,
        FwdIterator s2,
        std::forward_iterator_tag) {
...
basic_fbstring<E, T, A, S>::rfind(const value_type* s, size_type pos, size_type n) const {
	if(n > length()) {
		return npos;
	}
...
template<typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type
basic_fbstring<E, T, A, S>::find_first_of(const value_type* s, size_type pos, size_type n) const {
	if(pos > length() || n == 0) {
		return npos;
	}
...
template<typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type
basic_fbstring<E, T, A, S>::find_last_of(const value_type* s, size_type pos, size_type n) const {
	if(!empty() && n > 0) {
...
template<typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type basic_fbstring<E, T, A, S>::find_first_not_of(
        const value_type* s,
        size_type pos,
        size_type n) const {
	if(pos < length()) {
...
template<typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type basic_fbstring<E, T, A, S>::find_last_not_of(
        const value_type* s,
        size_type pos,
        size_type n) const {
	if(!this->empty()) {
...
// C++11 21.4.8.1/1
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(const basic_fbstring<E, T, A, S>& lhs,
...
// C++11 21.4.8.1/5
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(const E* lhs, const basic_fbstring<E, T, A, S>& rhs) {
	//
	basic_fbstring<E, T, A, S> result;
...
// C++11 21.4.8.1/6
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(const E* lhs, basic_fbstring<E, T, A, S>&& rhs) {
	//
	const auto len = basic_fbstring<E, T, A, S>::traits_type::length(lhs);
...
// C++11 21.4.8.1/7
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(E lhs, const basic_fbstring<E, T, A, S>& rhs) {
	basic_fbstring<E, T, A, S> result;
...
// C++11 21.4.8.1/8
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(E lhs, basic_fbstring<E, T, A, S>&& rhs) {
	//
...
// C++11 21.4.8.1/9
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(const basic_fbstring<E, T, A, S>& lhs, const E* rhs) {
...
// C++11 21.4.8.1/10
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(basic_fbstring<E, T, A, S>&& lhs, const E* rhs) {
	//
...
// C++11 21.4.8.1/11
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(const basic_fbstring<E, T, A, S>& lhs, E rhs) {
	basic_fbstring<E, T, A, S> result;
...
// C++11 21.4.8.1/12
template<typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(basic_fbstring<E, T, A, S>&& lhs, E rhs) {
	//
	return std::move(lhs += rhs);
}
...
template<typename E, class T, class A, class S>
inline bool operator==(std::nullptr_t, const basic_fbstring<E, T, A, S>&) = delete;

template<typename E, class T, class A, class S>
inline bool operator==(const typename basic_fbstring<E, T, A, S>::value_type* lhs,
                       const basic_fbstring<E, T, A, S>& rhs) {
	return rhs == lhs;
}

template<typename E, class T, class A, class S>
inline bool operator==(const basic_fbstring<E, T, A, S>&, std::nullptr_t) = delete;
...
struct fmt::formatter<folly::fbstring> : private formatter<fmt::string_view> {
	...
	typename Context::iterator format(const folly::fbstring& s, Context& ctx) const {
