...
template<
        ...
        typename Key = typename Map::key_type,
        typename Func,
        typename = typename std::enable_if<is_invocable_r_v<typename Map::mapped_type, Func>>::type>
typename Map::mapped_type get_default(const Map& map, const Key& key, Func&& dflt) {
	auto pos = map.find(key);
...
template<class E = std::out_of_range, class Map, typename Key = typename Map::key_type>
typename Map::mapped_type& get_or_throw(Map& map,
                                        const Key& key,
                                        const StringPiece& exceptionStrPrefix = StringPiece()) {
	auto pos = map.find(key);
	if(pos != map.end()) {
 ...
 */
template<template<typename> class Optional = folly::Optional,
         class Map,
         typename Key = typename Map::key_type>
Optional<typename Map::mapped_type> get_optional(const Map& map, const Key& key) {
	auto pos = map.find(key);
	if(pos != map.end()) {
...
const typename Map::mapped_type& get_ref_default(const Map& map,
                                                 const Key& key,
...
template<class Map,
         ...
         typename = typename std::enable_if<std::is_reference<invoke_result_t<Func>>::value>::type>
const typename Map::mapped_type& get_ref_default(const Map& map, const Key& key, Func&& dflt) {
	auto pos = map.find(key);
	return (pos != map.end() ? pos->second : dflt());
}
...
template<class Map, typename Key = typename Map::key_type>
const typename Map::mapped_type* FOLLY_NULLABLE get_ptr(const Map* FOLLY_NULLABLE map,
                                                        const Key& key) {
...
template<class Map, typename Key = typename Map::key_type>
typename Map::mapped_type* FOLLY_NULLABLE get_ptr(Map* FOLLY_NULLABLE map, const Key& key) {
	return map ? get_ptr(*map, key) : nullptr;
}
...
get_ptr2(const Map& map, const Key& key0, const Key& key1) {
	...
	auto end = map.end();
	return std::make_pair(iter0 != end ? &iter0->second : nullptr,
 ...
 */
template<class Map, typename Key = typename Map::key_type>
std::pair<typename Map::mapped_type*, typename Map::mapped_type*> get_ptr2(Map& map,
                                                                           const Key& key0,
                                                                           const Key& key1) {
	const auto& iter_pair = map.find(key0, key1);
	auto iter0 = iter_pair.first;
...
namespace detail {
template<class T, size_t pathLength, class = typename std::enable_if<(pathLength > 0)>::type>
struct NestedMapType {
	using type =
	        typename NestedMapType<std::remove_pointer_t<T>, pathLength - 1>::type::mapped_type;
};
...
template<template<typename> class Optional = folly::Optional,
         ...
         class Key2,
         class... Keys>
auto get_optional(const Map& map, const Key1& key1, const Key2& key2, const Keys&... keys)
        -> Optional<typename detail::NestedMapType<Map, 2 + sizeof...(Keys)>::type> {
...
auto get_ptr(const Map& map, const Key1& key1, const Key2& key2, const Keys&... keys) ->
        typename detail::NestedMapType<Map, 2 + sizeof...(Keys)>::type const* FOLLY_NULLABLE {
	auto pos = map.find(key1);
	return pos != map.end() ? get_ptr(pos->second, key2, keys...) : nullptr;
}

template<class Map, class Key1, class Key2, class... Keys>
auto get_ptr(const Map* FOLLY_NULLABLE map, const Key1& key1, const Key2& key2, const Keys&... keys)
        -> typename detail::NestedMapType<Map, 2 + sizeof...(Keys)>::type const* FOLLY_NULLABLE {
...
auto get_ptr(Map& map, const Key1& key1, const Key2& key2, const Keys&... keys) ->
        typename detail::NestedMapType<Map, 2 + sizeof...(Keys)>::type* FOLLY_NULLABLE {
	auto pos = map.find(key1);
	return pos != map.end() ? get_ptr(pos->second, key2, keys...) : nullptr;
}

template<class Map, class Key1, class Key2, class... Keys>
auto get_ptr(Map* FOLLY_NULLABLE map, const Key1& key1, const Key2& key2, const Keys&... keys) ->
        typename detail::NestedMapType<Map, 2 + sizeof...(Keys)>::type* FOLLY_NULLABLE {
	return map ? get_ptr(*map, key1, key2, keys...) : nullptr;
}
...
auto get_default(const Map& map,
                 const Key1& key1,
                 const Key2& key2,
...
auto get_ref_default(const Map& map,
                     const Key1& key1,
                     const Key2& key2,
