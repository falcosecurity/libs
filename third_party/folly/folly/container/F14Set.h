...
class F14BasicSet {
	...
	using EnableHeterogeneousFind = std::enable_if_t<
	        ::folly::detail::EligibleForHeterogeneousFind<typename Policy::Value,
	...
	using EnableHeterogeneousInsert = std::enable_if_t<
	        ::folly::detail::EligibleForHeterogeneousInsert<typename Policy::Value,
	                                                        typename Policy::Hasher,
	                                                        typename Policy::KeyEqual,
	                                                        K>::value,
	        T>;
	...
	using EnableHeterogeneousErase = std::enable_if_t<
	        ::folly::detail::EligibleForHeterogeneousFind<
	                ...
	                typename Policy::Hasher,
	                typename Policy::KeyEqual,
	                std::conditional_t<IsIter<K>::value, typename Policy::Value, K>>::value &&
	                !IsIter<K>::value,
	        T>;
...
public:
	...
	explicit F14BasicSet(std::size_t initialCapacity,
	...
	F14BasicSet(F14BasicSet const& rhs, allocator_type const& alloc): table_(rhs.table_, alloc) {}

	F14BasicSet(F14BasicSet&& rhs) = default;

	F14BasicSet(F14BasicSet&& rhs,
	            allocator_type const& alloc) noexcept(Policy::kAllocIsAlwaysEqual):
	        table_{std::move(rhs.table_), alloc} {}

	/* implicit */ F14BasicSet(std::initializer_list<value_type> init,
	                           std::size_t initialCapacity = 0,
	                           hasher const& hash = hasher{},
	                           key_equal const& eq = key_equal{},
	                           allocator_type const& alloc = allocator_type{}):
	...
	F14BasicSet(std::initializer_list<value_type> init,
	            std::size_t initialCapacity,
	            allocator_type const& alloc):
	        table_{initialCapacity, hasher{}, key_equal{}, alloc} {
		initialInsert(init.begin(), init.end(), initialCapacity);
	...
	/// @methodset Iterators
	const_iterator cbegin() const noexcept { return table_.makeIter(table_.begin()); }
...
private:
	...
	FOLLY_ALWAYS_INLINE void bulkInsert(InputIt first, InputIt last, bool autoReserve) {
		if(autoReserve) {
...
private:
	...
	using UsableAsKey =
	        ::folly::detail::EligibleForHeterogeneousFind<key_type, hasher, key_equal, Arg>;
...
public:
	...
	std::pair<iterator, bool> emplace(Args&&... args) {
		auto rv = folly::detail::callWithConstructedKey<key_type, UsableAsKey>(
		        table_.alloc(),
		        [&](auto&&... inner) {
			        return table_.tryEmplaceValue(std::forward<decltype(inner)>(inner)...);
		        },
	...
	template<class... Args>
	std::pair<iterator, bool> emplace_token(F14HashToken const& token, Args&&... args) {
		auto rv = folly::detail::callWithConstructedKey<key_type, UsableAsKey>(
		        table_.alloc(),
		        [&](auto&&... inner) {
			        return table_.tryEmplaceValueWithToken(token,
	 ...
	 * @methodset Modifiers
	 */
	FOLLY_ALWAYS_INLINE iterator erase(const_iterator pos) { return eraseInto(pos, variadic_noop); }
	...
	FOLLY_ALWAYS_INLINE iterator eraseInto(const_iterator pos, BeforeDestroy&& beforeDestroy) {
	...
	template<typename BeforeDestroy>
	iterator eraseInto(const_iterator first, const_iterator last, BeforeDestroy&& beforeDestroy) {
		while(first != last) {
	...
	EnableHeterogeneousErase<K, size_type> eraseInto(K const& key, BeforeDestroy&& beforeDestroy) {
		return table_.eraseKeyInto(key, beforeDestroy);
	}
	 ...
	 * @methodset Lookup
	 */
	FOLLY_ALWAYS_INLINE size_type count(key_type const& key) const { return contains(key) ? 1 : 0; }

	template<typename K>
	FOLLY_ALWAYS_INLINE EnableHeterogeneousFind<K, size_type> count(K const& key) const {
	 ...
	 */
	F14HashToken prehash(key_type const& key) const { return table_.prehash(key); }
	/// @copydoc prehash
	...
	/// @copydoc prehash
	template<typename K>
	EnableHeterogeneousFind<K, F14HashToken> prehash(K const& key, std::size_t hash) const {
		return table_.prehash(key, hash);
	}
	...
	FOLLY_ALWAYS_INLINE iterator find(F14HashToken const& token, key_type const& key) {
		return const_cast<F14BasicSet const*>(this)->find(token, key);
	}

	FOLLY_ALWAYS_INLINE const_iterator find(F14HashToken const& token, key_type const& key) const {
	...
	template<typename K>
	FOLLY_ALWAYS_INLINE EnableHeterogeneousFind<K, const_iterator> find(K const& key) const {
		return table_.makeIter(table_.find(key));
	}

	template<typename K>
	FOLLY_ALWAYS_INLINE EnableHeterogeneousFind<K, iterator> find(F14HashToken const& token,
	                                                              K const& key) {
		return const_cast<F14BasicSet const*>(this)->find(token, key);
	}

	template<typename K>
	FOLLY_ALWAYS_INLINE EnableHeterogeneousFind<K, const_iterator> find(F14HashToken const& token,
	                                                                    K const& key) const {
		return table_.makeIter(table_.find(token, key));
	}
	...
	template<typename K>
	FOLLY_ALWAYS_INLINE EnableHeterogeneousFind<K, bool> contains(K const& key) const {
		return !table_.find(key).atEnd();
	}

	FOLLY_ALWAYS_INLINE bool contains(F14HashToken const& token, key_type const& key) const {
		return !table_.find(token, key).atEnd();
	}

	template<typename K>
	FOLLY_ALWAYS_INLINE EnableHeterogeneousFind<K, bool> contains(F14HashToken const& token,
	                                                              K const& key) const {
		return !table_.find(token, key).atEnd();
	}
	...
	template<typename K>
	EnableHeterogeneousFind<K, std::pair<iterator, iterator>> equal_range(K const& key) {
		return equal_range(*this, key);
	}

	template<typename K>
	EnableHeterogeneousFind<K, std::pair<const_iterator, const_iterator>> equal_range(
	        K const& key) const {
		return equal_range(*this, key);
	}
	 ...
	 * @methodset Bucket interface
	 */
	std::size_t max_bucket_count() const noexcept { return table_.max_bucket_count(); }
	...
	bool containsEqualValue(value_type const& value) const {
		return !table_.findMatching(value, [&](auto& k) { return value == k; }).atEnd();
	}
	 ...
	 * @methodset Capacity
	 */
	std::size_t getAllocatedMemorySize() const { return table_.getAllocatedMemorySize(); }
...
class F14ValueSet : public f14::detail::F14BasicSet<
                            f14::detail::SetPolicyWithDefaults<f14::detail::ValueContainerPolicy,
                                                               Key,
                                                               Hasher,
                                                               KeyEqual,
                                                               Alloc>> {
protected:
	...
	using Policy = f14::detail::
	        SetPolicyWithDefaults<f14::detail::ValueContainerPolicy, Key, Hasher, KeyEqual, Alloc>;
...
public:
	...
	void swap(F14ValueSet& rhs) noexcept(Policy::kSwapIsNoexcept) { this->table_.swap(rhs.table_); }
...
F14ValueSet(InputIt, InputIt, std::size_t = {}, Hasher = {}, KeyEqual = {}, Alloc = {})
...
F14ValueSet(InputIt, InputIt, std::size_t, Alloc)
        -> F14ValueSet<iterator_value_type_t<InputIt>,
...
template<typename Key,
         ...
         typename = detail::RequireNotAllocator<KeyEqual>,
         typename = detail::RequireAllocator<Alloc>>
F14ValueSet(std::initializer_list<Key>, std::size_t = {}, Hasher = {}, KeyEqual = {}, Alloc = {})
        -> F14ValueSet<Key, Hasher, KeyEqual, Alloc>;

template<typename Key, typename Alloc, typename = detail::RequireAllocator<Alloc>>
F14ValueSet(std::initializer_list<Key>,
            std::size_t,
            Alloc) -> F14ValueSet<Key, f14::DefaultHasher<Key>, f14::DefaultKeyEqual<Key>, Alloc>;

template<typename Key, typename Hasher, typename Alloc, typename = detail::RequireAllocator<Alloc>>
F14ValueSet(std::initializer_list<Key>, std::size_t, Hasher, Alloc)
        -> F14ValueSet<Key, Hasher, f14::DefaultKeyEqual<Key>, Alloc>;

#if FOLLY_F14_VECTOR_INTRINSICS_AVAILABLE
template<typename Key, typename Hasher, typename KeyEqual, typename Alloc>
class F14NodeSet : public f14::detail::F14BasicSet<
                           f14::detail::SetPolicyWithDefaults<f14::detail::NodeContainerPolicy,
                                                              Key,
                                                              Hasher,
                                                              KeyEqual,
                                                              Alloc>> {
protected:
	using Policy = f14::detail::
	        SetPolicyWithDefaults<f14::detail::NodeContainerPolicy, Key, Hasher, KeyEqual, Alloc>;
...
public:
	...
	void swap(F14NodeSet& rhs) noexcept(Policy::kSwapIsNoexcept) { this->table_.swap(rhs.table_); }
...
template<typename InputIt,
         ...
         typename = detail::RequireNotAllocator<KeyEqual>,
         typename = detail::RequireAllocator<Alloc>>
F14NodeSet(InputIt, InputIt, std::size_t = {}, Hasher = {}, KeyEqual = {}, Alloc = {})
...
F14NodeSet(InputIt, InputIt, std::size_t, Alloc)
        -> F14NodeSet<iterator_value_type_t<InputIt>,
                      f14::DefaultHasher<iterator_value_type_t<InputIt>>,
                      f14::DefaultKeyEqual<iterator_value_type_t<InputIt>>,
                      Alloc>;
...
template<typename Key,
         ...
         typename = detail::RequireNotAllocator<KeyEqual>,
         typename = detail::RequireAllocator<Alloc>>
F14NodeSet(std::initializer_list<Key>, std::size_t = {}, Hasher = {}, KeyEqual = {}, Alloc = {})
        -> F14NodeSet<Key, Hasher, KeyEqual, Alloc>;

template<typename Key, typename Alloc, typename = detail::RequireAllocator<Alloc>>
F14NodeSet(std::initializer_list<Key>,
           std::size_t,
           Alloc) -> F14NodeSet<Key, f14::DefaultHasher<Key>, f14::DefaultKeyEqual<Key>, Alloc>;
...
class F14VectorSetImpl
        : public F14BasicSet<SetPolicyWithDefaults<VectorContainerPolicy,
                                                   Key,
                                                   Hasher,
                                                   KeyEqual,
                                                   Alloc,
...
protected:
	using Policy = SetPolicyWithDefaults<VectorContainerPolicy,
	                                     Key,
	                                     Hasher,
	                                     KeyEqual,
	                                     Alloc,
...
private:
	...
	template<typename K>
	using IsIter = Disjunction<std::is_same<typename Policy::Iter, remove_cvref_t<K>>,
...
public:
	...
	const_iterator cbegin() const { return this->table_.linearBegin(this->size()); }
...
private:
	template<typename BeforeDestroy>
	void eraseUnderlying(typename Policy::ItemIter underlying, BeforeDestroy&& beforeDestroy) {
		...
		if(tailIndex != index) {
			auto tail =
			        this->table_.find(VectorContainerIndexSearch{static_cast<uint32_t>(tailIndex)});
			tail.item() = index;
...
public:
	...
	std::size_t erase(key_type const& key) { return eraseInto(key, variadic_noop); }
	...
	FOLLY_ALWAYS_INLINE iterator eraseInto(const_iterator pos, BeforeDestroy&& beforeDestroy) {
		auto underlying =
		        this->table_.find(VectorContainerIndexSearch{this->table_.iterToIndex(pos)});
		eraseUnderlying(underlying, beforeDestroy);
		return ++pos;
	}
	...
	template<typename K, typename BeforeDestroy>
	EnableHeterogeneousVectorErase<K, std::size_t> eraseInto(K const& key,
...
template<typename Key, typename Hasher, typename KeyEqual, typename Alloc>
class F14VectorSet
        : public f14::detail::F14VectorSetImpl<Key, Hasher, KeyEqual, Alloc, std::false_type> {
	using Super = f14::detail::F14VectorSetImpl<Key, Hasher, KeyEqual, Alloc, std::false_type>;
...
public:
	...
	const_reverse_iterator crend() const { return this->table_.values_ + this->table_.size(); }
	...
	const_iterator iter(const_reverse_iterator riter) const { return this->table_.iter(riter); }

	reverse_iterator riter(iterator it) { return this->table_.riter(it); }
	const_reverse_iterator riter(const_iterator it) const { return this->table_.riter(it); }

	friend Range<const_reverse_iterator> tag_invoke(order_preserving_reinsertion_view_fn,
...
template<typename InputIt,
         ...
         typename = detail::RequireNotAllocator<KeyEqual>,
         typename = detail::RequireAllocator<Alloc>>
F14VectorSet(InputIt, InputIt, std::size_t = {}, Hasher = {}, KeyEqual = {}, Alloc = {})
...
F14VectorSet(InputIt, InputIt, std::size_t, Alloc)
        -> F14VectorSet<iterator_value_type_t<InputIt>,
                        f14::DefaultHasher<iterator_value_type_t<InputIt>>,
                        f14::DefaultKeyEqual<iterator_value_type_t<InputIt>>,
                        Alloc>;
...
template<typename Key,
         ...
         typename = detail::RequireNotAllocator<KeyEqual>,
         typename = detail::RequireAllocator<Alloc>>
F14VectorSet(std::initializer_list<Key>, std::size_t = {}, Hasher = {}, KeyEqual = {}, Alloc = {})
        -> F14VectorSet<Key, Hasher, KeyEqual, Alloc>;

template<typename Key, typename Alloc, typename = detail::RequireAllocator<Alloc>>
F14VectorSet(std::initializer_list<Key>,
             std::size_t,
             Alloc) -> F14VectorSet<Key, f14::DefaultHasher<Key>, f14::DefaultKeyEqual<Key>, Alloc>;
...
class F14FastSet
        : public std::conditional_t<
                  sizeof(Key) < 24,
                  F14ValueSet<Key, Hasher, KeyEqual, Alloc>,
                  f14::detail::F14VectorSetImpl<Key, Hasher, KeyEqual, Alloc, std::true_type>> {
...
template<typename InputIt,
         ...
         typename = detail::RequireNotAllocator<KeyEqual>,
         typename = detail::RequireAllocator<Alloc>>
F14FastSet(InputIt, InputIt, std::size_t = {}, Hasher = {}, KeyEqual = {}, Alloc = {})
...
F14FastSet(InputIt, InputIt, std::size_t, Alloc)
        -> F14FastSet<iterator_value_type_t<InputIt>,
                      f14::DefaultHasher<iterator_value_type_t<InputIt>>,
                      f14::DefaultKeyEqual<iterator_value_type_t<InputIt>>,
                      Alloc>;
...
template<typename Key,
         ...
         typename = detail::RequireNotAllocator<KeyEqual>,
         typename = detail::RequireAllocator<Alloc>>
F14FastSet(std::initializer_list<Key>, std::size_t = {}, Hasher = {}, KeyEqual = {}, Alloc = {})
        -> F14FastSet<Key, Hasher, KeyEqual, Alloc>;

template<typename Key, typename Alloc, typename = detail::RequireAllocator<Alloc>>
F14FastSet(std::initializer_list<Key>,
           std::size_t,
           Alloc) -> F14FastSet<Key, f14::DefaultHasher<Key>, f14::DefaultKeyEqual<Key>, Alloc>;
...
bool operator==(F14ValueSet<K, H, E, A> const& lhs, F14ValueSet<K, H, E, A> const& rhs) {
...
template<typename K, typename H, typename E, typename A>
bool operator==(F14NodeSet<K, H, E, A> const& lhs, F14NodeSet<K, H, E, A> const& rhs) {
	return setsEqual(lhs, rhs);
}
...
template<typename K, typename H, typename E, typename A>
bool operator==(F14VectorSet<K, H, E, A> const& lhs, F14VectorSet<K, H, E, A> const& rhs) {
	return setsEqual(lhs, rhs);
}
...
template<typename K, typename H, typename E, typename A>
bool operator==(F14FastSet<K, H, E, A> const& lhs, F14FastSet<K, H, E, A> const& rhs) {
	return setsEqual(lhs, rhs);
}
