...
template<std::size_t Index,
         typename Type,
         std::enable_if_t<IsMemberGetFound<void, Type>::value, int> = 0>
auto get_impl(Type&& instance) -> decltype(static_cast<Type&&>(instance).template get<Index>()) {
...
using EnableIfTuple =
        void_t<decltype(get_impl<0>(std::declval<T>())), decltype(std::tuple_size<T>::value)>;
template<typename, typename T>
 ...
 */
template<typename Type, typename T = typename std::decay<Type>::type>
using EnableIfRange = void_t<decltype(access::begin(std::declval<T&>())),
...
using ForEachImplTag = std::conditional_t<
        ...
        std::conditional_t<
                ...
                index_constant<2>,
                std::conditional_t<is_invocable_v<Func, Item>, index_constant<1>, void>>>;
...
void for_each_range_impl(index_constant<2>, Sequence&& range, Func& func) {
	...
	auto three_arg_adaptor = [&func](auto&& ele, auto index, auto) -> decltype(auto) {
		return func(std::forward<decltype(ele)>(ele), index);
	};
	for_each_range_impl(index_constant<3>{}, std::forward<Sequence>(range), three_arg_adaptor);
}
...
void for_each_tuple_impl(std::index_sequence<Indices...>, Sequence&& seq, Func& func) {
	using _ = int[];
	...
	void(_{(((control == loop_continue) ? (control = invoke_returning_loop_control(
	                                               func,
...
void for_each_tuple_impl(index_constant<2>, Sequence&& seq, Func& func) {
	...
	for_each_tuple_impl(std::make_index_sequence<size::value>{}, std::forward<Sequence>(seq), func);
}
template<typename Sequence, typename Func>
void for_each_tuple_impl(index_constant<1>, Sequence&& seq, Func& func) {
	...
	for_each_tuple_impl(index_constant<2>{}, std::forward<Sequence>(seq), two_arg_adaptor);
}
...
decltype(auto) fetch_impl(RangeTag, Sequence&& sequence, Index&& index) {
	...
	using tag = std::conditional_t<std::is_same<iter_cat, std::random_access_iterator_tag>::value,
	                               BeginAddTag,
	                               IndexingTag>;
	return fetch_impl(tag{}, std::forward<Sequence>(sequence), std::forward<Index>(index));
}
