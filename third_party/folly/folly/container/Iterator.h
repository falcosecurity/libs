...
inline constexpr bool
        iterator_category_matches_v_<Iter, Category, void_t<iterator_category_t<Iter>>> =
...
template<typename Iter>
using iterator_key_type_t = remove_cvref_t<typename iterator_value_type_t<Iter>::first_type>;

//  iterator_mapped_type_t
//
//  Extracts a mapped type from an iterator.
template<typename Iter>
using iterator_mapped_type_t = typename iterator_value_type_t<Iter>::second_type;
...
public:
	...
	template<typename... Args>
	Derived& operator=(emplace_args<Args...>&& args) {
		return unpackAndEmplace(std::move(args), std::index_sequence_for<Args...>{});
	}
...
public:
	...
	template<typename... Args>
	Derived& operator=(std::pair<Args...>&& args) {
		return this->unpackAndEmplace(std::move(args), std::index_sequence_for<Args...>{});
	}
...
private:
	using Base =
	        emplace_iterator_base<emplace_iterator_impl, EmplaceImplT<Container>, implicit_unpack>;
...
using hint_emplace_iterator =
        detail::emplace_iterator_impl<detail::EmplaceHint, Container, implicit_unpack>;
...
using front_emplace_iterator =
        detail::emplace_iterator_impl<detail::EmplaceFront, Container, implicit_unpack>;
...
using back_emplace_iterator =
        detail::emplace_iterator_impl<detail::EmplaceBack, Container, implicit_unpack>;
 ...
 */
template<bool implicit_unpack = true, typename Container>
hint_emplace_iterator<Container, implicit_unpack> hint_emplacer(Container& c,
                                                                typename Container::iterator i) {
...
struct index_iterator_access_at {
	...
	constexpr decltype(auto) operator()(Container& container, Index index) const {
		if constexpr(folly::is_tag_invocable_v<index_iterator_access_at, Container&, Index>) {
...
class index_iterator {
	...
	constexpr static decltype(auto) get_reference_by_index(Container& container, IndexType index) {
...
public:
	...
	using reference = decltype(get_reference_by_index(FOLLY_DECLVAL(container_type&), size_type{}));
	using difference_type = detected_or_t<std::ptrdiff_t, get_difference_type_t, Container>;
	...
	static_assert(std::is_signed<difference_type>::value, "difference_type must be signed");
	...
	template<typename OtherContainer,
	         typename = std::enable_if_t<
	                 std::is_same<std::remove_const_t<container_type>, OtherContainer>::value &&
	...
	constexpr reference operator*() const { return get_reference_by_index(*container_, index_); }
	...
	constexpr friend index_iterator operator+(index_iterator x, difference_type n) {
		return x += n;
	}

	constexpr friend index_iterator operator+(difference_type n, index_iterator x) { return x + n; }

	constexpr friend index_iterator operator-(index_iterator x, difference_type n) {
		return x -= n;
	}

	constexpr friend difference_type operator-(index_iterator x, index_iterator y) {
		assert(x.container_ == y.container_);
		return static_cast<difference_type>(x.index_) - static_cast<difference_type>(y.index_);
	}

	// comparisons
	friend constexpr bool operator==(const index_iterator& x, const index_iterator& y) {
		assert(x.container_ == y.container_);
