...
struct span_extent {
	...
	explicit constexpr span_extent([[maybe_unused]] std::size_t const e) noexcept {
		assert(e == N);
	}
...
struct span_extent<dynamic_extent> {
	...
	/* implicit */ constexpr operator std::size_t() const noexcept { return extent; }
};
...
private:
	...
	static constexpr bool is_range_v = !std::is_same_v<bool, Size> && std::is_unsigned_v<Size> &&
	                                   std::is_pointer_v<Data> && array_element_match_v<U>;

	static constexpr size_type subspan_extent(size_type const offset, size_type const count) {
		// clang-format off
    return
...
public:
	...
	constexpr span(pointer const first, size_type const count): data_{first}, extent_{count} {}

	constexpr span(pointer const first, pointer const last):
	        data_{first},
	        extent_{to_unsigned(last - first)} {
		assert(!(last < first));
	}

	template<std::size_t N, std::size_t E = extent, if_<E == dynamic_extent || E == N> = 0>
	...
	/* implicit */ constexpr span(std::array<U, N>& arr) noexcept: data_{arr.data()}, extent_{N} {}
	...
	/* implicit */ constexpr span(Rng&& range):
	        data_{access::data(range)},
	        extent_{access::size(range)} {}
	...
	constexpr span<element_type, dynamic_extent> first(size_type const count) const {
		assert(!(extent < count));
		assert(!(size() < count));
		return {data_, count};
	}
	...
	constexpr span<element_type, dynamic_extent> last(size_type const count) const {
		assert(!(extent < count));
		assert(!(size() < count));
		return {data_ + size() - count, count};
	}
...
struct span_cast_impl_fn {
	template<template<typename, std::size_t> class Span, typename U, typename T, std::size_t Extent>
	constexpr auto operator()(Span<T, Extent> in, U* castData) const {
		assert(static_cast<void const*>(in.data()) == static_cast<void const*>(castData));
		...
		if constexpr(Extent == dynamic_extent) {
			...
			return Span<U, dynamic_extent>(castData, in.size() * sizeof(T) / sizeof(U));
		} else {
...
template<typename T, std::size_t Extent, std::enable_if_t<!std::is_const_v<T>, int> = 0>
