...
class F14BasicSet : public std::unordered_set<KeyType, Hasher, KeyEqual, Alloc> {
...
private:
	...
	template<typename K>
	using IsIter = Disjunction<std::is_same<iterator, remove_cvref_t<K>>,
	                           std::is_same<const_iterator, remove_cvref_t<K>>>;

	template<typename K, typename T>
	using EnableHeterogeneousErase =
	        std::enable_if_t<::folly::detail::EligibleForHeterogeneousFind<
	                                 key_type,
	                                 hasher,
	                                 key_equal,
		...
		return folly::detail::callWithConstructedKey<key_type, UsableAsKey>(
		        ...
		        [&](auto const&, auto&& key) {
			        if(!std::is_same<key_type, remove_cvref_t<decltype(key)>>::value) {
				        ...
				        FOLLY_SAFE_DCHECK(rv.second, "post-find emplace should always insert");
				        return rv;
			        } else {
...
private:
	...
	using BottomKeyEqualBase =
	        conditional_t<std::is_empty<KeyEqual>::value,
	                      BottomKeyEqualEmpty,
	...
	struct BottomKeyEqualCond : BottomKeyEqualBase {
		[[noreturn]] bool operator()(K const&, K const&) const { assume_unreachable(); }
	};
	template<typename K>
	struct BottomKeyEqualCond<true, K> final : BottomKeyEqualCond<false, K> {};
	template<typename K>
	using BottomKeyEqual =
	        BottomKeyEqualCond<std::is_final<KeyEqual>::value || std::is_union<KeyEqual>::value, K>;
	...
	static_assert(std::is_empty<BottomTest>::value == std::is_empty<KeyEqual>::value,
	              "mismatch is-empty");
	static_assert((std::is_final<BottomTest>::value || std::is_union<BottomTest>::value) ==
	                      (std::is_final<KeyEqual>::value || std::is_union<KeyEqual>::value),
	              "mismatch is-final");

	template<typename Iter, typename LocalIter>
	static std::enable_if_t<std::is_constructible<Iter, LocalIter const&>::value, Iter> fromLocal(
	        LocalIter const& src,
	        int = 0) {
		return Iter(src);
	}
	...
	static std::enable_if_t<!std::is_constructible<Iter, LocalIter const&>::value, Iter> fromLocal(
		...
		FOLLY_SAFE_CHECK(std::addressof(*src) == std::addressof(*dst),
	...
	static Iter findImpl(Self& self, K const& key) {
		...
		using A = typename std::allocator_traits<allocator_type>::template rebind_alloc<K>;
		using E = BottomKeyEqual<K>;
		// this is exceedingly wicked!
		auto slot = reinterpret_cast<std::unordered_set<K, hasher, E, A> const&>(self).bucket(key);
		...
		FOLLY_SAFE_DCHECK(self.size() > 3 || std::none_of(self.begin(),
		                                                  self.end(),
		                                                  [&](auto const& k) {
			                                                  return self.key_eq()(key, k);
		                                                  }),
		                  "");
		return self.end();
	}
...
public:
	...
	std::size_t getAllocatedMemorySize() const {
		std::size_t rv = 0;
		visitAllocationClasses([&](std::size_t bytes, std::size_t n) { rv += bytes * n; });
		return rv;
	}
	...
	void visitAllocationClasses(V&& visitor) const {
		...
		if(this->size() > 0) {
			visitor(sizeof(StdNodeReplica<key_type, value_type, hasher>), this->size());
		}
	}
	...
	const_iterator find(F14HashToken const&, key_type const& key) const { return find(key); }
	...
	template<typename K>
	EnableHeterogeneousFind<K, const_iterator> find(F14HashToken const&, K const& key) const {
		return find(key);
	}
	...
	template<typename K>
	EnableHeterogeneousFind<K, bool> contains(F14HashToken const&, K const& key) const {
		return find(key) != this->end();
	}
};
}  // namespace detail
...
class F14NodeSet : public f14::detail::F14BasicSet<Key, Hasher, KeyEqual, Alloc> {
...
template<typename Key, typename Hasher, typename KeyEqual, typename Alloc>
class F14ValueSet : public f14::detail::F14BasicSet<Key, Hasher, KeyEqual, Alloc> {
	using Super = f14::detail::F14BasicSet<Key, Hasher, KeyEqual, Alloc>;
...
template<typename Key, typename Hasher, typename KeyEqual, typename Alloc>
class F14VectorSet : public f14::detail::F14BasicSet<Key, Hasher, KeyEqual, Alloc> {
	using Super = f14::detail::F14BasicSet<Key, Hasher, KeyEqual, Alloc>;
...
template<typename Key, typename Hasher, typename KeyEqual, typename Alloc>
class F14FastSet : public f14::detail::F14BasicSet<Key, Hasher, KeyEqual, Alloc> {
	using Super = f14::detail::F14BasicSet<Key, Hasher, KeyEqual, Alloc>;
