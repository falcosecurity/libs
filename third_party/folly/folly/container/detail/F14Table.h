...
typename Container::size_type erase_if_impl(Container& c, Predicate& predicate) {
...
public:
	...
	friend constexpr bool operator==(F14HashToken const& a, F14HashToken const& b) noexcept {
...
#else
class F14HashToken final {
	friend constexpr bool operator==(F14HashToken const&, F14HashToken const&) noexcept {
		return true;
	}
	friend constexpr bool operator!=(F14HashToken const&, F14HashToken const&) noexcept {
		return false;
...
struct ShouldAssume32BitHash<Hasher, void_t<typename Hasher::folly_assume_32bit_hash>>
...
std::pair<std::size_t, std::size_t> splitHashImpl(std::size_t hash) {
	...
	if(!IsAvalanchingHasher<Hasher, Key>::value) {
		...
		__int64 signedHi;
		__int64 signedLo =
		        _mul128(static_cast<__int64>(hash), static_cast<__int64>(kMul), &signedHi);
		auto hi = static_cast<uint64_t>(signedHi);
		auto lo = static_cast<uint64_t>(signedLo);
#else
		auto hi = static_cast<uint64_t>((static_cast<unsigned __int128>(hash) * kMul) >> 64);
		auto lo = hash * kMul;
#endif
		hash = hi ^ lo;
		hash *= kMul;
...
public:
	...
	template<typename T>
	using IsRangeConvertible = std::enable_if_t<detail::TransparentlyConvertibleToRange<T>::value>;

	template<typename T>
	using RangeT = Range<typename detail::ValueTypeForTransparentConversionToRange<T>::type const*>;
	...
	friend bool operator!=(const F14HashedKey& a, const F14HashedKey& b) { return !(a == b); }
	friend bool operator==(const F14HashedKey& a, const TKeyType& b) {
		return KeyEqual{}(a.key_, b);
	}
	friend bool operator!=(const F14HashedKey& a, const TKeyType& b) { return !(a == b); }
	friend bool operator==(const TKeyType& a, const F14HashedKey& b) {
		return KeyEqual{}(a, b.key_);
	}
	friend bool operator!=(const TKeyType& a, const F14HashedKey& b) { return !(a == b); }
	template<typename K>
	friend EnableHeterogeneousCompare<K, bool> operator==(const F14HashedKey& a, const K& b) {
		return KeyEqual{}(a.key_, b);
	}
	template<typename K>
	friend EnableHeterogeneousCompare<K, bool> operator!=(const F14HashedKey& a, const K& b) {
	...
	template<typename K>
	friend EnableHeterogeneousCompare<K, bool> operator==(const K& a, const F14HashedKey& b) {
		return KeyEqual{}(a, b.key_);
	}
	template<typename K>
	friend EnableHeterogeneousCompare<K, bool> operator!=(const K& a, const F14HashedKey& b) {
...
using VoidDefault = std::conditional_t<std::is_same<Arg, Default>::value, void, Arg>;

template<typename Arg, typename Default>
using Defaulted = std::conditional_t<std::is_same<Arg, void>::value, Default, Arg>;
...
FOLLY_EXPORT inline F14EmptyTagVector& getF14EmptyTagVector() noexcept {
	...
	FOLLY_SAFE_DCHECK((raw % kRequiredVectorAlignment) == 0,
	                  raw,
	                  " not aligned to ",
...
struct alignas(kRequiredVectorAlignment) F14Chunk {
	...
	static constexpr unsigned kAllocatedCapacity = kCapacity + (sizeof(Item) == 16 ? 1 : 0);
	...
	static constexpr uint8_t kDecrHostedOverflowCount = static_cast<uint8_t>(-0x10);
	...
	void setCapacityScale(std::size_t scale) {
		FOLLY_SAFE_DCHECK(!isEmptyInstance(this) && scale > 0 &&
	...
	SparseMaskIter tagMatchIter(uint8x16_t needleV, svbool_t inPred, svbool_t& outPred) const {
		svuint8_t tagV = svld1_u8(inPred, &tags_[0]);
		auto eqV = svset_neonq_u8(svundef_u8(), vceqq_u8(svget_neonq(tagV), needleV));
	...
	MaskType occupiedMask() const {
		...
		auto occupiedV = vreinterpretq_u8_s8(vshrq_n_s8(vreinterpretq_s8_u8(tagV), 7));
	...
	DenseMaskIter occupiedIter() const { return DenseMaskIter{&tags_[0], this->occupiedMask()}; }

	MaskRangeIter occupiedRangeIter() const { return MaskRangeIter{this->occupiedMask()}; }

	LastOccupiedInMask lastOccupied() const { return LastOccupiedInMask{this->occupiedMask()}; }
	...
	Item* itemAddr(std::size_t i) const {
		return static_cast<Item*>(const_cast<void*>(static_cast<void const*>(&rawItems_[i])));
	}
	...
	static F14Chunk& owner(Item& item, std::size_t index) {
		auto rawAddr = static_cast<uint8_t*>(static_cast<void*>(std::addressof(item))) -
...
public:
	...
	bool operator!=(PackedChunkItemPtr const& rhs) const { return !(*this == rhs); }
...
class PackedChunkItemPtr<T*> {
	...
	static constexpr uintptr_t kAlignBits =
	        constexpr_min(uintptr_t{4}, constexpr_find_first_set(uintptr_t{sizeof(T)}) - 1);

	static constexpr uintptr_t kAlignMask = (uintptr_t{1} << kAlignBits) - 1;

	static constexpr uintptr_t kModulus = uintptr_t{1} << (kIndexBits - kAlignBits);
...
public:
	...
	std::size_t index() const {
		...
		auto deduced = ((raw_ >> kAlignBits) * kSizeInverse) & (kIndexMask >> kAlignBits);
		return encoded | deduced;
	}

	bool operator<(PackedChunkItemPtr const& rhs) const { return raw_ < rhs.raw_; }
	bool operator==(PackedChunkItemPtr const& rhs) const { return raw_ == rhs.raw_; }
	bool operator!=(PackedChunkItemPtr const& rhs) const { return !(*this == rhs); }
...
public:
	...
	using ItemConstPtr = typename std::pointer_traits<ChunkPtr>::template rebind<Item const>;
	...
	explicit F14ItemIter(Packed const& packed): itemPtr_{packed.ptr()}, index_{packed.index()} {}
	...
	F14ItemIter(ChunkPtr chunk, std::size_t index):
		...
		assume(std::pointer_traits<ItemPtr>::pointer_to(chunk->item(index)) != nullptr);
	...
	ChunkPtr chunk() const {
		return std::pointer_traits<ChunkPtr>::pointer_to(Chunk::owner(*itemPtr_, index_));
	}
...
public:
	...
	static constexpr uint8_t kMaxSupportedChunkShift = std::numeric_limits<uint64_t>::digits - 1;

	static constexpr std::size_t kMaxSize =
	        (size_t(1) << (std::numeric_limits<std::size_t>::digits - kSizeShift)) - 1;

	uint64_t size() const noexcept { return packedSizeAndChunkShift_ >> kSizeShift; }

	uint8_t chunkShift() const noexcept { return packedSizeAndChunkShift_ & kChunkCountShiftMask; }
	...
	void setSize(std::size_t sz) noexcept {
		packedSizeAndChunkShift_ = chunkShift() | (static_cast<uint64_t>(sz) << kSizeShift);
	}

	void setChunkCount(std::size_t newCount) {
		FOLLY_SAFE_DCHECK(folly::isPowTwo(newCount), newCount);  // note that this forbids 0!
		const auto shift = findFirstSet(newCount) - 1;           // firstSet is 1-based.
		packedSizeAndChunkShift_ = (static_cast<uint64_t>(size()) << kSizeShift) | shift;
...
public:
	...
	static constexpr uint8_t kMaxSupportedChunkShift = std::numeric_limits<std::size_t>::digits - 1;

	static constexpr std::size_t kMaxSize = std::numeric_limits<std::size_t>::max();
...
using SizeAndChunkShift = std::conditional_t<sizeof(uint64_t) == sizeof(std::size_t),
...
public:
	...
	void setChunkCount(std::size_t count) { sizeAndChunkShift_.setChunkCount(count); }
...
public:
	...
	[[noreturn]] typename ItemIter::Packed& packedBegin() { assume_unreachable(); }

	[[noreturn]] typename ItemIter::Packed const& packedBegin() const { assume_unreachable(); }
};
...
private:
	...
	using ChunkPtr = typename std::pointer_traits<BytePtr>::template rebind<Chunk>;
...
private:
	...
	auto chunkShift() const { return sizeAndChunkShiftAndPackedBegin_.chunkShift(); }

	auto chunkCount() const { return sizeAndChunkShiftAndPackedBegin_.chunkCount(); }
	...
	void swapContents(F14Table& rhs) noexcept {
		...
		swap(sizeAndChunkShiftAndPackedBegin_, rhs.sizeAndChunkShiftAndPackedBegin_);
	}
...
public:
	...
	F14Table(std::size_t initialCapacity,
	         Hasher const& hasher,
	...
	F14Table(F14Table&& rhs) noexcept(std::is_nothrow_move_constructible<Hasher>::value &&
	...
	F14Table& operator=(F14Table&& rhs) noexcept(
	        ...
	        (kAllocIsAlwaysEqual || (AllocTraits::propagate_on_container_move_assignment::value &&
	...
	void swap(F14Table& rhs) noexcept(kSwapIsNoexcept) {
		...
		FOLLY_SAFE_CHECK(AllocTraits::propagate_on_container_swap::value || kAllocIsAlwaysEqual ||
...
private:
	...
	std::pair<std::size_t, std::size_t> computeChunkCountAndScale(
		...
		if(desiredCapacity <= Chunk::kCapacity) {
			...
			std::size_t minChunks = (desiredCapacity - 1) / Chunk::kDesiredCapacity + 1;
	...
	static std::size_t chunkAllocSize(std::size_t chunkCount, std::size_t capacityScale) {
	...
	ChunkPtr initializeChunks(BytePtr raw, std::size_t chunkCount, std::size_t capacityScale) {
	...
	std::size_t itemCount() const noexcept {
		if(chunkShift() == 0) {
			return computeCapacity(1, Chunk::capacityScale(access::to_address(chunks_)));
		} else {
...
public:
	...
	std::size_t size() const noexcept { return to_narrow(sizeAndChunkShiftAndPackedBegin_.size()); }
	...
	std::size_t bucket_count() const noexcept {
		return computeCapacity(chunkCount(), Chunk::capacityScale(access::to_address(chunks_)));
	}

	std::size_t max_bucket_count() const noexcept { return max_size(); }

	float load_factor() const noexcept {
		return empty() ? 0.0f : static_cast<float>(size()) / static_cast<float>(bucket_count());
	}
...
private:
	...
	template<typename K>
	FOLLY_ALWAYS_INLINE ItemIter findImpl(HashPair hp, K const& key, Prefetch prefetch) const {
	...
	HashPair computeHash(F14HashedKey<HKKey, HKHasher, HKEqual> const& hashedKey) const {
...
public:
	...
	template<typename K>
	FOLLY_ALWAYS_INLINE ItemIter find(F14HashToken const& token, K const& key) const {
	...
	FOLLY_ALWAYS_INLINE ItemIter findMatching(K const& key, F&& func) const {
		...
		for(std::size_t tries = chunkCount(); tries > 0; --tries) {
			...
			if(nonzero) {
				do {
					auto i = hits.next();
					if(FOLLY_LIKELY(func(this->keyForValue(this->valueAtItem(chunk->item(i)))))) {
						return ItemIter{chunk, i};
					}
				} while(hits.hasNext());
			}
...
private:
	...
	[[FOLLY_ATTR_GNU_COLD]] FOLLY_NOINLINE static void eraseBlankCold(F14Table* self,
	                                                                  ItemIter iter,
	                                                                  HashPair hp) {
	...
	void directBuildFrom(T&& src) {
		...
		auto undoState = this->beforeBuild(src.size(), bucket_count(), std::forward<T>(src));
		bool success = false;
		SCOPE_EXIT {
			this->afterBuild(undoState, success, src.size(), bucket_count(), std::forward<T>(src));
		};
		   ...
		   itemCount() == src.itemCount()) {
			...
			if constexpr(kEnableItemIteration) {
				...
				sizeAndChunkShiftAndPackedBegin_.packedBegin() =
				        ItemIter{chunks_ + (srcBegin.chunk() - src.chunks_), srcBegin.index()}
				                .pack();
			}
			...
			do {
				...
				for(; iter.hasNext(); ++dstI) {
					auto srcI = iter.next();
					auto&& srcArg = std::forward<T>(src).buildArgForItem(srcChunk->item(srcI));
					auto dst = dstChunk->itemAddr(dstI);
					this->constructValueAtItem(0, dst, std::forward<decltype(srcArg)>(srcArg));
		...
		SCOPE_EXIT {
			if(cc > stackBuf.size()) {
				...
				std::allocator_traits<ByteAlloc>::deallocate(
				        a,
				        std::pointer_traits<typename std::allocator_traits<ByteAlloc>::pointer>::
				                pointer_to(*fullness),
				        cc);
			}
		};
		...
		while(true) {
			...
			if(srcChunk->hostedOverflowCount() == 0) {
				...
				while(iter.hasNext()) {
					...
					insertAtBlank(allocateTag(fullness, hp),
					              hp,
	...
	[[FOLLY_ATTR_GNU_COLD]] FOLLY_NOINLINE static void buildFromF14TableCatchCold(F14Table* self) {
		self->reset();
	...
	FOLLY_NOINLINE void buildFromF14Table(T&& src) {
		...
		auto ccas = std::make_pair(src.chunkCount(),
	...
	void maybeRehash(std::size_t desiredCapacity, bool attemptExact) {
		...
		std::tie(newChunkCount, newCapacityScale) =
		        computeChunkCountAndScale(desiredCapacity,
		                                  attemptExact,
		                                  kContinuousCapacity && attemptExact);
		auto newCapacity = computeCapacity(newChunkCount, newCapacityScale);

		if(origCapacity != newCapacity) {
			rehashImpl(size(), origChunkCount, origCapacityScale, newChunkCount, newCapacityScale);
		}
	}

	void reserveImpl(std::size_t requestedCapacity) {
		const size_t targetCapacity = std::max<std::size_t>(requestedCapacity, size());
		if(targetCapacity == 0) {
			reset();
			return;
		}
		...
		bool attemptExact = targetCapacity > origCapacity + ((origCapacity + 7) / 8);
	...
	FOLLY_NOINLINE void reserveForInsertImpl(std::size_t capacityMinusOne,
		...
		std::size_t minGrowth =
		        origCapacity + (origCapacity >> 2) + (origCapacity >> 3) + (origCapacity >> 5);
		...
		FOLLY_SAFE_DCHECK(computeCapacity(newChunkCount, newCapacityScale) > origCapacity, "");
	...
	void initialReserve(std::size_t desiredCapacity) {
		...
		auto undoState = this->beforeRehash(0, 0, newCapacity, newAllocSize, rawAllocation);
		...
		this->afterRehash(std::move(undoState), true, 0, 0, newCapacity, nullptr, 0);
	}

	void rehashImpl(std::size_t origSize,
	                std::size_t origChunkCount,
	                std::size_t origCapacityScale,
	                std::size_t newChunkCount,
	                std::size_t newCapacityScale) {
		...
		BytePtr rawAllocation;
		auto undoState = this->beforeRehash(origSize,
		                                    origCapacity,
		                                    newCapacity,
		                                    newAllocSize,
		                                    rawAllocation);
		...
		SCOPE_EXIT {
			...
			this->afterRehash(std::move(undoState),
			                  success,
			                  origSize,
			                  origCapacity,
			                  newCapacity,
		...
		if(origSize == 0) {
			...
			while(dstI < origSize) {
				if(FOLLY_LIKELY(srcChunk->occupied(srcI))) {
					dstChunk->setTag(dstI, srcChunk->tag(srcI));
					this->moveItemDuringRehash(dstChunk->itemAddr(dstI), srcChunk->item(srcI));
					++dstI;
				}
				++srcI;
			}
			...
			if(newChunkCount <= stackBuf.size()) {
				...
				ByteAlloc a{this->alloc()};
				// may throw
				fullness = &*std::allocator_traits<ByteAlloc>::allocate(a, newChunkCount);
			}
			...
			while(remaining > 0) {
				...
				while(iter.hasNext()) {
					...
					auto hp = splitHash(this->computeItemHash(const_cast<Item const&>(srcItem)));
	...
	FOLLY_ALWAYS_INLINE void debugModeBeforeInsert() {
		...
		if constexpr(kIsLibrarySanitizeAddress) {
			if(!tlsPendingSafeInserts() && size() > 0 && tlsMinstdRand(size()) == 0) {
	...
	FOLLY_ALWAYS_INLINE void debugModePerturbSlotInsertOrder(ChunkPtr chunk,
...
public:
	...
	std::pair<ItemIter, bool> tryEmplaceValueWithToken(F14HashToken const& token,
	                                                   ...
	                                                   Args&&... args) {
		FOLLY_SAFE_DCHECK(computeHash(key) == static_cast<HashPair>(token), "");
		return tryEmplaceValueImpl(static_cast<HashPair>(token), key, std::forward<Args>(args)...);
	}

	template<typename K, typename... Args>
	std::pair<ItemIter, bool> tryEmplaceValueImpl(HashPair hp, K const& key, Args&&... args) {
		if(size() > 0) {
...
private:
	...
	void clearImpl() noexcept {
		...
		if(willReset) {
			...
			std::size_t rawSize =
			        chunkAllocSize(chunkCount(), Chunk::capacityScale(access::to_address(chunks_)));
...
public:
	...
	std::size_t getAllocatedMemorySize() const {
		std::size_t sum = 0;
		visitAllocationClasses([&sum](std::size_t bytes, std::size_t n) { sum += bytes * n; });
		return sum;
	}
	...
	void visitAllocationClasses(V&& visitor) const {
		auto scale = Chunk::capacityScale(access::to_address(chunks_));
		this->visitPolicyAllocationClasses(scale == 0 ? 0 : chunkAllocSize(chunkCount(), scale),
		                                   size(),
		                                   bucket_count(),
		                                   visitor);
	}
	...
	void visitContiguousItemRanges(V&& visitor) const {
		...
		for(std::size_t i = 0; i <= maxChunkIndex; ++i, ++chunk) {
			for(auto iter = chunk->occupiedRangeIter(); iter.hasNext();) {
				auto be = iter.next();
				FOLLY_SAFE_DCHECK(chunk->occupied(be.first) && chunk->occupied(be.second - 1), "");
...
private:
	static std::size_t& histoAt(std::vector<std::size_t>& histo, std::size_t index) {
		if(histo.size() <= index) {
...
public:
	...
	F14TableStats computeStats() const {
		...
		FOLLY_SAFE_DCHECK(
		        (Chunk::isEmptyInstance(access::to_address(chunks_))) == (bucket_count() == 0),
		        "");
		...
		for(std::size_t ci = 0; ci < cc; ++ci) {
			...
			histoAt(stats.chunkOutboundOverflowHisto, chunk->outboundOverflowCount())++;
...
inline void disableInsertOrderRandomization() {
	if constexpr(kIsLibrarySanitizeAddress || kIsDebug) {
		detail::tlsPendingSafeInserts(
		        static_cast<std::ptrdiff_t>((std::numeric_limits<std::size_t>::max)() / 2));
	}
}
}  // namespace test
}  // namespace f14
