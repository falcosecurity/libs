...
template<typename F, typename... Args>
constexpr bool CallableNoexcept = noexcept(FOLLY_DECLVAL(F&&)(FOLLY_DECLVAL(Args&&)...));

template<typename From,
         typename To,
         typename = typename std::enable_if<!std::is_reference<To>::value ||
...
public:
	...
	friend bool operator==(FunctionTraitsSharedProxy const& proxy, std::nullptr_t) noexcept {
	...
	friend bool operator==(std::nullptr_t, FunctionTraitsSharedProxy const& proxy) noexcept {
		return proxy.sp_ == nullptr;
	}
	friend bool operator!=(std::nullptr_t, FunctionTraitsSharedProxy const& proxy) noexcept {
		return proxy.sp_ != nullptr;
...
template<typename Fun, bool Small, bool Nx, typename ReturnType, typename... Args>
...
struct FunctionTraits<ReturnType(Args...)> {
	...
	static constexpr Call call = call_<Fun, Small, false, ReturnType, CallArg<Args>...>;
	...
	using SharedProxy = FunctionTraitsSharedProxy<NonConstSignature, false, ReturnType, Args...>;
};
...
struct FunctionTraits<ReturnType(Args...) const> {
	...
	template<typename Fun, bool Small>
	static constexpr Call call = call_<const Fun, Small, false, ReturnType, CallArg<Args>...>;
	...
	using SharedProxy = FunctionTraitsSharedProxy<ConstSignature, false, ReturnType, Args...>;
};
...
struct FunctionTraits<ReturnType(Args...) noexcept> {
	...
	template<typename Fun, bool Small>
	static constexpr Call call = call_<Fun, Small, true, ReturnType, CallArg<Args>...>;
	...
	using SharedProxy = FunctionTraitsSharedProxy<NonConstSignature, true, ReturnType, Args...>;
};
...
struct FunctionTraits<ReturnType(Args...) const noexcept> {
	...
	template<typename Fun, bool Small>
	static constexpr Call call = call_<const Fun, Small, true, ReturnType, CallArg<Args>...>;
	...
	using SharedProxy = FunctionTraitsSharedProxy<ConstSignature, true, ReturnType, Args...>;
};
...
struct DispatchBigTrivial {
	...
	static std::size_t exec_(Op o, Data* src, Data* dst) noexcept {
		...
		case Op::NUKE:
			IsAlignLarge
			        ? operator_delete(src->big, src->bigt.size, std::align_val_t(src->bigt.align))
	...
	FOLLY_ALWAYS_INLINE static void ctor(Data& data,
	                                     void const* fun,
	                                     std::size_t size,
	                                     std::size_t align) noexcept {
		// cannot use type-specific new since type-specific new is overrideable
		// in concert with type-specific delete
		data.bigt.big = is_align_large(align) ? operator_new(size, std::align_val_t(align))
		                                      : operator_new(size);
		data.bigt.size = size;
	...
	static std::size_t exec(Op o, Data* src, Data* dst) noexcept {
		...
		case Op::MOVE:
			::new(static_cast<void*>(&dst->tiny))
			        Fun(static_cast<Fun&&>(*static_cast<Fun*>(static_cast<void*>(&src->tiny))));
			[[fallthrough]];
		case Op::NUKE:
...
using DispatchOf =
        Dispatch<InSituSize && InSituAlign && InSituNoexcept, std::is_trivially_copyable_v<Fun>>;
...
template<typename T>
constexpr bool is_instantiation_of_folly_function_v = is_instantiation_of_v<Function, T>;
...
public:
	...
	/*implicit*/ Function(ReturnType (^objCBlock)(Args... args)):
	        Function([blockCopy = (ReturnType(^)(Args...))[objCBlock copy]](Args... args) {
	...
	template<typename Fun,
	         typename =
	                 std::enable_if_t<!detail::function::is_instantiation_of_folly_function_v<Fun>>,
	...
	/* implicit */ constexpr Function(Fun fun) noexcept(
		...
		if constexpr(Dispatch::is_in_situ) {
			if constexpr(!std::is_empty<Fun>::value || !std::is_trivially_copyable_v<Fun>) {
	...
	/* implicit */ Function& operator=(ReturnType (^objCBlock)(Args... args)) {
		(*this) = [blockCopy = (ReturnType(^)(Args...))[objCBlock copy]](Args... args) {
			return blockCopy(args...);
		};
		return *this;
	}
	 ...
	 * compatible function.
	 */
	template<typename Fun, typename..., bool Nx = noexcept(Function(FOLLY_DECLVAL(Fun&&)))>
	 ...
	 */
	std::size_t heapAllocatedMemory() const noexcept { return exec(Op::HEAP, &data_, nullptr); }
...
Function<ReturnType(Args...) const> constCastFunction(
	...
	return Function<ReturnType(Args...) const>{std::move(that), detail::function::CoerceTag{}};
}
...
Function<ReturnType(Args...) const noexcept> constCastFunction(
	...
	return Function<ReturnType(Args...) const noexcept>{std::move(that),
...
template<typename P>
struct function_ctor_deduce_<std::enable_if_t<std::is_function<std::remove_pointer_t<P>>::value>,
                             P> {
...
struct function_ctor_deduce_<void_t<decltype(&F::operator())>, F> {
	using type = typename member_pointer_traits<decltype(&F::operator())>::member_type;
};
...
class FunctionRef<ReturnType(Args...)> final {
	...
	template<typename Fun, std::enable_if_t<!std::is_pointer<Fun>::value, int> = 0>
	static ReturnType call(CallArg<Args>... args, void* object) {
		using Pointer = std::add_pointer_t<Fun>;
		return static_cast<ReturnType>(invoke(static_cast<Fun&&>(*static_cast<Pointer>(object)),
...
public:
	 ...
	 */
	template<typename Fun,
	         std::enable_if_t<Conjunction<Negation<std::is_same<FunctionRef, std::decay_t<Fun>>>,
