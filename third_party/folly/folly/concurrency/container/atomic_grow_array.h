...
private:
	static constexpr bool is_nothrow_grow_v = noexcept(FOLLY_DECLVAL(Policy const&).grow(0, 0)) &&
	...
	private:
		...
		down const& as_down() const noexcept { return static_cast<down const&>(*this); }
	...
	public:
		...
		down& operator+=(difference_type const n) noexcept { return index_ += n, as_down(); }
		down operator+(difference_type const n) noexcept { return down{as_down()} += n; }
		down& operator-=(difference_type const n) noexcept { return index_ -= n, as_down(); }
		down operator-(difference_type const n) noexcept { return down{as_down()} -= n; }
	...
	private:
		...
		template<bool ThatC, bool ThisC = Const, typename = std::enable_if_t<!ThatC && ThisC>>
	...
	public:
		...
		reference operator[](size_type index) noexcept { return *array_->list[index]; }
		const_reference operator[](size_type index) const noexcept { return *array_->list[index]; }
...
public:
	...
	Policy const& policy() const noexcept { return static_cast<Policy const&>(*this); }
	...
	const_pointer_span as_ptr_span() const noexcept { return as_view().as_ptr_span(); }
	pointer_span as_ptr_span(size_type const sz) noexcept { return as_view().as_ptr_span(sz); }
...
private:
	...
	static value_type* array_slab(array* const curr) {
		return reinterpret_cast<value_type*>(
		        folly::constexpr_ceil(reinterpret_cast<uintptr_t>(&curr->list[curr->size]),
	...
	array* new_array(size_type const size, array*& next) {
		...
		auto rollback = folly::makeGuard(std::bind(&atomic_grow_array::del_array, this, curr));
		curr->size = size;
		curr->next = next;
	...
	void del_array(array* const curr) {
		...
		operator_delete(static_cast<void*>(curr),
