...
class alignas(alignof(ValueType)) AlignedStorage {
	...
	using DummyAllocator = typename std::allocator_traits<Allocator>::template rebind_alloc<Dummy>;
...
protected:
	static Class* allocate(size_t count) {
		...
		auto* buf = DummyAllocTraits::allocate(allocator, calculateNeededDummyCount(count));
		return reinterpret_cast<Class*>(buf);
	}

	void deallocate(size_t count) {
		auto allocator = DummyAllocator();
		DummyAllocTraits::deallocate(allocator,
...
public:
	...
	ValueHolder(std::piecewise_construct_t, Arg&& k, Args&&... args) {
		...
		new(item_) CountedItem(std::piecewise_construct,
		                       std::forward<Arg>(k),
...
class NodeT : public hazptr_obj_base_linked<NodeT<KeyType, ValueType, Allocator, Atom>,
                                            Atom,
                                            concurrenthashmap::HazptrDeleter<Allocator>> {
public:
	using value_type = std::pair<const KeyType, ValueType>;

	explicit NodeT(hazptr_obj_cohort<Atom>* cohort, NodeT* other): item_(other->item_) {
		init(cohort);
	}
...
public:
	...
	using Node = concurrenthashmap::bucket::NodeT<KeyType, ValueType, Allocator, Atom>;
	using InsertType = concurrenthashmap::InsertType;
	class Iterator;

	BucketTable(size_t initial_buckets,
	            float load_factor,
	            size_t max_size,
		...
		DCHECK(max_size_ == 0 ||
		       (isPowTwo(max_size_) && (folly::popcount(max_size_ - 1) + ShardBits <= 32)));
		auto buckets = Buckets::create(initial_buckets, cohort);
		buckets_.store(buckets, std::memory_order_release);
		load_factor_nodes_ = to_integral(static_cast<float>(initial_buckets) * load_factor_);
	...
	bool insert(Iterator& it,
	            ...
	            hazptr_obj_cohort<Atom>* cohort,
	            Args&&... args) {
		return doInsert(it, h, k, type, match, nullptr, cohort, std::forward<Args>(args)...);
	}
	...
	void rehash(size_t bucket_count, hazptr_obj_cohort<Atom>* cohort) {
		...
		load_factor_nodes_ = to_integral(static_cast<float>(bucket_count) * load_factor_);
		for(size_t i = 0; i < oldcount; i++) {
			...
			for(; node != lastrun; node = node->next_.load(std::memory_order_relaxed)) {
			...
			while(node) {
				if(KeyEqual()(key, node->getItem().first)) {
					...
					if(iter) {
						...
						iter->setNode(node->next_.load(std::memory_order_acquire),
						              buckets,
						              bcount,
						              idx);
						iter->next();
	...
	void max_load_factor(float factor) {
		std::lock_guard g(m_);
		load_factor_ = factor;
		load_factor_nodes_ = bucket_count_.load(std::memory_order_relaxed) * load_factor_;
	}
...
private:
	...
	class Buckets : public hazptr_obj_base<Buckets, Atom, concurrenthashmap::HazptrTableDeleter>,
...
public:
	class Iterator {
	public:
		FOLLY_ALWAYS_INLINE Iterator(): hazptrs_(make_hazard_pointer_array<3, Atom>()) {}
...
private:
	...
	bool doInsert(Iterator& it,
		...
		if(size() >= load_factor_nodes_ &&
		   (type == InsertType::DOES_NOT_EXIST || type == InsertType::MATCH_OR_DOES_NOT_EXIST)) {
		...
		while(node) {
			...
			if(KeyEqual()(k, node->getItem().first)) {
				...
				if(type == InsertType::MATCH || type == InsertType::MATCH_OR_DOES_NOT_EXIST) {
					if(!match(node->getItem().second)) {
						return false;
					}
					matched = true;
				...
				if(type == InsertType::DOES_NOT_EXIST ||
				   (type == InsertType::MATCH_OR_DOES_NOT_EXIST && !matched)) {
					...
					if(!cur) {
						cur = AllocNodeGuard<Node, Allocator>::make(Allocator(),
						                                            cohort,
						                                            std::forward<Args>(args)...);
					}
					auto next = node->next_.load(std::memory_order_relaxed);
		...
		if(type != InsertType::DOES_NOT_EXIST &&
		   (type != InsertType::MATCH_OR_DOES_NOT_EXIST || matched) && type != InsertType::ANY) {
...
#if(FOLLY_SSE_PREREQ(4, 2) || (FOLLY_AARCH64 && FOLLY_F14_CRC_INTRINSIC_AVAILABLE)) && \
...
template<typename KeyType,
         ...
         typename Allocator,
         template<typename> class Atom = std::atomic>
class NodeT : public hazptr_obj_base<NodeT<KeyType, ValueType, Allocator, Atom>,
                                     Atom,
...
public:
	using Node = concurrenthashmap::simd::NodeT<KeyType, ValueType, Allocator, Atom>;
...
private:
	...
	public:
		...
		void setNodeAndTag(std::size_t index, Node* node, std::size_t tag) {
			FOLLY_SAFE_DCHECK(index < kCapacity && (tag == 0x0 || (tag >= 0x80 && tag <= 0xff)),
			                  "");
			item(index).store(node, std::memory_order_release);
			setTag(index, tag);
		}

		void clearNodeAndTag(std::size_t index) { setNodeAndTag(index, nullptr, 0); }
		...
		MaskType occupiedMask() const {
			...
			auto occupiedV = vreinterpretq_u8_s8(vshrq_n_s8(vreinterpretq_s8_u64(vec), 7));
		...
		FirstEmptyInMask firstEmpty() const { return FirstEmptyInMask{occupiedMask() ^ kFullMask}; }
		...
		static constexpr uint64_t kOutboundOverflowOperand = 0x1ULL << kOutboundOverflowIndex;
		...
		void incrOutboundOverflowCount() {
			...
			if(count < kSaturatedOutboundOverflowCount) {
				tags_hi_.store(count + kOutboundOverflowOperand, std::memory_order_relaxed);
			}
		}
		...
		static constexpr uint64_t kHostedOverflowOperand = 0x10ULL << kHostedOverflowIndex;
		...
		void reclaim_nodes(size_t count) {
			for(size_t i = 0; i < count; i++) {
				...
				while(occupied.hasNext()) {
					...
					chunk.setTag(idx, 0);
					Node* node = chunk.item(idx).exchange(nullptr, std::memory_order_relaxed);
...
public:
	static constexpr float kDefaultLoadFactor = Chunk::kDesiredCapacity / (float)Chunk::kCapacity;
	...
	class Iterator {
	public:
		FOLLY_ALWAYS_INLINE Iterator(): hazptrs_(make_hazard_pointer_array<2, Atom>()) {}
		FOLLY_ALWAYS_INLINE explicit Iterator(std::nullptr_t): hazptrs_() {}
		FOLLY_ALWAYS_INLINE ~Iterator() {}
	...
	bool insert(Iterator& it,
		...
		auto cur = AllocNodeGuard<Node, Allocator>::make(Allocator(),
		                                                 cohort,
		                                                 std::forward<Args>(args)...);
...
private:
	...
	template<typename MatchFunc, typename K, typename... Args>
	bool prepare_insert(Iterator& it,
	                    const K& k,
	                    InsertType type,
	                    MatchFunc match,
	                    hazptr_obj_cohort<Atom>* cohort,
		...
		if(node) {
			...
			if(type != InsertType::DOES_NOT_EXIST && type != InsertType::MATCH_OR_DOES_NOT_EXIST &&
			   type != InsertType::ANY) {
	...
	void rehash_internal(size_t new_chunk_count, hazptr_obj_cohort<Atom>* cohort) {
		...
		grow_threshold_ = to_integral(new_chunk_count * Chunk::kCapacity * load_factor_);
	...
	void getChunksAndCount(size_t& ccount, Chunks*& chunks, hazptr_holder<Atom>& hazptr) {
		while(true) {
			auto seqlock = seqlock_.load(std::memory_order_acquire);
	...
	std::pair<size_t, size_t> findEmptyInsertLocation(Chunks* chunks,
	                                                  size_t ccount,
	                                                  const HashPair& hp) {
...
class alignas(64) ConcurrentHashMapSegment {
	using ImplT = Impl<KeyType, ValueType, ShardBits, HashFn, KeyEqual, Allocator, Atom, Mutex>;
...
public:
	...
	ConcurrentHashMapSegment(size_t initial_buckets,
	                         float load_factor,
	                         size_t max_size,
	                         hazptr_obj_cohort<Atom>* cohort):
	        impl_(initial_buckets, load_factor, max_size, cohort),
	        cohort_(cohort) {
		DCHECK(cohort);
	}
	...
	bool insert(Iterator& it, size_t h, Key&& k, Value&& v) {
		concurrenthashmap::AllocNodeGuard<Node, Allocator> g(Allocator(),
		                                                     cohort_,
		                                                     std::forward<Key>(k),
		                                                     std::forward<Value>(v));
	...
	bool insert_or_assign_if(Iterator& it,
	                         size_t h,
	                         Key&& k,
	                         Value&& desired,
	                         Predicate&& predicate) {
	...
	template<typename Key, typename Value, typename Predicate>
	bool assign_if(Iterator& it, size_t h, Key&& k, Value&& desired, Predicate&& predicate) {
		concurrenthashmap::AllocNodeGuard<Node, Allocator> g(Allocator(),
		                                                     cohort_,
		                                                     std::forward<Key>(k),
		                                                     std::forward<Value>(desired));
	...
	template<typename MatchFunc, typename K, typename... Args>
	bool insert_internal(Iterator& it,
	                     size_t h,
	                     const K& k,
	                     InsertType type,
	                     MatchFunc match,
	                     Args&&... args) {
		return impl_.insert(it, h, k, type, match, cohort_, std::forward<Args>(args)...);
	}
	...
	// Must hold lock.
	void rehash(size_t bucket_count) { impl_.rehash(folly::nextPowTwo(bucket_count), cohort_); }
	...
	template<typename K, typename MatchFunc>
	size_type erase_internal(size_t h, const K& key, Iterator* iter, MatchFunc match) {
