...
CacheLocality CacheLocality::readFromSysfsTree(std::string_view root) {
	...
	auto checkNoEnt = [](std::string_view name) {
		...
		if(err != ENOENT) {
			throw std::runtime_error(
			        fmt::format("unexpected error while opening {}: {}", name, strerror(err)));
		}
	};
	...
	if(allfd < 0) {
		auto err = errno;
		throw std::runtime_error(fmt::format("unable to open sysfs: {}", strerror(err)));
	}
	SCOPE_EXIT {
		::close(allfd);
	};
	...
	for(size_t cpu = 0;; ++cpu) {
		...
		const int cpufd = ::openat(allfd, cpuroot.c_str(), O_DIRECTORY | O_CLOEXEC, O_RDONLY);
		if(cpufd < 0) {
			checkNoEnt(cpuroot);
			break;
		}
...
CacheLocality CacheLocality::readFromProcCpuinfoLines(std::vector<std::string> const& lines) {
	// (physicalId, coreId, cpu)
	std::vector<std::tuple<size_t, size_t, size_t>> cpus = parseProcCpuinfoLines(lines);
	...
	for(size_t i = 0; i < cpus.size(); ++i) {
		...
		// in the class.
		if(i == 0 || physicalId != std::get<0>(cpus[i - 1]) || coreId != std::get<1>(cpus[i - 1])) {
			l1Equiv = cpu;
		}
...
bool AccessSpreaderBase::initialize(GlobalState& state,
	...
	for(size_t width = 0; width <= kMaxCpus; ++width) {
		...
		for(size_t cpu = 0; cpu < kMaxCpus && cpu < n; ++cpu) {
			...
			// 0..numStripes
			make_atomic_ref(row[cpu]).store(static_cast<CompactStripe>((index * numStripes) / n),
			                                relaxed);
...
public:
	...
	SimpleAllocator(Ctor, size_t sz): sz_(sz) {
		...
		if(sz_ < sizeof(void*)) {
			folly::throw_exception<std::invalid_argument>(
			        fmt::format("SimpleAllocator size {} is too small (minimum: {})",
			                    sz_,
			                    sizeof(void*)));
		}
	}
	...
	static void deallocate(void* ptr) {
		...
		auto addr = reinterpret_cast<void*>(intptr_t(ptr) & ~intptr_t(kAllocSize - 1));
		...
		*static_cast<void**>(ptr) = allocator->freelist_;
		if constexpr(kIsSanitizeAddress) {
			...
			std::fill(base + sizeof(void*), base + allocator->sz_, static_cast<char>(0));
		}
...
void* coreMalloc(size_t size, size_t numStripes, size_t stripe) {
	static folly::Indestructible<Allocator> allocators[AccessSpreader<>::maxLocalityIndexValue()];
...
void* coreMallocFromGuard(size_t size) {
	...
	return coreMalloc(size, gCoreAllocatorGuard->numStripes_, gCoreAllocatorGuard->stripe_);
}
