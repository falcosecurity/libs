...
struct CacheLocality {
	...
	static CacheLocality readFromProcCpuinfoLines(std::vector<std::string> const& lines);
...
using FallbackGetcpuType =
        FallbackGetcpu<conditional_t<kIsMobile, HashingThreadId, SequentialThreadId>>;
...
protected:
	...
	static bool initialize(GlobalState& out, Getcpu::Func (&)(), const CacheLocality& (&)());
};
...
public:
	...
	/// current()).
	static size_t cachedCurrent(size_t numStripes, const GlobalState& s = state()) {
		if(kIsMobile) {
	...
	static size_t localityIndexForStripe(size_t numStripes, size_t stripe) {
		...
		return stripe * std::min(size_t(kMaxCpus), CacheLocality::system<Atom>().numCpus) /
		       numStripes;
	}
...
private:
	...
	static bool initialize(GlobalState& state) {
		return detail::AccessSpreaderBase::initialize(state,
		                                              pickGetcpuFunc,
...
public:
	...
	friend bool operator==(const CoreAllocator&, const CoreAllocator&) noexcept { return true; }
	friend bool operator!=(const CoreAllocator&, const CoreAllocator&) noexcept { return false; }
