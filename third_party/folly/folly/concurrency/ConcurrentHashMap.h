...
class ConcurrentHashMap {
	using SegmentT = detail::ConcurrentHashMapSegment<KeyType,
	                                                  ValueType,
	                                                  ShardBits,
	                                                  HashFn,
	                                                  KeyEqual,
	                                                  ...
	                                                  Mutex,
	                                                  Impl>;
	using SegmentTAllocator =
	        typename std::allocator_traits<Allocator>::template rebind_alloc<SegmentT>;
...
private:
	...
	using EnableHeterogeneousInsert = std::enable_if_t<
	        ::folly::detail::EligibleForHeterogeneousInsert<KeyType, HashFn, KeyEqual, K>::value,
	        T>;
...
public:
	...
	ConcurrentHashMap(ConcurrentHashMap&& o) noexcept: size_(o.size_), max_size_(o.max_size_) {
		for(uint64_t i = 0; i < NumShards; i++) {
			segments_[i].store(o.segments_[i].load(std::memory_order_relaxed),
		...
		beginSeg_.store(o.beginSeg_.load(std::memory_order_relaxed), std::memory_order_relaxed);
		o.beginSeg_.store(NumShards, std::memory_order_relaxed);
		endSeg_.store(o.endSeg_.load(std::memory_order_relaxed), std::memory_order_relaxed);
	...
	std::pair<ConstIterator, bool> insert(Key&& k, Value&& v) {
		...
		std::pair<ConstIterator, bool> res(std::piecewise_construct,
		                                   std::forward_as_tuple(this, segment),
		                                   std::forward_as_tuple(false));
		res.second = ensureSegment(segment)->insert(res.first.it_,
		                                            h,
		                                            std::forward<Key>(k),
		                                            std::forward<Value>(v));
		return res;
	...
	std::pair<ConstIterator, bool> try_emplace(Key&& k, Args&&... args) {
		...
		std::pair<ConstIterator, bool> res(std::piecewise_construct,
		                                   std::forward_as_tuple(this, segment),
		                                   std::forward_as_tuple(false));
		res.second = ensureSegment(segment)->try_emplace(res.first.it_,
		                                                 h,
		                                                 std::forward<Key>(k),
	...
	std::pair<ConstIterator, bool> emplace(Args&&... args) {
		...
		detail::concurrenthashmap::AllocNodeGuard<Node, Allocator> g(Allocator(),
		                                                             ensureCohort(),
		                                                             std::forward<Args>(args)...);
		...
		std::pair<ConstIterator, bool> res(std::piecewise_construct,
		                                   ...
		                                   std::forward_as_tuple(false));
		res.second =
		        ensureSegment(segment)->emplace(res.first.it_, h, g.node->getItem().first, g.node);
		if(res.second) {
			g.dismiss();
		}
		return res;
	...
	std::pair<ConstIterator, bool> insert_or_assign(Key&& k, Value&& v) {
		...
		std::pair<ConstIterator, bool> res(std::piecewise_construct,
		                                   std::forward_as_tuple(this, segment),
		                                   std::forward_as_tuple(false));
		res.second = ensureSegment(segment)->insert_or_assign(res.first.it_,
		                                                      h,
		                                                      std::forward<Key>(k),
		                                                      std::forward<Value>(v));
		return res;
	...
	std::pair<ConstIterator, bool> insert_or_assign_if(Key&& k,
	                                                   Value&& desired,
	                                                   Predicate&& predicate) {
		auto h = HashFn{}(k);
		auto segment = pickSegment(h);
		std::pair<ConstIterator, bool> res(std::piecewise_construct,
		                                   ...
		                                   std::forward_as_tuple(false));
		res.second =
		        ensureSegment(segment)->insert_or_assign_if(res.first.it_,
		                                                    h,
		                                                    std::forward<Key>(k),
	...
	folly::Optional<ConstIterator> assign(Key&& k, Value&& v) {
		...
		if(!seg) {
			...
			auto r = seg->assign(res.it_, h, std::forward<Key>(k), std::forward<Value>(v));
			if(!r) {
				return none;
			}
		}
	...
	template<typename Key, typename Value, typename Predicate>
	folly::Optional<ConstIterator> assign_if(Key&& k, Value&& desired, Predicate&& predicate) {
		auto h = HashFn{}(k);
		auto segment = pickSegment(h);
		ConstIterator res(this, segment);
		auto seg = segments_[segment].load(std::memory_order_acquire);
	...
	template<typename Key, typename Value>
	folly::Optional<ConstIterator> assign_if_equal(Key&& k,
		...
		if(!seg) {
			...
			auto r = seg->assign_if_equal(res.it_,
			                              h,
			                              std::forward<Key>(k),
			                              expected,
			                              std::forward<Value>(desired));
	...
	template<typename K, typename Predicate, EnableHeterogeneousErase<K, int> = 0>
	...
	private:
		...
		void advanceIfAtSegmentEnd() {
			...
			SegmentT* seg = parent_->segments_[segment_].load(std::memory_order_acquire);
			DCHECK(seg);
			if(it_ == seg->cend()) {
				++segment_;
				advanceToSegmentBegin();
...
private:
	...
	std::pair<ConstIterator, bool> insertImpl(std::pair<Key, mapped_type>&& foo) {
		...
		std::pair<ConstIterator, bool> res(std::piecewise_construct,
		                                   std::forward_as_tuple(this, segment),
		                                   std::forward_as_tuple(false));
		res.second = ensureSegment(segment)->insert(res.first.it_, h, std::move(foo));
		return res;
	}
	...
	bool casSeg(Atom<uint64_t>& seg, uint64_t& expval, uint64_t newval) const {
		return seg.compare_exchange_weak(expval,
		                                 newval,
...
template<typename KeyType,
         ...
         template<typename> class Atom = std::atomic,
         class Mutex = std::mutex>
using ConcurrentHashMapSIMD = ConcurrentHashMap<KeyType,
                                                ValueType,
                                                HashFn,
                                                KeyEqual,
                                                Allocator,
