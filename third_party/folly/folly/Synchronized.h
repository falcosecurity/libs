...
inline constexpr bool
        kSynchronizedMutexIsUnique<decltype(void(std::declval<Mutex&>().lock())), Mutex> = true;

template<typename, typename Mutex>
inline constexpr bool kSynchronizedMutexIsShared = false;
template<typename Mutex>
inline constexpr bool
        kSynchronizedMutexIsShared<decltype(void(std::declval<Mutex&>().lock_shared())), Mutex> =
                true;
...
template<typename Mutex>
inline constexpr bool
        kSynchronizedMutexIsUpgrade<decltype(void(std::declval<Mutex&>().lock_upgrade())), Mutex> =
                true;
...
using SynchronizedLockPolicyExclusive =
        SynchronizedLockPolicy<SynchronizedMutexLevel::Unique, SynchronizedMutexMethod::Lock>;
using SynchronizedLockPolicyTryExclusive =
        SynchronizedLockPolicy<SynchronizedMutexLevel::Unique, SynchronizedMutexMethod::TryLock>;
using SynchronizedLockPolicyShared =
        SynchronizedLockPolicy<SynchronizedMutexLevel::Shared, SynchronizedMutexMethod::Lock>;
using SynchronizedLockPolicyTryShared =
        SynchronizedLockPolicy<SynchronizedMutexLevel::Shared, SynchronizedMutexMethod::TryLock>;
using SynchronizedLockPolicyUpgrade =
        SynchronizedLockPolicy<SynchronizedMutexLevel::Upgrade, SynchronizedMutexMethod::Lock>;
using SynchronizedLockPolicyTryUpgrade =
        SynchronizedLockPolicy<SynchronizedMutexLevel::Upgrade, SynchronizedMutexMethod::TryLock>;
...
public:
	...
	ConstWLockedPtr wlock() const { return ConstWLockedPtr(static_cast<const Subclass*>(this)); }
	 ...
	 * @methodset Exclusive lock
	 */
	TryWLockedPtr tryWLock() { return TryWLockedPtr{static_cast<Subclass*>(this)}; }
	...
	ConstLockedPtr rlock() const { return ConstLockedPtr(static_cast<const Subclass*>(this)); }
	 ...
	 * @methodset Shared lock
	 */
	TryRLockedPtr tryRLock() { return TryRLockedPtr{static_cast<Subclass*>(this)}; }
...
class SynchronizedBase<Subclass, detail::SynchronizedMutexLevel::Upgrade>
        : public SynchronizedBase<Subclass, detail::SynchronizedMutexLevel::Shared> {
private:
	template<typename T, typename P>
	using LockedPtr_ = ::folly::LockedPtr<T, P>;
...
public:
	...
	using ConstTryUpgradeLockedPtr = LockedPtr_<const Subclass, LockPolicyTryUpgrade>;
	 ...
	 */
	UpgradeLockedPtr ulock() { return UpgradeLockedPtr(static_cast<Subclass*>(this)); }
	 ...
	 * @methodset Upgrade lock
	 */
	TryUpgradeLockedPtr tryULock() { return TryUpgradeLockedPtr{static_cast<Subclass*>(this)}; }
...
public:
	 ...
	 * @methodset Non-shareable lock
	 */
	ConstLockedPtr lock() const { return ConstLockedPtr(static_cast<const Subclass*>(this)); }
...
struct Synchronized
        : public SynchronizedBase<Synchronized<T, Mutex>, detail::kSynchronizedMutexLevel<Mutex>> {
private:
...
public:
	 ...
	 * deprecated
	 */
	Synchronized(Synchronized&& rhs) noexcept(nxMoveCtor): Synchronized(std::move(rhs.datum_)) {}
	 ...
	 */
	explicit Synchronized(T&& rhs) noexcept(nxMoveCtor): datum_(std::move(rhs)) {}
	...
	Synchronized(std::piecewise_construct_t,
	 ...
	 * deprecated
	 */
	Synchronized& operator=(typename std::conditional<std::is_copy_constructible<T>::value &&
	 ...
	 * deprecated
	 */
	Synchronized& operator=(Synchronized&& rhs) { return *this = std::move(rhs.datum_); }
...
private:
	 ...
	 */
	Synchronized(const Synchronized& rhs, const ConstLockedPtr& /*guard*/) noexcept(nxCopyCtor):
	        datum_(rhs.datum_) {}

	Synchronized(Synchronized&& rhs, const LockedPtr& /*guard*/) noexcept(nxMoveCtor):
	        datum_(std::move(rhs.datum_)) {}

	template<typename... DatumArgs,
	         typename... MutexArgs,
...
public:
	 ...
	 */
	[[deprecated("use explicit lock(), wlock(), or rlock() instead")]] LockedPtr operator->() {
	 ...
	 * Prefer using lock(), wlock(), or rlock() instead.
	 */
	[[deprecated("use explicit lock(), wlock(), or rlock() instead")]] ConstLockedPtr operator->()
	        const {
...
using SynchronizedDataType =
        typename std::conditional<AllowsConcurrentAccess || std::is_const<SynchronizedType>::value,
...
template<typename Synchronized, typename LockFunc, typename TryLockFunc, typename... Args>
auto makeSynchronizedLocker(Synchronized& synchronized,
                            LockFunc&& lockFunc,
                            TryLockFunc&& tryLockFunc,
                            Args&&... args) {
	using LockFuncType = std::decay_t<LockFunc>;
	using TryLockFuncType = std::decay_t<TryLockFunc>;
	return SynchronizedLocker<Synchronized, LockFuncType, TryLockFuncType, std::decay_t<Args>...>{
	        synchronized,
...
auto wlock(Synchronized& synchronized, Args&&... args) {
	return detail::makeSynchronizedLocker(
	        synchronized,
	        [](auto& s, auto&&... a) { return s.wlock(std::forward<decltype(a)>(a)...); },
...
auto rlock(Synchronized& synchronized, Args&&... args) {
	return detail::makeSynchronizedLocker(
	        synchronized,
	        [](auto& s, auto&&... a) { return s.rlock(std::forward<decltype(a)>(a)...); },
...
auto ulock(Synchronized& synchronized, Args&&... args) {
	return detail::makeSynchronizedLocker(
	        synchronized,
	        [](auto& s, auto&&... a) { return s.ulock(std::forward<decltype(a)>(a)...); },
...
auto lock(Synchronized& synchronized, Args&&... args) {
	return detail::makeSynchronizedLocker(
	        synchronized,
	        [](auto& s, auto&&... a) { return s.lock(std::forward<decltype(a)>(a)...); },
...
public:
	...
	explicit LockedPtr(LockedPtr<SynchronizedType, LockPolicyType>&& other) noexcept:
	...
	DataType& asNonConstUnsafe() const {
		static_assert(AllowsConcurrentAccess && !std::is_const<SynchronizedType>::value,
	...
	template<
	        ...
	        decltype(void(std::declval<typename SyncType::MutexType&>().lock_upgrade()))* = nullptr>
	LockedPtr<SynchronizedType, detail::SynchronizedLockPolicyExclusive> moveFromUpgradeToWrite() {
	...
	template<
	        typename SyncType = SynchronizedType,
	        decltype(void(std::declval<typename SyncType::MutexType&>().lock_upgrade()))* = nullptr>
	LockedPtr<SynchronizedType, detail::SynchronizedLockPolicyUpgrade> moveFromWriteToUpgrade() {
		static_assert(std::is_same<SyncType, SynchronizedType>::value, "mismatch");
	...
	template<
	        typename SyncType = SynchronizedType,
	        decltype(void(std::declval<typename SyncType::MutexType&>().lock_upgrade()))* = nullptr>
	LockedPtr<SynchronizedType, detail::SynchronizedLockPolicyShared> moveFromUpgradeToRead() {
		static_assert(std::is_same<SyncType, SynchronizedType>::value, "mismatch");
	...
	template<typename SyncType = SynchronizedType,
	         decltype(void(std::declval<typename SyncType::MutexType&>().lock_shared()))* = nullptr>
	LockedPtr<SynchronizedType, detail::SynchronizedLockPolicyShared> moveFromWriteToRead() {
		static_assert(std::is_same<SyncType, SynchronizedType>::value, "mismatch");
		return transition_to_shared_lock(lock_);
	}
...
decltype(auto) synchronized(Func&& func, SynchronizedLockers&&... lockers) {
	return apply(std::forward<Func>(func), lock(std::forward<SynchronizedLockers>(lockers)...));
}
...
void lock(LockableOne& one, LockableTwo& two, Lockables&... lockables) {
	auto locker = [](auto& lockable) {
		...
		return detail::makeSynchronizedLocker(
		        ...
		        [](auto& l) { return std::unique_lock<Lockable>{l, std::try_to_lock}; });
	};
...
std::tuple<detail::LockedPtrType<Sync1>, detail::LockedPtrType<Sync2>> acquireLocked(Sync1& l1,
                                                                                     Sync2& l2) {
 ...
 */
template<class Sync1, class Sync2>
std::pair<detail::LockedPtrType<Sync1>, detail::LockedPtrType<Sync2>> acquireLockedPair(Sync1& l1,
                                                                                        Sync2& l2) {
	auto lockedPtrs = acquireLocked(l1, l2);
	return {std::move(std::get<0>(lockedPtrs)), std::move(std::get<1>(lockedPtrs))};
}
...
#define SYNCHRONIZED_CONST(...)                       \
	SYNCHRONIZED(FB_VA_GLUE(FB_ARG_1, (__VA_ARGS__)), \
...
#define SYNCHRONIZED_DUAL(n1, e1, n2, e2)                                                      \
	if(bool SYNCHRONIZED_VAR(state) = false) {                                                 \
		...
		for(auto SYNCHRONIZED_VAR(ptrs) = acquireLockedPair(e1, e2); !SYNCHRONIZED_VAR(state); \
		    SYNCHRONIZED_VAR(state) = true)                                                    \
			for(auto& n1 = *SYNCHRONIZED_VAR(ptrs).first; !SYNCHRONIZED_VAR(state);            \
			    SYNCHRONIZED_VAR(state) = true)                                                \
				for(auto& n2 = *SYNCHRONIZED_VAR(ptrs).second; !SYNCHRONIZED_VAR(state);       \
				    SYNCHRONIZED_VAR(state) = true)
