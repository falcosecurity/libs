...
[[FOLLY_ATTR_GNU_COLD]] uint32_t getMaxDeferredReadersSlow(relaxed_atomic<uint32_t>& cache);
...
class SharedMutexImpl : std::conditional_t<Policy::track_thread_id,
...
public:
	...
	~SharedMutexImpl() {
		...
		if(folly::kIsDebug) {
			...
			if((state & kMayDefer) != 0) {
				const uint32_t maxDeferredReaders = shared_mutex_detail::getMaxDeferredReaders();
	...
	void unlock_shared() {
		...
		// have deferred
		if((state & (kMayDefer | kPrevDefer)) == 0 || !tryUnlockTokenlessSharedDeferred()) {
	...
	void release_token(Token& token) {
		...
		deferredReader(slot)->compare_exchange_strong(slotValue, tokenlessSlotValue());
...
private:
	...
	struct WaitForDuration {
		...
		bool shouldTimeOut() { return std::chrono::steady_clock::now() > deadline(); }

		bool doWait(Futex& futex, uint32_t expected, uint32_t waitMask) {
			auto result = detail::futexWaitUntil(&futex, expected, deadline(), waitMask);
	...
	struct WaitUntilDeadline {
		...
		bool doWait(Futex& futex, uint32_t expected, uint32_t waitMask) {
			auto result = detail::futexWaitUntil(&futex, expected, absDeadline_, waitMask);
			return result != folly::detail::FutexResult::TIMEDOUT;
		}
	};

	void annotateLazyCreate() {
		if(AnnotateForThreadSanitizer && (state_.load() & kAnnotationCreated) == 0) {
			auto guard = shared_mutex_detail::annotationGuard(this);
			// check again
			if((state_.load() & kAnnotationCreated) == 0) {
				state_.fetch_or(kAnnotationCreated);
				annotate_benign_race_sized(&state_,
				                           sizeof(state_),
				                           "init TSAN",
				                           __FILE__,
				                           __LINE__);
	...
	static constexpr uint32_t kWaitingAny = kWaitingNotS | kWaitingE | kWaitingU | kWaitingS;
...
private:
	...
	FOLLY_EXPORT FOLLY_ALWAYS_INLINE static relaxed_atomic<uint32_t>& tls_lastTokenlessSlot() {
	...
	FOLLY_EXPORT FOLLY_ALWAYS_INLINE static relaxed_atomic<uint32_t>& tls_lastDeferredReaderSlot() {
		static relaxed_atomic<uint32_t> non_tl{};
		static thread_local relaxed_atomic<uint32_t> tl{};
		return kIsMobile ? non_tl : tl;
	}
...
private:
	...
	bool lockExclusiveImpl(uint32_t preconditionGoalMask, WaitContext& ctx) {
		...
		if(FOLLY_LIKELY((state & (preconditionGoalMask | kMayDefer | kHasS)) == 0 &&
	...
	bool lockExclusiveImpl(uint32_t& state, uint32_t preconditionGoalMask, WaitContext& ctx) {
		while(true) {
			if(FOLLY_UNLIKELY((state & preconditionGoalMask) != 0) &&
			   !waitForZeroBits(state, preconditionGoalMask, kWaitingE, ctx) && ctx.canTimeOut()) {
				return false;
			}
			...
			if(state_.compare_exchange_strong(state, after)) {
				...
				while(true) {
					...
					if(FOLLY_UNLIKELY((state & kHasS) != 0) &&
					   !waitForZeroBits(state, kHasS, kWaitingNotS, ctx) && ctx.canTimeOut()) {
					...
					if(kReaderPriority && (state & kHasE) == 0) {
						...
						if(!state_.compare_exchange_strong(state, (state & ~kBegunE) | kHasE)) {
							continue;
						}
					}
	...
	bool waitForZeroBits(uint32_t& state, uint32_t goal, uint32_t waitMask, WaitContext& ctx) {
		for(uint64_t start = hardware_timestamp();;) {
			...
			if(FOLLY_UNLIKELY(elapsed >= kMaxSpinCycles)) {
				return ctx.canBlock() && yieldWaitForZeroBits(state, goal, waitMask, ctx);
			}
			asm_volatile_pause();
		}
	}
	...
	bool yieldWaitForZeroBits(uint32_t& state, uint32_t goal, uint32_t waitMask, WaitContext& ctx) {
		...
		long before = -1;
		for(uint32_t yieldCount = 0; yieldCount < kMaxSoftYieldCount; ++yieldCount) {
			for(int softState = 0; softState < 3; ++softState) {
				if(softState < 2) {
					std::this_thread::yield();
				} else {
					thread_nivcsw =
					        shared_mutex_detail::getCurrentThreadInvoluntaryContextSwitchCount();
				}
	...
	bool futexWaitForZeroBits(uint32_t& state, uint32_t goal, uint32_t waitMask, WaitContext& ctx) {
		assert(waitMask == kWaitingNotS || waitMask == kWaitingE || waitMask == kWaitingU ||
		       waitMask == kWaitingS);
	...
	void wakeRegisteredWaitersImpl(uint32_t& state, uint32_t wakeMask) {
		...
		if((wakeMask & kWaitingE) == kWaitingE && (state & wakeMask) == kWaitingE &&
	...
	void applyDeferredReaders(uint32_t& state, WaitContext& ctx) {
		...
		const uint32_t maxDeferredReaders = shared_mutex_detail::getMaxDeferredReaders();
		for(uint64_t start = hardware_timestamp();;) {
			while(!slotValueIsThis(deferredReader(slot)->load(std::memory_order_acquire))) {
	...
	void applyDeferredReaders(uint32_t& state, WaitContext& ctx, uint32_t slot) {
		...
		for(; slot < maxDeferredReaders; ++slot) {
			...
			if(slotValueIsThis(slotValue) && slotPtr->compare_exchange_strong(slotValue, 0)) {
				++movedSlotCount;
			}
		}
	...
	uint32_t unlockSharedInline() {
		...
		assert((state & (kHasE | kBegunE | kMayDefer)) != 0 || state < state + kIncrHasS);
		if((state & kHasS) == 0) {
...
using SharedMutexTracked =
        SharedMutexImpl<false, void, std::atomic, shared_mutex_detail::PolicyTracked>;
using SharedMutexSuppressTSAN =
        SharedMutexImpl<false, void, std::atomic, shared_mutex_detail::PolicySuppressTSAN>;
...
template<bool ReaderPriority, typename Tag_, template<typename> class Atom, typename Policy>
alignas(hardware_destructive_interference_size)
        typename SharedMutexImpl<ReaderPriority, Tag_, Atom, Policy>::DeferredReaderSlot
        SharedMutexImpl<ReaderPriority, Tag_, Atom, Policy>::deferredReaders
                [shared_mutex_detail::kMaxDeferredReadersAllocated * kDeferredSeparationFactor] =
                        {};

template<bool ReaderPriority, typename Tag_, template<typename> class Atom, typename Policy>
bool SharedMutexImpl<ReaderPriority, Tag_, Atom, Policy>::tryUnlockTokenlessSharedDeferred() {
...
template<bool ReaderPriority, typename Tag_, template<typename> class Atom, typename Policy>
template<class WaitContext>
bool SharedMutexImpl<ReaderPriority, Tag_, Atom, Policy>::lockSharedImpl(uint32_t& state,
                                                                         Token* token,
                                                                         WaitContext& ctx) {
	const uint32_t maxDeferredReaders = shared_mutex_detail::getMaxDeferredReaders();
	while(true) {
		if(FOLLY_UNLIKELY((state & kHasE) != 0) && !waitForZeroBits(state, kHasE, kWaitingS, ctx) &&
		   ctx.canTimeOut()) {
			return false;
		}
		...
		bool aboveDeferThreshold = (state & kHasS) >= (kNumSharedToStartDeferring - 1) * kIncrHasS;
...
public:
	using mutex_type = ::folly::SharedMutexImpl<ReaderPriority, Tag_, Atom, Policy>;
	using token_type = typename mutex_type::Token;

	shared_lock() noexcept = default;

	[[nodiscard]] explicit shared_lock(mutex_type& mutex): mutex_(std::addressof(mutex)) { lock(); }

	shared_lock(mutex_type& mutex, std::defer_lock_t) noexcept: mutex_(std::addressof(mutex)) {}
	...
	[[nodiscard]] shared_lock(mutex_type& mutex, std::adopt_lock_t): mutex_(std::addressof(mutex)) {
		token_.state_ = token_type::State::LockedShared;
	}

	template<typename Clock, typename Duration>
	[[nodiscard]] shared_lock(mutex_type& mutex,
	...
	[[nodiscard]] shared_lock(mutex_type& mutex, const std::chrono::duration<Rep, Period>& timeout):
	        mutex_(std::addressof(mutex)) {
	...
	[[nodiscard]] bool owns_lock() const noexcept { return static_cast<bool>(token_); }
