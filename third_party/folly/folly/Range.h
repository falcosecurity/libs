...
inline size_t qfind(const Range<Iter>& haystack, const Range<Iter>& needle, Comp eq = Comp());
 ...
 */
template<class Iter>
inline size_t qfind_first_of(const Range<Iter>& haystack, const Range<Iter>& needle);
...
#if(defined(__cpp_char8_t) && __cpp_char8_t >= 201811L) || FOLLY_CPLUSPLUS >= 202002
...
using range_is_char_type_d_ = decltype(folly::detail::range_is_char_type_f_(FOLLY_DECLVAL(Iter)));
template<typename Iter>
constexpr bool range_is_char_type_v_ = is_detected_v<range_is_char_type_d_, Iter>;
...
template<typename Iter>
using range_is_byte_type_d_ = decltype(folly::detail::range_is_byte_type_f_(FOLLY_DECLVAL(Iter)));
template<typename Iter>
constexpr bool range_is_byte_type_v_ = is_detected_v<range_is_byte_type_d_, Iter>;
...
struct range_traits_byte_ {
	...
	struct apply {
		...
		FOLLY_ERASE static constexpr int compare(Value const* a, Value const* b, std::size_t c) {
...
template<typename Iter>
using range_traits_c_ = conditional_t<range_is_char_type_v_<Iter>,
                                      range_traits_char_,
                                      conditional_t<  //
...
public:
	...
	using const_range_type =
	        typename std::conditional<std::is_same<Iter, char*>::value ||
...
public:
	...
	constexpr /* implicit */ Range(Iter str): b_(str), e_(str + constexpr_strlen(str)) {
		static_assert(std::is_same<int, typename detail::IsCharPointer<Iter>::type>::value,
		              "This constructor is only available for character ranges");
	}

	template<class Alloc, class T = Iter, typename detail::IsCharPointer<T>::const_type = 0>
	/* implicit */ Range(const string<Alloc>& str): b_(str.data()), e_(b_ + str.size()) {}
	...
	template<class Container,
	         ...
	         class = typename std::enable_if<
	                 std::is_same<typename Container::const_pointer, const char*>::value>::type,
	         class = decltype(Iter(std::declval<Container const&>().data()),
	                          Iter(std::declval<Container const&>().data() +
	                               std::declval<Container const&>().size()))>
	explicit Range(const Container& str):
	        b_(reinterpret_cast<Iter>(str.data())),
	        e_(b_ + str.size()) {}
	/**
	...
	template<class OtherIter,
	         typename std::enable_if<(std::is_same<Iter, const unsigned char*>::value &&
	...
	template<class OtherIter,
	         typename std::enable_if<(std::is_same<Iter, unsigned char*>::value &&
	                                  std::is_same<OtherIter, char*>::value),
	                                 int>::type = 0>
	/* implicit */ Range(const Range<OtherIter>& other):
	...
	template<class OtherIter,
	         typename std::enable_if<(std::is_same<Iter, const char*>::value &&
	...
	template<class OtherIter,
	         typename std::enable_if<(std::is_same<Iter, char*>::value &&
	                                  std::is_same<OtherIter, unsigned char*>::value),
	                                 int>::type = 0>
	explicit Range(const Range<OtherIter>& other):
	...
	template<class OtherIter,
	         typename std::enable_if<(!std::is_same<Iter, OtherIter>::value &&
	                                 ...
	                                 int>::type = 0>
	constexpr explicit Range(const Range<OtherIter>& other): b_(other.begin()), e_(other.end()) {}
	...
	template<class T,
	         size_t N,
	         typename = typename std::enable_if<std::is_convertible<const T*, Iter>::value>::type>
	constexpr explicit Range(const std::array<T, N>& array):
	        b_{array.empty() ? nullptr : &array.at(0)},
	        e_{array.empty() ? nullptr : &array.at(0) + N} {}
	template<class T,
	         size_t N,
	         typename = typename std::enable_if<std::is_convertible<T*, Iter>::value>::type>
	...
	constexpr size_type walk_size() const { return size_type(std::distance(b_, e_)); }
...
private:
	...
	template<typename ValueType>
	using StringViewTypeFunc = std::conditional_t<detail::range_is_char_type_v_<Iter>,
	                                              StringViewTypeChar,
	                                              StringViewTypeNone>;
	template<typename ValueType>
	struct StringViewType {
		using type = typename StringViewTypeFunc<ValueType>::template apply<ValueType>;
	};
...
public:
	...
	template<typename Tgt,
	         std::enable_if_t<std::is_constructible<Tgt, Iter const&, size_type>::value &&
	                                  !IsConstructibleViaStringView<Tgt>::value,
	                          int> = 0>
	constexpr explicit operator Tgt() const
	        noexcept(std::is_nothrow_constructible<Tgt, Iter const&, size_type>::value) {
	...
	template<typename Tgt,
	         std::enable_if_t<!std::is_constructible<Tgt, Iter const&, size_type>::value &&
	                          ...
	                          int> = 0>
	constexpr explicit operator Tgt() const
	        noexcept(std::is_nothrow_constructible<Tgt, Iter const&, Iter const&>::value) {
		return Tgt(b_, e_);
	}
	...
	template<typename Tgt,
	         ...
	         std::enable_if_t<StrictConjunction<std::is_same<Tgt, _t<StringViewType<ValueType>>>,
	                                            std::is_constructible<_t<StringViewType<ValueType>>,
	                                                                  Iter const&,
	                                                                  size_type>>::value,
	                          int> = 0>
	constexpr operator Tgt() const
	        noexcept(std::is_nothrow_constructible<Tgt, Iter const&, size_type>::value) {
		return Tgt(b_, walk_size());
	}

#if FMT_VERSION < 100000
	template<typename IterType = Iter,
	...
	constexpr std::enable_if_t<std::is_constructible<Tgt, Iter const&, size_type>::value, Tgt> to(
	        Args&&... args) const
	        noexcept(std::is_nothrow_constructible<Tgt, Iter const&, size_type, Args&&...>::value) {
		return Tgt(b_, walk_size(), static_cast<Args&&>(args)...);
	}
	template<typename Tgt, typename... Args>
	constexpr std::enable_if_t<!std::is_constructible<Tgt, Iter const&, size_type>::value &&
	                                   std::is_constructible<Tgt, Iter const&, Iter const&>::value,
	                           Tgt>
	to(Args&&... args) const noexcept(
	        std::is_nothrow_constructible<Tgt, Iter const&, Iter const&, Args&&...>::value) {
	...
	size_type find(const_range_type str) const { return qfind(castToConst(), str); }
	...
	size_type find(Iter s, size_t pos, size_t n) const {
		...
		size_t ret = qfind(pos ? forFinding.subpiece(pos) : forFinding, const_range_type(s, n));
		return ret == npos ? ret : ret + pos;
	}

	// Works only for Range<(const) (unsigned) char*> which have Range(Iter) ctor
	size_type find(const Iter s) const { return qfind(castToConst(), const_range_type(s)); }
	...
	size_type rfind(const value_type& c) const { return folly::rfind(castToConst(), c); }
	...
	// Works only for Range<(const) (unsigned) char*> which have Range(Iter) ctor
	size_type find_first_of(Iter needles) const { return find_first_of(const_range_type(needles)); }
	...
	size_type find_first_of(const value_type& c, size_t pos) const { return find(c, pos); }
	 ...
	 */
	bool contains(const const_range_type& other) const { return find(other) != std::string::npos; }

	bool contains(const value_type& other) const { return find(other) != std::string::npos; }
	...
	bool startsWith(const value_type& c) const { return !empty() && front() == c; }
	...
	bool starts_with(const_range_type other) const noexcept { return startsWith(other); }
	...
	bool ends_with(const_range_type other) const noexcept { return endsWith(other); }
	...
	bool removePrefix(const value_type& prefix) { return startsWith(prefix) && (++b_, true); }
	...
	bool removeSuffix(const value_type& suffix) { return endsWith(suffix) && (--e_, true); }
	...
	size_t replaceAll(const_range_type source, const_range_type dest) {
		if(source.size() != dest.size()) {
			throw_exception<std::invalid_argument>("replacement must have the same size as source");
		}
	...
	Range split_step(Range delimiter) {
		...
		b_ = result.end() == e_ ? e_ : std::next(result.end(), difference_type(delimiter.size()));
	...
	auto split_step(const value_type& delimiter, TProcess&& process, Args&&... args)
...
template<class Iter>
void swap(Range<Iter>& lhs, Range<Iter>& rhs) noexcept(noexcept(lhs.swap(rhs))) {
	lhs.swap(rhs);
}
...
struct ComparableAsStringPiece {
	enum {
		value = (std::is_convertible<A, StringPiece>::value &&
		        ...
		        (std::is_convertible<B, StringPiece>::value && std::is_same<A, StringPiece>::value)
	};
};
...
std::enable_if_t<detail::ComparableAsStringPiece<T, U>::value, bool> operator==(const T& lhs,
                                                                                const U& rhs) {
...
inline size_t qfind(const Range<const char*>& haystack, const char& needle) {
	...
	auto pos = static_cast<const char*>(::memchr(haystack.data(), needle, haystack.size()));
...
template<>
inline size_t qfind(const Range<const unsigned char*>& haystack, const unsigned char& needle) {
	// memchr expects a not-null pointer, early return if the range is empty.
	if(haystack.empty()) {
		return std::string::npos;
	}
	auto pos =
	        static_cast<const unsigned char*>(::memchr(haystack.data(), needle, haystack.size()));
	return pos == nullptr ? std::string::npos : pos - haystack.data();
}

template<>
inline size_t rfind(const Range<const unsigned char*>& haystack, const unsigned char& needle) {
	// memchr expects a not-null pointer, early return if the range is empty.
	if(haystack.empty()) {
		return std::string::npos;
	}
	auto pos = static_cast<const unsigned char*>(memrchr(haystack.data(), needle, haystack.size()));
	return pos == nullptr ? std::string::npos : pos - haystack.data();
}
...
inline size_t qfind_first_of(const Range<const unsigned char*>& haystack,
	...
	return detail::qfind_first_byte_of(StringPiece(haystack), StringPiece(needles));
}
...
struct hasher<folly::Range<T*>, std::enable_if_t<std::is_integral<T>::value, void>> {
	...
	size_t operator()(folly::Range<T*> r) const {
		...
		return static_cast<size_t>(hash::SpookyHashV2::Hash64(r.begin(), r.size() * sizeof(T), 0));
	}
};
...
#if defined(__cpp_char8_t) && __cpp_char8_t >= 201811L
constexpr Range<char8_t const*> operator""_sp(char8_t const* str, size_t len) noexcept {
