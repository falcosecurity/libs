...
public:
	...
	constexpr Indestructible() noexcept(noexcept(T())): storage_{std::in_place} {}
	...
	template<
	        ...
	        std::enable_if_t<!std::is_same<Indestructible<T>, remove_cvref_t<U>>::value>* = nullptr,
	        std::enable_if_t<!std::is_convertible<U&&, T>::value>* = nullptr>
	explicit constexpr Indestructible(U&& u) noexcept(noexcept(T(std::declval<U>()))):
	...
	template<
	        ...
	        std::enable_if_t<!std::is_same<Indestructible<T>, remove_cvref_t<U>>::value>* = nullptr,
	        std::enable_if_t<std::is_convertible<U&&, T>::value>* = nullptr>
	/* implicit */ constexpr Indestructible(U&& u) noexcept(noexcept(T(std::declval<U>()))):
	        storage_{std::in_place, std::forward<U>(u)} {}
	...
	explicit constexpr Indestructible(std::initializer_list<U> il, Args... args) noexcept(
	        noexcept(T(std::declval<std::initializer_list<U>&>(), std::declval<Args>()...))):
	...
	T const* get() const noexcept { return reinterpret_cast<T const*>(&storage_.bytes); }
...
private:
	struct Storage {
		...
		template<typename... Args, typename = decltype(T(std::declval<Args>()...))>
		explicit constexpr Storage(std::in_place_t,
		                           Args&&... args) noexcept(noexcept(T(std::declval<Args>()...))) {
			::new(&bytes) T(std::forward<Args>(args)...);
		}

		template<typename Factory>
		constexpr Storage(factory_constructor_t, Factory factory) noexcept(noexcept(factory())) {
			::new(&bytes) T(factory());
		}
	};
