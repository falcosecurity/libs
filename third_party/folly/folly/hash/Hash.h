...
constexpr uint64_t hash_128_to_64(const uint64_t upper, const uint64_t lower) noexcept {
...
constexpr uint64_t hash_sequence(const H& h, const T& v) noexcept(noexcept(h(v))) {
	return h(v);
}

template<typename H, typename T, typename... Ts>
constexpr uint64_t hash_sequence(const H& h, const T& v, const Ts&... ts) noexcept(noexcept(h(v))) {
	return hash::hash_128_to_64(h(v), hash_sequence(h, ts...));
}

template<typename H, typename Tuple, size_t... Is>
uint64_t hash_tuple_sequence(const H& h, const Tuple& t, std::index_sequence<Is...>) {
 ...
 */
FOLLY_DISABLE_UNDEFINED_BEHAVIOR_SANITIZER("unsigned-integer-overflow")
constexpr uint64_t commutative_hash_128_to_64(const uint64_t upper, const uint64_t lower) noexcept {
...
struct integral_hasher {
	using folly_is_avalanching = std::bool_constant<(sizeof(Int) >= 8 || sizeof(size_t) == 4)>;
...
struct IsAvalanchingHasherFromMemberType : std::bool_constant<!require_sizeof<Hasher>> {};
...
template<>
struct hasher<unsigned long long> : detail::integral_hasher<unsigned long long> {};
...
struct hasher<std::string> {
	...
	size_t operator()(const std::string& key) const {
		return static_cast<size_t>(hash::SpookyHashV2::Hash64(key.data(), key.size(), 0));
	}
};
template<typename K>
...
struct hash_one_fn {
	template<typename T>
	constexpr size_t operator()(T const& v) const noexcept(noexcept(hasher<T>{}(v))) {
		return hasher<T>{}(v);
	}
};
...
struct hasher<std::unique_ptr<T>> {
	...
	size_t operator()(const std::unique_ptr<T>& key) const { return hasher<T*>{}(key.get()); }
};
...
struct hasher<std::shared_ptr<T>> {
	...
	size_t operator()(const std::shared_ptr<T>& key) const { return hasher<T*>{}(key.get()); }
};

// combiner for multi-arg tuple also mixes bits
template<typename T, typename K>
struct IsAvalanchingHasher<hasher<std::tuple<T>>, K> : IsAvalanchingHasher<hasher<T>, K> {};
template<typename T1, typename T2, typename... Ts, typename K>
struct IsAvalanchingHasher<hasher<std::tuple<T1, T2, Ts...>>, K> : std::true_type {};
...
stdCompatibleHash(std::string_view sv) noexcept(noexcept(std::hash<std::string_view>{}(sv))) {
...
size_t hash_combine_generic(const Hasher& h, const T& t, const Ts&... ts) noexcept(
...
struct TupleHasher {
	size_t operator()(std::tuple<Ts...> const& key) const {
		return hash::hash_combine(TupleHasher<index - 1, Ts...>()(key), std::get<index>(key));
	}
};
...
public:
	using folly_is_avalanching = std::bool_constant<(
	        sizeof...(Ts) != 1 || folly::IsAvalanchingHasher<std::hash<FirstT>, FirstT>::value)>;

	size_t operator()(std::tuple<Ts...> const& key) const {
		folly::TupleHasher<sizeof...(Ts) - 1,  // start index
		                   Ts...>
		        hasher;
...
struct IsAvalanchingHasher<std::hash<std::basic_string<Args...>>, K> : std::true_type {};

template<typename... Args, typename K>
struct IsAvalanchingHasher<std::hash<std::basic_string_view<Args...>>, K> : std::true_type {};
