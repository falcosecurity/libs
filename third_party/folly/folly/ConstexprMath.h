...
inline constexpr T e_v =
        detail::enable_if_floating_t<T>(2.71828182845904523536028747135266249775724709369995L);
...
inline constexpr T ln2_v =
        detail::enable_if_floating_t<T>(0.69314718055994530941723212145817656807550013436025L);
...
public:
	explicit constexpr constexpr_iterated_squares_desc(base_type r) noexcept: base{r}, scaling{} {
...
inline constexpr auto constexpr_iterated_squares_desc_v =
        constexpr_iterated_squares_desc<typename Base::value_type,
...
constexpr auto& constexpr_iterated_squares_desc_2_v =
        constexpr_iterated_squares_desc_v<floating_point_integral_constant<T, int, 2>>;
...
template<typename T, typename E, std::enable_if_t<std::is_signed<E>::value, int> = 1>
...
template<typename T, typename E, std::enable_if_t<std::is_unsigned<E>::value, int> = 1>
constexpr T constexpr_ipow(T const base, E const exp) {
	if(std::is_floating_point<T>::value) {
		if(exp == E(0)) {
			return T(1);
...
template<typename T,
         typename N,
         std::enable_if_t<std::is_floating_point<T>::value && std::is_integral<N>::value &&
...
template<typename N,
         std::enable_if_t<std::is_integral<N>::value && !std::is_same<N, bool>::value, int> = 0>
...
template<typename T,
         typename E,
         std::enable_if_t<std::is_integral<E>::value && !std::is_same<E, bool>::value, int> = 0>
...
constexpr T constexpr_pow(T const base, T const exp) {
	...
	if(base == -lim::infinity()) {
		auto const oddi =  //
		        exp == constexpr_trunc(exp) && exp != constexpr_trunc(exp / T(2)) * T(2);
...
constexpr std::size_t constexpr_find_first_set(T t) {
	...
	return t == T(0) ? 0 : 1 + detail::constexpr_find_first_set_(size{}, 0, static_cast<U>(t));
}
...
constexpr typename std::enable_if<std::is_integral<Src>::value, Dst>::type constexpr_clamp_cast(
        Src src) {
	static_assert(std::is_integral<Dst>::value && sizeof(Dst) <= sizeof(int64_t),
