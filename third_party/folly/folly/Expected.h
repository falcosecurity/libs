...
#define FOLLY_REQUIRES_IMPL(...)                                                                  \
	bool FOLLY_EXPECTED_ID(Requires) = false,                                                     \
	     typename std::enable_if<(FOLLY_EXPECTED_ID(Requires) || static_cast<bool>(__VA_ARGS__)), \
	                             int>::type = 0
...
public:
	...
	[[FOLLY_ATTR_GNU_COLD]] constexpr /* implicit */ Unexpected(const Error& err): error_(err) {}
	[[FOLLY_ATTR_GNU_COLD]] constexpr /* implicit */ Unexpected(Error&& err):
	        error_(std::move(err)) {}

	template<class Other FOLLY_REQUIRES_TRAILING(std::is_constructible<Error, Other&&>::value)>
	constexpr /* implicit */ Unexpected(Unexpected<Other> that): error_(std::move(that.error())) {}

	/**
	 * Assignment
	 */
	template<class Other FOLLY_REQUIRES_TRAILING(std::is_assignable<Error&, Other&&>::value)>
...
template<class Error FOLLY_REQUIRES_TRAILING(IsEqualityComparable<Error>::value)>
inline bool operator==(const Unexpected<Error>& lhs, const Unexpected<Error>& rhs) {
 ...
 */
template<class Error>
constexpr Unexpected<typename std::decay<Error>::type> makeUnexpected(Error&& err) {
	return Unexpected<typename std::decay<Error>::type>{static_cast<Error&&>(err)};
}
...
public:
	explicit BadExpectedAccess(Error error): error_{static_cast<Error&&>(error)} {}
...
[[nodiscard]] constexpr Expected<typename std::decay<Value>::type, Error> makeExpected(Value&&);
...
template<class T>
using IsNothrowCopyable = StrictConjunction<std::is_nothrow_copy_constructible<T>,
                                            std::is_nothrow_copy_assignable<T>>;

template<class T>
using IsNothrowMovable = StrictConjunction<std::is_nothrow_move_constructible<T>,
                                           std::is_nothrow_move_assignable<T>>;

template<class From, class To>
using IsConvertible =
        StrictConjunction<std::is_constructible<To, From>, std::is_assignable<To&, From>>;
...
struct ExpectedStorage<Value, Error, StorageType::eUnion>
        : ExpectedUnion<Value, Error>,
          CopyConstructible<ExpectedStorage<Value, Error, StorageType::eUnion>,
                            StrictAllOf<std::is_copy_constructible, Value, Error>::value,
                            StrictAllOf<std::is_nothrow_copy_constructible, Value, Error>::value>,
          MoveConstructible<ExpectedStorage<Value, Error, StorageType::eUnion>,
	...
	bool uninitializedByException() const noexcept { return this->which_ == Which::eEmpty; }
	template<class... Vs>
	void assignValue(Vs&&... vs) {
		auto& val = this->value();
		if(this->which_ == Which::eValue) {
			...
			this->clear();
			auto addr = const_cast<void*>(static_cast<void const*>(std::addressof(val)));
	...
	void assignError(Es&&... es) {
		if(this->which_ == Which::eError) {
			expected_detail::doEmplaceAssign(0, this->error(), static_cast<Es&&>(es)...);
		} else {
			this->clear();
			::new((void*)std::addressof(this->error())) Error(static_cast<Es&&>(es)...);
...
struct ExpectedStorage<Value, Error, StorageType::ePODStruct> {
	...
	constexpr ExpectedStorage() noexcept: which_(Which::eError), error_{}, value_{} {}
	explicit constexpr ExpectedStorage(EmptyTag) noexcept:
	        which_(Which::eEmpty),
	        error_{},
	        value_{} {}
...
struct ExpectedHelper {
	...
	static constexpr Expected<typename std::decay<T>::type, Error> return_(T&& t) {
		return folly::makeExpected<Error>(static_cast<T&&>(t));
	}

	template<class Error,
	         class T,
	         class U FOLLY_REQUIRES_TRAILING(expected_detail::IsConvertible<U&&, Error>::value)>
	...
	static auto then_(This&& ex, Fn&& fn, Fns&&... fns)
	        -> decltype(T::then_(T::template return_<E>(
	...
	template<class This,
	         ...
	         class T = ExpectedHelper,
	         class RetValue = decltype(T::then_(T::template return_<E>((std::declval<No>()(
	                                                    std::declval<This>().error()))),
	                                            std::declval<AndFns>()...)
	                                           .value()),
	         typename std::enable_if<
	                 !std::is_same<std::remove_reference<RetValue>,
	...
	static auto orElse_(This&& ex, No&& no, AndFns&&... fns) -> Expected<V, E> {
		...
		return T::then_(
		        T::template return_<E>((static_cast<No&&>(no)(static_cast<This&&>(ex).error()))),
	...
	template<class This,
	         ...
	         class Err = decltype(std::declval<No>()(std::declval<This&>().error()))
	                 FOLLY_REQUIRES_TRAILING(!std::is_void<Err>::value)>
	static auto orElse_(This&& ex, No&& no, AndFns&&... fns) -> Expected<folly::Unit, E> {
		// Note - this basically decays into then_ once the first type (No) is
		// called for the error.
	...
	template<class This,
	         class OnError,
	         class Err = decltype(std::declval<OnError>()(std::declval<This>().error()))
	                 FOLLY_REQUIRES_TRAILING(std::is_void<Err>::value)>
...
inline expected_detail::UnexpectedTag unexpected(expected_detail::UnexpectedTag = {}) {
	return {};
}
...
public:
	...
	template<class V,
	         class E FOLLY_REQUIRES_TRAILING(!std::is_same<Expected<V, E>, Expected>::value &&
	...
	template<class T FOLLY_REQUIRES_TRAILING(std::is_convertible<T, Value>::value &&
	...
	template<class... Ts FOLLY_REQUIRES_TRAILING(std::is_constructible<Value, Ts&&...>::value)>
	...
	template<class U,
	         class... Ts FOLLY_REQUIRES_TRAILING(
	                 std::is_constructible<Value, std::initializer_list<U>&, Ts&&...>::value)>
	explicit constexpr Expected(std::in_place_t,
	 ...
	 * swap
	 */
	void swap(Expected& that) noexcept(std::is_nothrow_swappable_v<Value> &&
	...
	Value* get_pointer() & noexcept { return hasValue() ? std::addressof(this->value_) : nullptr; }
	...
	auto then(Fns&&... fns)
	        const& -> decltype(expected_detail::ExpectedHelper::then_(std::declval<const Base&>(),
	                                                                  std::declval<Fns>()...)) {
		if(this->uninitializedByException()) {
			throw_exception<BadExpectedAccess<void>>();
		}
		return expected_detail::ExpectedHelper::then_(base(), static_cast<Fns&&>(fns)...);
	}
	...
	template<class... Fns FOLLY_REQUIRES_TRAILING(sizeof...(Fns) >= 1)>
	auto orElse(Fns&&... fns)
	        const& -> decltype(expected_detail::ExpectedHelper::orElse_(std::declval<const Base&>(),
	                                                                    std::declval<Fns>()...)) {
		if(this->uninitializedByException()) {
			throw_exception<BadExpectedAccess<void>>();
		}
		return expected_detail::ExpectedHelper::orElse_(base(), static_cast<Fns&&>(fns)...);
	}

	template<class... Fns FOLLY_REQUIRES_TRAILING(sizeof...(Fns) >= 1)>
	auto orElse(Fns&&... fns) & -> decltype(expected_detail::ExpectedHelper::orElse_(
	                                    std::declval<Base&>(),
	                                    std::declval<Fns>()...)) {
		if(this->uninitializedByException()) {
			throw_exception<BadExpectedAccess<void>>();
	...
	auto orElse(Fns&&... fns) && -> decltype(expected_detail::ExpectedHelper::orElse_(
		...
		return expected_detail::ExpectedHelper::orElse_(std::move(base()),
		                                                static_cast<Fns&&>(fns)...);
	}
	...
	auto thenOrThrow(Yes&& yes, No&& no = No{})
		...
		return Ret(expected_detail::ExpectedHelper::thenOrThrow_(base(),
		                                                         static_cast<Yes&&>(yes),
		                                                         static_cast<No&&>(no)));
	}

	template<class Yes, class No = MakeBadExpectedAccess>
	auto thenOrThrow(Yes&& yes,
	                 No&& no = No{}) & -> decltype(std::declval<Yes>()(std::declval<Value&>())) {
	...
	auto thenOrThrow(Yes&& yes,
		...
		return Ret(expected_detail::ExpectedHelper::thenOrThrow_(std::move(base()),
		                                                         static_cast<Yes&&>(yes),
		                                                         static_cast<No&&>(no)));
	}
	...
	auto onError(OnError&& onError) const& {
		...
		return expected_detail::ExpectedHelper::onError_(*this, static_cast<OnError&&>(onError));
	}
	...
	auto onError(OnError&& onError) && {
		...
		return expected_detail::ExpectedHelper::onError_(std::move(*this),
		                                                 static_cast<OnError&&>(onError));
	}
...
private:
	...
	Expected(EmptyTag tag, Expected*& pointer) noexcept: Base{tag} { pointer = this; }
...
template<class Value, class Error FOLLY_REQUIRES_TRAILING(IsEqualityComparable<Value>::value)>
...
inline typename std::enable_if<IsLessThanComparable<Value>::value, bool>::type operator<(
        const Expected<Value, Error>& lhs,
        const Expected<Value, Error>& rhs) {
	if(FOLLY_UNLIKELY(lhs.uninitializedByException() || rhs.uninitializedByException())) {
		throw_exception<BadExpectedAccess<void>>();
	}
	if(FOLLY_UNLIKELY(lhs.hasError())) {
		return !rhs.hasError();
...
template<class Value, class Error FOLLY_REQUIRES_TRAILING(IsLessThanComparable<Value>::value)>
inline bool operator<=(const Expected<Value, Error>& lhs, const Expected<Value, Error>& rhs) {
	return !(rhs < lhs);
}
 ...
 */
template<class Value, class Error>
void swap(Expected<Value, Error>& lhs,
          Expected<Value, Error>& rhs) noexcept(std::is_nothrow_swappable_v<Value> &&
                                                std::is_nothrow_swappable_v<Error>) {
	lhs.swap(rhs);
}
 ...
 */
template<class Error, class Value>
[[nodiscard]] constexpr Expected<typename std::decay<Value>::type, Error> makeExpected(
        Value&& val) {
	return Expected<typename std::decay<Value>::type, Error>{std::in_place,
...
struct PromiseReturn {
	...
	/* implicit */ PromiseReturn(PromiseBase<Value, Error>& p) noexcept: pointer_{p.value_} {
		pointer_ = &storage_;
	}
...
template<typename Value, typename Error>
struct Promise  //
        : conditional_t<ReturnsVoid<Value>,
...
struct UnexpectedAwaitable {
	...
	template<typename U>
	FOLLY_ALWAYS_INLINE void await_suspend(coro::coroutine_handle<Promise<U, Error>> h) noexcept(
