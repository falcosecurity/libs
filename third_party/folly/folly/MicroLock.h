...
protected:
	...
	uint8_t lockSlowPath(uint8_t oldWord, unsigned maxSpins, unsigned maxYields) noexcept;

	static constexpr unsigned kNumLockBits = 2;
	static constexpr uint8_t kLockBits = static_cast<uint8_t>((1 << kNumLockBits) - 1);
	...
	static constexpr uint8_t encodeDataToWord(uint8_t word, uint8_t value) noexcept {
...
public:
	 ...
	 */
	uint8_t load(std::memory_order order = std::memory_order_seq_cst) const noexcept {
	 ...
	 * ignored.
	 */
	void store(uint8_t value, std::memory_order order = std::memory_order_seq_cst) noexcept;
...
inline void MicroLockCore::store(uint8_t value, std::memory_order order) noexcept {
	auto oldWord = atomic_ref(lock_).load(std::memory_order_relaxed);
	while(true) {
		auto newWord = encodeDataToWord(oldWord, value);
		if(atomic_ref(lock_).compare_exchange_weak(oldWord,
		                                           newWord,
		                                           order,
...
void MicroLockCore::unlockAndStoreWithModifier(Func modifier) noexcept {
	...
	} while(!atomic_ref(lock_).compare_exchange_weak(oldWord,
	                                                 newWord,
...
inline void MicroLockCore::unlockAndStore(uint8_t value) noexcept {
	unlockAndStoreWithModifier(
	        [value](uint8_t oldWord) { return encodeDataToWord(oldWord, value); });
}
...
public:
	...
	struct LockGuardWithData {
		explicit LockGuardWithData(MicroLockBase<MaxSpins, MaxYields>& lock): lock_(lock) {
...
uint8_t MicroLockBase<MaxSpins, MaxYields>::lockAndLoad() noexcept {
	...
	if((oldWord & heldBit()) == 0 &&
	   atomic_ref(lock_).compare_exchange_weak(oldWord,
