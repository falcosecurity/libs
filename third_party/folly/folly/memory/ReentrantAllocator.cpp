...
reentrant_allocator_base::reentrant_allocator_base(reentrant_allocator_base const& that) noexcept {
	meta_ = that.meta_;
...
void* reentrant_allocator_base::allocate(std::size_t const n, std::size_t const a) noexcept {
	if(!n) {
		return &dummy;
	}
	...
	while(true) {
		...
		if(offset + n > block_size || !head) {
			...
			auto const exchanged = meta_->head.compare_exchange_weak(head,
			                                                         newhead,
		...
		auto const exchanged = head->size.compare_exchange_weak(size,
		                                                        newsize,
		                                                        std::memory_order_release,
		                                                        std::memory_order_relaxed);
		if(!exchanged) {
...
void reentrant_allocator_base::deallocate(void* const p, std::size_t const n) noexcept {
	if(p == &dummy) {
