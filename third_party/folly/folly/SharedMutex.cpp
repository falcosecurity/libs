...
std::unique_lock<std::mutex> annotationGuard(void* ptr) {
	if(folly::kIsSanitizeThread) {
		...
		static Indestructible<std::array<std::mutex, kNumAnnotationMutexes>> kAnnotationMutexes;
...
uint32_t getMaxDeferredReadersSlow(relaxed_atomic<uint32_t>& cache) {
	uint32_t maxDeferredReaders =
	        std::min(static_cast<uint32_t>(folly::nextPowTwo(CacheLocality::system().numCpus) << 1),
