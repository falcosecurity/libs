...
public:
	...
	T await_resume() noexcept(noexcept(T(FOLLY_DECLVAL(T&&)))) { return static_cast<T&&>(value_); }
...
inline constexpr await_suspend_return_coroutine_fn await_suspend_return_coroutine{};
...
public:
	...
	auto await_ready() noexcept((noexcept(FOLLY_DECLVAL(A&).await_ready()) && ...)) {
	...
	auto await_resume() noexcept((noexcept(FOLLY_DECLVAL(A&).await_resume()) && ...)) {
...
struct detect_promise_return_object_eager_conversion_ {
	...
	static detect_promise_return_object_eager_conversion_ go() noexcept {
		...
		FOLLY_PUSH_WARNING
#if defined(__clang__) && (13 < __clang_major__ && __clang_major__ < 17 - defined(__APPLE__))
...
private:
	...
	struct use_extended_handle<T, void_t<use_extended_handle_of_<T>>> {
		static constexpr bool value = std::is_same_v<use_extended_handle_of_<T>, PrivateTag>;
	};
...
public:
	...
	/*implicit*/ ExtendedCoroutineHandle(coroutine_handle<Promise> handle) noexcept:
	        basic_(handle),
	        extended_(fromBasic(handle)) {}

	/*implicit*/ ExtendedCoroutineHandle(coroutine_handle<> handle) noexcept: basic_(handle) {}

	template<typename Promise, std::enable_if_t<use_extended_handle<Promise>::value, int> = 0>
...
// coros required it.
template<typename Promise>
class ExtendedCoroutinePromiseCrtp : public ExtendedCoroutineHandle::PromiseBase {
public:
	using use_extended_handle_concept = ExtendedCoroutineHandle::PrivateTag;

	static ExtendedCoroutineHandle::PromiseBase* getPromiseBase(ExtendedCoroutineHandle::PrivateTag,
