...
// Minimal replacement for Boost.Preprocessor for FOLLY_DETAIL_CREATE_FREE_INVOKE_TRAITS_USING
// (vendored Folly: avoid external Boost dependency; supports 0 or 1 namespace only)
#define FOLLY_DETAIL_IS_EMPTY(...) FOLLY_DETAIL_IS_EMPTY_(__VA_ARGS__ _, 0, 1)
#define FOLLY_DETAIL_IS_EMPTY_(a, b, c, ...) c
#define FOLLY_DETAIL_NOT(x) FOLLY_DETAIL_NOT_IMPL(x)
#define FOLLY_DETAIL_NOT_IMPL(x) FOLLY_DETAIL_NOT_##x
#define FOLLY_DETAIL_NOT_0 1
#define FOLLY_DETAIL_NOT_1 0
#define FOLLY_DETAIL_IIF(bit, expr) FOLLY_DETAIL_IIF_IMPL(bit, expr)
#define FOLLY_DETAIL_IIF_IMPL(bit, expr) FOLLY_DETAIL_IIF_##bit(expr)
#define FOLLY_DETAIL_IIF_0(expr)
#define FOLLY_DETAIL_IIF_1(expr) expr
...
struct conv_r_<false> {
	...
	template<bool NX, typename R, typename FR, bool C = noexcept(conv<R>(FOLLY_DECLVAL(FR)))>
...
template<typename F, typename... A>
inline constexpr bool is_invocable_v<void_t<invoke_result_t<F, A...>>, F, A...> = true;
...
template<typename F, typename... A>
inline constexpr bool is_nothrow_invocable_v<void_t<invoke_result_t<F, A...>>, F, A...> =
...
template<typename F, typename... A>
inline constexpr bool is_invocable_v = invoke_detail::is_invocable_v<void, F, A...>;
...
template<typename R, typename F, typename... A>
inline constexpr bool is_invocable_r_v = invoke_detail::is_invocable_r_v<void, R, F, A...>;
...
template<typename F, typename... A>
inline constexpr bool is_nothrow_invocable_v = invoke_detail::is_nothrow_invocable_v<void, F, A...>;

//  mimic: std::is_nothrow_invocable, C++17
template<typename F, typename... A>
struct is_nothrow_invocable : std::bool_constant<is_nothrow_invocable_v<F, A...>> {};
...
template<typename R, typename F, typename... A>
struct is_nothrow_invocable_r : std::bool_constant<is_nothrow_invocable_r_v<R, F, A...>> {};
...
template<typename I>
using invoke_traits_base = invoke_traits_base_<is_constexpr_default_constructible_v<I>, I>;
...
public:
	...
	template<typename... A>
	inline static constexpr bool is_invocable_v = invoke_detail::is_invocable_v<void, I, A...>;
	template<typename... A>
	struct is_invocable  //
	...
	struct is_nothrow_invocable  //
	        : std::bool_constant<invoke_detail::is_nothrow_invocable_v<void, I, A...>> {};
	template<typename R, typename... A>
	inline static constexpr bool is_nothrow_invocable_r_v =
	        invoke_detail::is_nothrow_invocable_r_v<void, R, I, A...>;
	template<typename R, typename... A>
	struct is_nothrow_invocable_r  //
	        : std::bool_constant<invoke_detail::is_nothrow_invocable_r_v<void, R, I, A...>> {};
};
...
#define FOLLY_DETAIL_CREATE_FREE_INVOKE_TRAITS_USING_1(funcname, ns) using ns::funcname;

#define FOLLY_DETAIL_CREATE_FREE_INVOKE_TRAITS_USING(_, funcname, ...)     \
	FOLLY_DETAIL_IIF(FOLLY_DETAIL_NOT(FOLLY_DETAIL_IS_EMPTY(__VA_ARGS__)), \
	                 FOLLY_DETAIL_CREATE_FREE_INVOKE_TRAITS_USING_1(funcname, __VA_ARGS__))
...
#define FOLLY_CREATE_FREE_INVOKER(classname, funcname, ...)                                       \
	namespace classname##__folly_detail_invoke_ns {                                               \
		...
		struct __folly_detail_invoke_obj {                                                        \
			...
			[[maybe_unused]] FOLLY_ERASE_HACK_GCC constexpr auto operator()(Args&&... args) const \
	...
	struct classname : classname##__folly_detail_invoke_ns::__folly_detail_invoke_obj {}
...
#define FOLLY_CREATE_QUAL_INVOKER(classname, funcpath)                                  \
	struct classname {                                                                  \
		template<typename... A>                                                         \
		[[maybe_unused]] FOLLY_ERASE_HACK_GCC constexpr auto operator()(A&&... a) const \
...
#define FOLLY_CREATE_MEMBER_INVOKER(classname, membername)                                       \
	struct classname {                                                                           \
		...
		[[maybe_unused]] FOLLY_ERASE_HACK_GCC constexpr auto operator()(O&& o,                   \
		                                                                Args&&... args) const    \
		        noexcept(noexcept(static_cast<O&&>(o).membername(static_cast<Args&&>(args)...))) \
...
struct invoke_member_wrapper_fn : private F {
	...
	constexpr explicit invoke_member_wrapper_fn(G&& f) noexcept(noexcept(F(static_cast<G&&>(f)))):
	        F(static_cast<G&&>(f)) {}
	using F::operator();
};
template<typename F>
...
template<typename Tag, typename... Args>
using tag_invoke_result_t = decltype(tag_invoke(static_cast<Tag && (*)() noexcept>(nullptr)(),
                                                static_cast<Args && (*)() noexcept>(nullptr)()...));

template<typename Tag, typename... Args>
auto try_tag_invoke(int) noexcept(noexcept(tag_invoke(FOLLY_DECLVAL(Tag&&),
                                                      FOLLY_DECLVAL(Args&&)...)))
        -> decltype(static_cast<void>(tag_invoke(FOLLY_DECLVAL(Tag&&), FOLLY_DECLVAL(Args&&)...)),
                    std::true_type{});
...
template<typename Tag, typename... Args>
struct is_nothrow_tag_invocable : std::bool_constant<is_nothrow_tag_invocable_v<Tag, Args...>> {};
...
using is_tag_invocable_r = folly::is_invocable_r<R, decltype(folly::tag_invoke), Tag, Args...>;

template<typename R, typename Tag, typename... Args>
inline constexpr bool is_tag_invocable_r_v = is_tag_invocable_r<R, Tag, Args...>::value;
...
struct tag_invoke_result
        : conditional_t<is_tag_invocable_v<Tag, Args...>,
