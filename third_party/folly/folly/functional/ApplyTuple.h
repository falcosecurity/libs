...
using index_sequence_for_tuple = std::make_index_sequence<std::tuple_size<Tuple>::value>;
...
struct invoke_get_fn {
	template<typename T>
	constexpr auto operator()(T&& t) const noexcept(noexcept(get<I>(static_cast<T&&>(t))))
...
template<typename Tuple,
         ...
         typename ReturnTuple =
                 std::tuple<decltype(adl::invoke_get_fn<Indices>{}(std::declval<Tuple>()))...>>
auto forward_tuple(Tuple&& tuple, std::index_sequence<Indices...>) -> ReturnTuple {
	return ReturnTuple{adl::invoke_get_fn<Indices>{}(static_cast<Tuple&&>(tuple))...};
}
...
private:
	...
	static constexpr auto invoke_(F&& f, T&& t, std::index_sequence<I...>) noexcept(
	        noexcept(invoke(static_cast<F&&>(f), get<I>{}(static_cast<T&&>(t))...)))
	        -> decltype(invoke(static_cast<F&&>(f), get<I>{}(static_cast<T&&>(t))...)) {
...
public:
	...
	constexpr auto operator()(F&& f, T&& t) const
	        noexcept(noexcept(invoke_(static_cast<F&&>(f), static_cast<T&&>(t), seq<T>{})))
...
auto forward_tuple(Tuple&& tuple) noexcept
        -> decltype(detail::apply_tuple::forward_tuple(
                std::declval<Tuple>(),
                std::declval<index_sequence_for_tuple<std::remove_reference_t<Tuple>>>())) {
...
inline constexpr bool is_applicable_r_v = is_invocable_r_v<R, ApplyInvoke, F, Tuple>;
template<typename R, typename F, typename Tuple>
using is_applicable_r = is_invocable_r<R, ApplyInvoke, F, Tuple>;
template<typename F, typename Tuple>
inline constexpr bool is_nothrow_applicable_v = is_nothrow_invocable_v<ApplyInvoke, F, Tuple>;
template<typename F, typename Tuple>
...
public:
	...
	auto operator()(Tuple&& tuple) const -> decltype(apply(std::declval<F>(),
...
auto uncurry(F&& f) -> detail::apply_tuple::Uncurry<typename std::decay<F>::type> {
	return detail::apply_tuple::Uncurry<typename std::decay<F>::type>(std::forward<F>(f));
}
