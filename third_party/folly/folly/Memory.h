...
private:
	...
	template<typename T>
	using element_type_of = detected_or_t<first_arg_of<T>, detect_element_type, T>;

	template<typename T>
	using detect_to_address = decltype(std::pointer_traits<T>::to_address(FOLLY_DECLVAL(T const&)));

	template<typename T>
	static inline constexpr bool use_pointer_traits_to_address =
	        Conjunction<is_detected<element_type_of, T>, is_detected<detect_to_address, T>>::value;
...
inline constexpr to_address_fn to_address;
        ...
        (defined(__ANDROID__) && (__ANDROID_API__ > 16)) || (defined(__APPLE__)) || \
...
void rawOverAlignedImpl(Alloc const& alloc, size_t n, void*& raw) {
	...
	using BaseAllocTraits = typename AllocTraits::template rebind_traits<BaseType>;
	using BaseAlloc = typename BaseAllocTraits::allocator_type;
	static_assert(sizeof(BaseType) == kBaseAlign && alignof(BaseType) == kBaseAlign);
	...
	if(kAlign <= kBaseAlign) {
		...
		// we want, happy path
		if(kAllocate) {
			raw = static_cast<void*>(std::addressof(*BaseAllocTraits::allocate(a, quanta)));
		} else {
	...
	if(kAllocate) {
		char* base = reinterpret_cast<char*>(
		        std::addressof(*BaseAllocTraits::allocate(a, quanta + kAlign / sizeof(BaseType))));
		size_t byteDelta = kAlign - (reinterpret_cast<uintptr_t>(base) & (kAlign - 1));
...
typename std::allocator_traits<Alloc>::pointer allocateOverAligned(Alloc const& alloc, size_t n) {
	void* raw = nullptr;
	detail::rawOverAlignedImpl<Alloc, kAlign, true>(alloc, n, raw);
	return std::pointer_traits<typename std::allocator_traits<Alloc>::pointer>::pointer_to(
	        *static_cast<typename std::allocator_traits<Alloc>::value_type*>(raw));
}
...
template struct GenerateWeakPtrInternalsAccessor<MemoryAnonTag, &std::__weak_ptr<void>::_M_ptr>;
}  // namespace detail
#endif
...
std::shared_ptr<U> fmap_shared_ptr_aliasing(const std::shared_ptr<T>& owner, GetU getU) {
...
template<typename T,
         typename GetU,
         typename U = std::remove_pointer_t<std::invoke_result_t<GetU&, const T*>>>
std::shared_ptr<U> fmap_shared_ptr_aliasing(std::shared_ptr<T>&& owner, GetU getU) {
	if(auto* tPtr = owner.get()) {
		if(auto* uPtr = getU(tPtr)) {
			return to_shared_ptr_aliasing(owner, uPtr);
		}
...
std::unique_ptr<T> copy_through_unique_ptr(const std::unique_ptr<T>& t) {
	static_assert(!std::is_polymorphic<T>::value || std::is_final<T>::value, "possibly slicing");
...
public:
	...
	friend bool operator==(Self const& a, Self const& b) noexcept { return a.align_ == b.align_; }
	friend bool operator!=(Self const& a, Self const& b) noexcept { return a.align_ != b.align_; }
};
...
public:
	...
	template<typename S = Align, std::enable_if_t<std::is_default_constructible<S>::value, int> = 0>
	...
	friend bool operator==(Self const& a, Self const& b) noexcept { return a.align() == b.align(); }
	friend bool operator!=(Self const& a, Self const& b) noexcept { return a.align() != b.align(); }
};
...
public:
	...
	explicit allocator_delete(const allocator_type& alloc): allocator_type(alloc) {}

	explicit allocator_delete(allocator_type&& alloc): allocator_type(std::move(alloc)) {}

	template<typename U>
	allocator_delete(const allocator_delete<U>& other): allocator_type(other.get_allocator()) {}
...
std::unique_ptr<T,
                allocator_delete<typename std::allocator_traits<Alloc>::template rebind_alloc<T>>>
...
struct AllocatorCustomizesConstruct_ : folly::is_invocable<AllocatorConstruct_, Alloc, Args...> {};

template<typename Alloc, typename... Args>
struct AllocatorCustomizesConstruct_<void_t<typename Alloc::folly_has_default_object_construct>,
                                     Alloc,
                                     Args...>
        : Negation<typename Alloc::folly_has_default_object_construct> {};

template<typename Void, typename Alloc, typename... Args>
struct AllocatorCustomizesDestroy_ : folly::is_invocable<AllocatorDestroy_, Alloc, Args...> {};

template<typename Alloc, typename... Args>
struct AllocatorCustomizesDestroy_<void_t<typename Alloc::folly_has_default_object_destroy>,
                                   Alloc,
                                   Args...>
...
struct AllocatorHasDefaultObjectConstruct
        : Negation<detail::AllocatorCustomizesConstruct_<void, Alloc, T*, Args...>> {};

template<typename Value, typename T, typename... Args>
struct AllocatorHasDefaultObjectConstruct<std::allocator<Value>, T, Args...> : std::true_type {};
...
struct AllocatorHasDefaultObjectDestroy<std::allocator<Value>, T> : std::true_type {};
