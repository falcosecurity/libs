...
int nativeFutexWake(const void* addr, int count, uint32_t wakeMask) {
	const auto rv = syscall(__NR_futex,
	                        addr,                                   /* addr1 */
...
FutexResult nativeFutexWaitImpl(const void* addr,
                                uint32_t expected,
...
int emulatedFutexWake(const void* addr, int count, uint32_t waitMask) {
	...
	parkingLot.unpark(addr, [&](const uint32_t& mask) {
		...
		count--;
		woken++;
		return count > 0 ? UnparkControl::RemoveContinue : UnparkControl::RemoveBreak;
	});
	return woken;
}

template<typename F>
FutexResult emulatedFutexWaitImpl(F* futex,
                                  uint32_t expected,
                                  system_clock::time_point const* absSystemTime,
                                  steady_clock::time_point const* absSteadyTime,
                                  uint32_t waitMask) {
	static_assert(std::is_same<F, const Futex<std::atomic>>::value ||
	...
	if(absSystemTime) {
		...
		res = parkingLot.park(futex, waitMask, [&] { return *futex == expected; }, [] {});
	}
	switch(res) {
...
FutexResult futexWaitImpl(const Futex<std::atomic>* futex,
                          uint32_t expected,
                          system_clock::time_point const* absSystemTime,
                          steady_clock::time_point const* absSteadyTime,
                          uint32_t waitMask) {
#ifdef __linux__
	return nativeFutexWaitImpl(futex, expected, absSystemTime, absSteadyTime, waitMask);
#else
	return emulatedFutexWaitImpl(futex, expected, absSystemTime, absSteadyTime, waitMask);
#endif
}

FutexResult futexWaitImpl(const Futex<EmulatedFutexAtomic>* futex,
                          uint32_t expected,
                          system_clock::time_point const* absSystemTime,
                          steady_clock::time_point const* absSteadyTime,
                          uint32_t waitMask) {
