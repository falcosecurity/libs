...
template<>
struct __is_location_invariant<::folly::threadlocal_detail::SharedPtrDeleter> : std::true_type {};
...
struct ElementWrapper {
	...
	void set(Ptr p) {
		...
		auto const fun = +[](void* pt, TLPDestructionMode) { delete static_cast<Ptr>(pt); };
	...
	static auto makeDeleter(const Deleter& d) {
		return [d](void* pt, TLPDestructionMode mode) { d(static_cast<Ptr>(pt), mode); };
	}
...
struct ThreadEntry {
	...
	std::thread::id& tid() { return *reinterpret_cast<std::thread::id*>(&tid_data); }
...
struct StaticMetaBase {
	 ...
	 * before returning.
	 */
	FOLLY_NOINLINE void ensureThreadEntryIsInSet(ThreadEntry* te,
	 ...
	 */
	FOLLY_ALWAYS_INLINE bool isThreadEntryRemovedFromAllInMap(ThreadEntry* te, bool needForkLock) {
...
struct FakeUniqueInstance {
	...
	FOLLY_ERASE constexpr explicit FakeUniqueInstance(tag_t<Z<Key..., Mapped...>>,
	                                                  tag_t<Key...>,
...
private:
	...
	static constexpr bool IsAccessModeStrict = std::is_same_v<AccessMode, AccessModeStrict>;
	static_assert(!IsTagVoid || !IsAccessModeStrict);

	using UniqueInstance = conditional_t<IsTagVoid, FakeUniqueInstance, detail::UniqueInstance>;
	static UniqueInstance unique;

public:
	StaticMeta(): StaticMetaBase(&StaticMeta::getThreadEntrySlow, IsAccessModeStrict) {
		AtFork::registerHandler(this,
	...
	FOLLY_NOINLINE static void getSlowReserveAndCache(EntryID* ent, LocalCache& cache) {
...
typename StaticMeta<Tag, AccessMode>::UniqueInstance StaticMeta<Tag, AccessMode>::unique{
        tag<StaticMeta>,
        tag<Tag>,
        tag<AccessMode>};
FOLLY_POP_WARNING
