...
[[gnu::weakref("__cxa_demangle")]] static char* cxxabi_demangle(char const*, char*, size_t*, int*);
...
[[maybe_unused]] FOLLY_ERASE int rust_demangle_callback_fallback(const char*,
                                                                 int,
...
FOLLY_CREATE_QUAL_INVOKER(invoke_rust_demangle_primary, ::rust_demangle_callback);
FOLLY_CREATE_QUAL_INVOKER(invoke_rust_demangle_fallback, rust_demangle_callback_fallback);

using invoke_rust_demangle_fn =
        folly::invoke_first_match<invoke_rust_demangle_primary, invoke_rust_demangle_fallback>;
constexpr invoke_rust_demangle_fn invoke_rust_demangle;

int call_rust_demangle_callback(const char* mangled,
                                int options,
                                demangle_callbackref cb,
                                void* opaque) {
...
static constexpr liberty_demangle_t* liberty_cplus_demangle = cplus_demangle_v3_callback;
static constexpr liberty_demangle_t* liberty_rust_demangle = call_rust_demangle_callback;

#if defined(DMGL_NO_RECURSE_LIMIT)
static constexpr auto liberty_demangle_options_no_recurse_limit = DMGL_NO_RECURSE_LIMIT;
#else
...
fbstring demangle(const char* name) {
	...
	if(folly::demangle_build_has_liberty()) {
		...
		for(auto func : funcs) {
			...
			// failure
			int success = func(name, liberty_demangle_options, demangleStringCallback, &demangled);
...
size_t demangle(const char* name, char* out, size_t outSize) {
	...
	if(folly::demangle_build_has_liberty()) {
		...
		for(auto func : funcs) {
			...
			// Unlike most library functions, this returns 1 on success and 0 on
			// failure
			int success = func(name, liberty_demangle_options, demangleBufCallback, &dbuf);
			if(success) {
				if(outSize != 0) {
					*dbuf.dest = '\0';
				}
