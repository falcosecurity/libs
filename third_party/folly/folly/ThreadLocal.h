...
public:
	...
	template<typename SourceT,
	         typename Deleter,
	         typename = typename std::enable_if<std::is_convertible<SourceT*, T*>::value>::type>
	void reset(std::unique_ptr<SourceT, Deleter> source) {
		auto deleter = [delegate = source.get_deleter()](T* ptr, TLPDestructionMode) {
			delegate(ptr);
		};
	...
	public:
		...
		class Iterator {
			...
			const T& dereference() const { return *static_cast<T*>(iter_->wrapper.ptr); }
	...
	Accessor accessAllThreads() const {
		static_assert(AccessAllThreadsEnabled::value,
...
private:
	void destroy() noexcept {
		...
		// reallocated.
		DCHECK(id_.value.load(std::memory_order_relaxed) == threadlocal_detail::kEntryIDInvalid);
	}
