...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
	...
	event->nparams =
	        std::min(event->nparams,
	                 std::min<uint32_t>(event_info->nparams, max_length_entries_in_buffer));
	if(event->nparams == 0) {
		return 0;
	}
	...
	const uint32_t parameters_to_visit =
	        std::min(decoded_parameter_count, static_cast<uint32_t>(decoded_parameters.size()));
