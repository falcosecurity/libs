...
public:
	...
	/**
	 * @brief Assert if in our buffers there is an event generated by `pid_to_search`.
	 * If no `pid_to_search` is specified we search for an event generated by
	 * the process that calls this method.
	 *
	 * We search the event saved in the `event_test` object in case `CURRENT_EVENT_TYPE`
	 * is specified, otherwise we search the passed event_type.
	 *
	 * Search until all buffers are empty. If we don't find any event of this type,
	 * generated by the specified `pid_to_search`, we fail.
	 *
