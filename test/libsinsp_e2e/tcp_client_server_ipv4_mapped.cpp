                   ...
                   bool exit_no_close = false)
{
	...
	std::shared_ptr<tcp_server_ipv4m> server = std::make_shared<tcp_server_ipv4m>(iot, false, use_shutdown, use_accept4, ntransactions, exit_no_close);
	...
	captured_event_callback_t callback = [&](const callback_param& param)
	{
		...
		{
			if(!parse_tuple(evt->get_param_value_str("tuple"), src_addr, src_port, dst_addr, dst_port))
			{
				return;
			}
		         ...
		         evt->get_type() == PPME_SYSCALL_READ_X)
		{
			...
			{
				if(event_capture::get_engine() != MODERN_BPF_ENGINE)
				{
					EXPECT_EQ("NULL", evt->get_param_value_str("tuple"));
				}
				else
		...
		if ((PPME_SYSCALL_CLOSE_X == evt->get_type() ||
		     PPME_SOCKET_SHUTDOWN_X == evt->get_type()) &&
		...
		if (!(use_shutdown || exit_no_close))
		{
			if (evt->get_type() == PPME_GENERIC_E)
			{
				...
				{
					sinsp_threadinfo* ti = param.m_inspector->get_thread_ref(server->get_tid(), false, true).get();
