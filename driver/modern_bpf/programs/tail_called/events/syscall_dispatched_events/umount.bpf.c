...
int BPF_PROG(umount_e, struct pt_regs *regs, long id) {
	struct ringbuf_struct ringbuf;
	if(!ringbuf__reserve_space(&ringbuf, ctx, UMOUNT_E_SIZE, PPME_SYSCALL_UMOUNT_1_E)) {
		return 0;
	}
...
SEC("tp_btf/sys_exit")
int BPF_PROG(umount_x, struct pt_regs *regs, long ret) {
