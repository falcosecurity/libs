...
int BPF_PROG(linkat_e, struct pt_regs *regs, long id) {
	struct ringbuf_struct ringbuf;
	if(!ringbuf__reserve_space(&ringbuf, LINKAT_E_SIZE, PPME_SYSCALL_LINKAT_2_E)) {
		return 0;
	}
...
SEC("tp_btf/sys_exit")
int BPF_PROG(linkat_x, struct pt_regs *regs, long ret) {
