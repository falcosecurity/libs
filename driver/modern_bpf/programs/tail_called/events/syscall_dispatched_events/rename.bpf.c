...
int BPF_PROG(rename_e,
	     struct pt_regs *regs,
	     long id)
{
	struct ringbuf_struct ringbuf;
	if(!ringbuf__reserve_space(&ringbuf, ctx, RENAME_E_SIZE, PPME_SYSCALL_RENAME_E))
	{
		return 0;
	}
	     ...
	     long ret)
{
	...
	auxmap__preload_event_header(auxmap, PPME_SYSCALL_RENAME_X);
