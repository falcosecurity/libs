...
int openat2_e(struct sys_enter_openat2_args *ctx) {
	...
	submit_openat2_enter_event(auxmap, dirfd, filename_pointer, how_pointer);
	return 0;
}

/*=========================== COMMON 64BIT SUPPORT (TRACEPOINT-BASED) =======================*/

/*=============================== IA-32 SUPPORT (FENTRY-BASED) ===========================*/
...
static __always_inline int ia32_common_handler(struct pt_regs *regs, long int retval) {
	...
	int64_t dirfd = (int64_t)(int32_t)extract__syscall_argument(regs, 0);
...
SEC("fentry/__ia32_compat_sys_openat2")
int BPF_PROG(ia32_compat_openat2_e, struct pt_regs *regs, long int retval) {
	return ia32_common_handler(regs, retval);
}

SEC("fentry/__ia32_sys_openat2")
int BPF_PROG(ia32_openat2_e, struct pt_regs *regs, long int retval) {
	return ia32_common_handler(regs, retval);
}
