...
SEC("fentry/__ia32_sys_creat")
int BPF_PROG(ia32_creat_e, struct pt_regs* regs, long int retval) {
	return ia32_handler(regs, retval);
}

SEC("fentry/__ia32_compat_sys_creat")
int BPF_PROG(ia32_compat_creat_e, struct pt_regs* regs, long int retval) {
	return ia32_handler(regs, retval);
}
