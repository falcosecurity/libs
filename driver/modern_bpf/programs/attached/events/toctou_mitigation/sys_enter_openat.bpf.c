...
SEC("fentry/__ia32_sys_openat")
int BPF_PROG(ia32_openat_e, struct pt_regs* regs, long int retval) {
	return ia32_handler(regs, retval);
}

SEC("fentry/__ia32_compat_sys_openat")
int BPF_PROG(ia32_compat_openat_e, struct pt_regs* regs, long int retval) {
	return ia32_handler(regs, retval);
}
